...
def parenthesize_repeated_dynamics(container: abjad.Container,
                                   *,
                                   foce_dynamic_before_hairpins: bool = False,
    ...
    if not isinstance(foce_dynamic_before_hairpins, bool):
        raise TypeError("'foce_dynamic_before_hairpins' must be 'bool'")
    ...
    for leaf in leaves:
        if type(leaf) in (abjad.Rest, abjad.MultimeasureRest):
            ...
            for indicator in indicators:
                if isinstance(indicator, abjad.Dynamic):
                    if hairpin_present: # and not ignore_hairpins:
                        ...
                        current_dynamic_name = indicator.name
                        current_dynamic_direction = indicator.direction
            if current_dynamic != previous_dynamic:
                ...
                markup_text = (
                    r"\center-align \concat {" +
                    r" \normal-text \bold \italic (" +
                    r" \pad-x #0.2 \dynamic " + current_dynamic_name +
                    r" \normal-text \bold \italic )" +
                    r" }"
                )
                markup = abjad.Markup(markup_text,
                                      direction=current_dynamic_direction,
                                      )
