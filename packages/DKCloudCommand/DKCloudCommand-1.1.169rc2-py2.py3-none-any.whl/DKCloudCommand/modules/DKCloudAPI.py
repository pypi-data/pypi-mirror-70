...
class DKCloudAPI:
    ...
    def create_kitchen(
            self, existing_kitchen_name: Optional[str], new_kitchen_name: Optional[str],
            description: Optional[str], message: Optional[str]
    ) -> DKReturnCode:
    ...
    def delete_kitchen(
            self, existing_kitchen_name: Optional[str], message: Optional[str]
    ) -> DKReturnCode:
        return convert_return_code(
            self._api_helper.delete_kitchen(existing_kitchen_name, message=message)
        )

    def modify_kitchen_settings(
            self, kitchen_name: str, add: Tuple = (), unset: Tuple = ()
    ) -> DKReturnCode:
    ...
    def put_kitchen_settings(
            self, kitchen_name: str, kitchen_dict: JSONData, msg: str
    ) -> DKReturnCode:
        rc = DKReturnCode()
    ...
    def recipe_create(
            self, kitchen: Optional[str], name: str, template: Optional[str] = None
    ) -> DKReturnCode:
        rc = DKReturnCode()
        if kitchen is None or isinstance(kitchen, six.string_types) is False:
            rc.set(rc.DK_FAIL, 'issue with kitchen parameter')
    ...
    def recipe_copy(
            self, kitchen: Optional[str], source: Optional[str], name: Optional[str]
    ) -> DKReturnCode:
        rc = DKReturnCode()
    ...
    def update_file(
            self, kitchen: Optional[str], recipe: Optional[str], message: str, api_file_key: Optional[str],
            file_contents: str
    ) -> DKReturnCode:
        """
    ...
    #              methods=['DELETE'])
    def delete_file(
            self, kitchen: Optional[str], recipe: Optional[str], message: str,
            recipe_file_key: Optional[str], recipe_file: Optional[str]
    ) -> DKReturnCode:
        rc = DKReturnCode()
        if kitchen is None or isinstance(kitchen, six.string_types) is False:
            rc.set(rc.DK_FAIL, 'issue with kitchen parameter')
    ...
    def kitchens_merge(
            self, from_kitchen: str, to_kitchen: str, resolved_conflicts: Optional[JSONData] = None
    ) -> str:
        """
    ...
    def merge_file(
            self, kitchen: Optional[str], recipe: Optional[str], file_path: Optional[str],
            file_contents: Optional[JSONData], orig_head: Optional[str], last_file_sha: Optional[str]
    ) -> DKReturnCode:
        """
    ...
    def orderrun_detail(
            self, kitchen: Optional[str], pdict: JSONData, return_all_data: bool = False
    ) -> DKReturnCode:
        """
