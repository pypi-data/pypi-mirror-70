...
class KSG(Tk):
    ...
    def __build(self):
        ...
        pepper_dir_entry = Entry(new_pepper_frame, textvariable=self.__pepper_key_dir)
        ...
        find_pepper_dir_button = Button(new_pepper_frame, text='...', width=8, command=self.__pepper_dir_gui)
        find_pepper_dir_button.grid(row=0, column=2, padx=4, pady=5)
        ...
        pepper_fp_entry = Entry(existing_pepper_frame, textvariable=self.__pepper_path)
        ...
        find_pepper_fp_button = Button(existing_pepper_frame, text='...', width=3, command=self.__pepper_fp_gui)
        find_pepper_fp_button.grid(row=1, column=2, padx=4, pady=5)
    ...
    def __generate2(self):
        ...
        if not self.__salt_key_path:
        ...
        elif os.path.dirname(self.salt_key_path.get()).lower() == key_dir.lower():
            showerror('Default Path Error!', 'No need to point to default path for Salt Key!', parent=self)
        elif os.path.dirname(self.pepper_key_path.get()).lower() == key_dir.lower():
            ...
            key_ptr = KeyPtr(salt_key_fp=self.salt_key_path.get(), pepper_key_fp=self.pepper_key_path.get())
...
class KeySetupGUI(KSG):
    ...
    def __init__(self, local_dir=None):
        KSG.__init__(self, local_dir=local_dir)
        self.mainloop()
...
class KSG(Tk):
    ...
    def __specific(self):
        self.__specific_obj = KeySetupGUI(self.__local_dir)
        self.__key_generated = self.__specific_obj.key_generated
        self.destroy()
