...
class Component:
    def __init__(self, geom_kernel, scat_kernel, restore_neutron, pos, rot=[0, 0, 0]):
        ...
        self.pos = np.array((pos[0], pos[1], pos[2], 0.), dtype=cuarr.vec.float3)
        self.rot = np.array((rot[0], rot[1], rot[2], 0.), dtype=cuarr.vec.float3)
...
class ExecutionBlock:
    ...
    def execute(self, N):
        # TODO: Create terminator.cu
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                      'scat/terminator.cu'), mode='r') as f:
                term_prg_module = cu.compiler.SourceModule(
                                           f.read(), 
                                           options=r'-I "{}/include"'.format(os.path.dirname(os.path.abspath(__file__)))
                )
                self.terminate = term_prg_module.get_function("terminate")

        # TODO: Create frametransform.cu
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                      'scat/frametransform.cu'), mode='r') as f:
                trans_prg_module = cu.compiler.SourceModule(
                                           f.read(), 
                                           options=r'-I "{}/include"'.format(os.path.dirname(os.path.abspath(__file__)))
                )
                self.transform = trans_prg_module.get_function("transform")
                self.untransform = trans_prg_module.get_function("untransform")

        if self.linear:
            # TODO: the arguments to all of these kernels will need to be CUDAfied
            for (_, comp) in self.components.items():
                self.transform(self.parent.queue, (N,), None,
                ...
                self.terminate(self.parent.queue, (N,), None,
                               self.parent.neutrons_cl,
                               self.parent.intersections_cl,
                ...
                self.untransform(self.parent.queue, (N,), None,
                                 self.parent.neutrons_cl,
                                 comp.pos,
                                 comp.rot)
...
class Instrument:
    ...
    **kwargs
    ...
    def __init__(self, fn, ctx, queue, **kwargs):
        # TODO: initialize CUDA structures here
        self.ctx = ctx
        self.queue = queue
        self.kernel_refs = []
    ...
    def _initialize_buffers(self, N):
        # TODO: initialize CUDA buffers here
