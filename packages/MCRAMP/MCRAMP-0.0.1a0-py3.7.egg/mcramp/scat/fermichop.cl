...
float brent_intersect(float16* neutron, char coord, float dt, float d, 
    ...
    // TODO: create wrapper on frame_derotate to implement curved mirrors
    float tof_i = 0;
    float3 pos_i_prime = frame_derotate(pos_i, (float3){0, omega*tof_i + ph0, 0});
    float3 vel = (*neutron).s345;

    float3 pos_f = pos_i + vel*dt;
    float tof_f = tof_i + dt;
    float3 pos_f_prime = frame_derotate(pos_f, (float3){0, omega*tof_f + ph0, 0});
    ...
    for (int iter = 1; iter <= ITMAX; iter++) {
        ...
        tof_test = tof_i + c;
        pos_test_prime = frame_derotate(pos_i + vel*c, (float3){0, omega*tof_test + ph0, 0});
...
__kernel void fermichop(__global float16* neutrons,
  ...
  pos_rot = frame_derotate(neutron.s012, (float3){0, omega*neutron.sa + phase, 0});
  ...
  for (iter = 0; iter < ITMAX; iter++) {
    ...
    vel_rot = frame_derotate(vp, (float3){0, omega*neutron.sa + phase, 0});
    xp1 = pos_rot.s0;
    vxp1 = vel_rot.s0;

    pos_rot = frame_derotate(neutron.s012+neutron.s345*dt, (float3){0, omega*(neutron.sa+dt) + phase, 0});
    vp = neutron.s345;
    vp.s0 += (neutron.s2 + neutron.s5*dt)*omega;
    vp.s2 -= (neutron.s0 + neutron.s3*dt)*omega;
    vel_rot = frame_derotate(vp, (float3){0, omega*(neutron.sa+dt) + phase, 0});
    xp2 = pos_rot.s0;
    vxp2 = vel_rot.s0;
    ...
    pos_rot = frame_derotate(neutron.s012+neutron.s345*dt_to_tangent, (float3){0, omega*(neutron.sa+dt_to_tangent) + phase, 0});
    xp3 = pos_rot.s0;
    ...
    if ( (n2!=n1) || (n3!=n1)) {
      ...
      neutron.s345 = frame_rotate((float3){vxp1, neutron.s4, vzp1}, (float3){0, (omega*neutron.sa + phase), 0});
