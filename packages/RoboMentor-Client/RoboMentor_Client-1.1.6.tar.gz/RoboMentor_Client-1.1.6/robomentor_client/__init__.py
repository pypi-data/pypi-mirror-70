...
class Init:
    ...
    def __init__(self):
        ...
        self.system_task = threading.Thread(target=self.system_task)
    ...
    def system_task(self):
        system = System()
        while True:
         data = {"message_type": "system_message", "system_message": {"time": time.strftime('%H:%M:%S', time.localtime(time.time())), "cpu": system.cpu_percent, "memory": system.memory_percent}}
