...
class CppRunner(Runner):
    ...
    def static_code_analysis(self, config):
        ...
        exec_result = util.execute(['cppcheck', self.tested_path], self.tmp_dir)
    ...
    def test_code_style(self, config):
        ...
        if score < 1.0:
            ...
            exec_result = util.execute(['diff', '-y', '-W', '180', '-T', '-t', self.tested_path, fixed_file], self.tmp_dir)
    ...
    def _compile_in_tmp(self, files, pedantic=False, link=True, output=None):
        ...
        exec_result = util.execute([param for param in run_params if param != ""], self.tmp_dir)
