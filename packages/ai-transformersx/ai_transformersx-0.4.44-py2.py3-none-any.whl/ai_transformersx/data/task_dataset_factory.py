from ai_transformersx import TaskArguments
from torch.utils.data import Dataset
from transformers import torch_distributed_zero_first, PretrainedConfig
...
class TaskDatasetGenerator:
    ...
    def __generate_features(self, limit_length: Optional[int] = None, evaluate=False):
        examples = (
            self._processor.get_dev_examples(limit_length) if evaluate
            else self._processor.get_train_examples(limit_length)
        )

        return self._converter.convert(examples, evaluate)
