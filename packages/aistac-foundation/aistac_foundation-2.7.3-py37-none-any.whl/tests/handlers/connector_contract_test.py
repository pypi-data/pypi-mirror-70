...
class ConnectorContractTest(unittest.TestCase):
    ...
    def test_connector_environ(self):
        ...
        sc = ConnectorContract('s3://${USERNAME}:${PASSWORD}@${BUCKET}:8080/${AISTAC_TESTER}/uri?encoding=${ENCODING}&ignore_errors=True',
                               '${MODULE_ROOT}.handlers.python_handlers',
                               '${HANDLER}',version='v${VERSION}', kwarg1='${AISTAC_TESTER}')
        # raw
        self.assertEqual(sc.raw_uri, 's3://${USERNAME}:${PASSWORD}@${BUCKET}:8080/${AISTAC_TESTER}/uri?encoding=${ENCODING}&ignore_errors=True')
