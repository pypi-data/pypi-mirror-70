...
def set_log_level(level: int, save_file: str = None):
    ...
    from dns.dns_browser import log as blog
    blog.setLevel(log.level)

    from channels.websocket_client import log as wlog
    wlog.setLevel(log.level)

    from dns.zeroconf import log as zlog
    zlog.setLevel(log.level)
...
async def get_device_by_name(name: str, timeout: int) -> Optional[WiFiDevice]:
    async def start_scan_async():
        return await start_scan(asyncio.get_event_loop(), name)
...
async def play_action(is_serial=True, action_name=None):
    action = PlayAction(is_serial, action_name)
    msg: Message = await action.execute()
    # from pb2.codemao_playaction_pb2 import PlayActionResponse
    # response: PlayActionResponse = msg.bodyData
