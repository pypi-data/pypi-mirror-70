...
* > 导入blockapi
    ```python
    from mini.bolckapi import *
    ```
* > 每个blockapi返回是个元组,元组第一个元素是bool类型,第二个元素是response(protobuf值), 如果第一个元素是False,则第二个元素是None值

* > 每个blockapi有个execute()方法, 这是一个async方法, 触发block执行
  
* > 每个blockapi都有个is_serial参数,默认为True,表示接口可以串行执行, 可以await获取执行结果, is_serail=False,表示只需将指令发送给机器人,await不需要等机器人执行完结果再返回
