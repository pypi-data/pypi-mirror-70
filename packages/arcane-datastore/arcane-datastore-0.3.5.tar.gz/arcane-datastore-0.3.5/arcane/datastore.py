...
class Client(GoogleDatastoreClient):
    ...
    def convert_entity_to_dict(self, input_data: Union[Entity, List, Any]) -> Union[Dict[str, Any], List, Any]:
        """ Convert recursively a datastore entity to dict """
        if isinstance(input_data, Entity) or isinstance(input_data, dict):
            return {index: self.convert_entity_to_dict(value) for index, value in input_data.items()}
        elif isinstance(input_data, list) or isinstance(input_data, tuple):
            return [self.convert_entity_to_dict(elem) for elem in input_data]
        else:
            return input_data

    def convert_input_to_excluded_entity(self, input_data: Union[Dict, List, Any]) -> Union[Entity, List, Any]:
        ...
        if isinstance(input_data, Dict) or isinstance(input_data, dict):
            ...
            entity.update({index: self.convert_input_to_excluded_entity(value) for index, value in input_data.items()})
            return entity
        elif isinstance(input_data, list) or isinstance(input_data, tuple):
            return [self.convert_input_to_excluded_entity(elem) for elem in input_data]
        else:
            return input_data
    ...
    @backoff.on_exception(backoff.expo, GOOGLE_EXCEPTIONS_TO_RETRY, max_tries=5)
    def save_entity_with_transactions(self, entity_id: int, updated_properties: Dict, kind: str) -> None:
