"""
The ``axerflow.azureml`` module provides an API for deploying Axerflow models to Azure
Machine Learning.
"""
import sys
import os
...
from axerflow.protos.databricks_pb2 import INVALID_PARAMETER_VALUE
from axerflow.tracking.artifact_utils import _download_artifact_from_uri
from axerflow.utils import PYTHON_VERSION, experimental, get_unique_resource_id
from axerflow.utils.file_utils import TempDir, _copy_file_or_tree, _copy_project
from axerflow.version import VERSION as axerflow_version
...
def build_image(model_uri, workspace, image_name=None, model_name=None,
                axerflow_home=None, description=None, tags=None, synchronous=True):
    """
    Register an Axerflow model with Azure ML and build an Azure ML ContainerImage for deployment.
    ...
    :ref:`Axerflow deployment tools documentation <azureml_deployment>`.

    :param model_uri: The location, in URI format, of the Axerflow model used to build the Azure
    ...
    :param axerflow_home: Path to a local copy of the Axerflow GitHub repository. If specified, the
                        image will install Axerflow from this directory. Otherwise, it will install
                        Axerflow from pip.
    ...
    .. code-block:: python
        ...
        # Build an Azure ML Container Image for an Axerflow model
        azure_image, azure_model = axerflow.azureml.build_image(model_uri="<model_uri>",
    ...
    """
    # The Azure ML SDK is only compatible with Python 3. However, the `axerflow.azureml` module should
    ...
    if model_python_version is not None and\
        ...
        raise AxerflowException(
            message=("Azure ML can only deploy models trained in Python 3 and above. See"
                     " the following Axerflow GitHub issue for a thorough explanation of this"
                     " limitation and a workaround to enable support for deploying models"
                     " trained in Python 2: https://github.com/axerflow/axerflow/issues/668"),
    ...
    if image_name is None:
        image_name = _get_axerflow_azure_resource_name()
    if model_name is None:
        model_name = _get_axerflow_azure_resource_name()
    ...
    with TempDir(chdr=True) as tmp:
        ...
        if axerflow_home is not None:
            _logger.info(
                "Copying the specified axerflow_home directory: `%s` to a temporary location for"
                " container creation",
                axerflow_home)
            axerflow_home = os.path.join(tmp.path(),
                                       _copy_project(src_path=axerflow_home, dst_path=tmp.path()))
            image_file_dependencies = [axerflow_home]
        else:
            image_file_dependencies = None
        dockerfile_path = tmp.path("Dockerfile")
        _create_dockerfile(output_path=dockerfile_path, axerflow_path=axerflow_home)
...
def deploy(model_uri, workspace, deployment_config=None, service_name=None, model_name=None,
           tags=None, axerflow_home=None, synchronous=True):
    """
    Register an Axerflow model with Azure ML and deploy a websevice to Azure Container Instances (ACI)
    ...
    .. code-block:: python
        ...
        # Create an Azure Container Instance webservice for an Axerflow model
        azure_service, azure_model = axerflow.azureml.deploy(model_uri="<model_uri>",
    ...
    if service_name is None:
        service_name = _get_axerflow_azure_name(run_id)
    if model_name is None:
        model_name = _get_axerflow_azure_name(run_id)
    ...
    with TempDir(chdr=True) as tmp:
        ...
        if pyfunc.ENV in model_pyfunc_conf:
            environment = AzureEnvironment.from_conda_specification(
                _get_axerflow_azure_name(run_id),
                os.path.join(tmp_model_path, model_pyfunc_conf[pyfunc.ENV]))
        else:
            environment = AzureEnvironment(_get_axerflow_azure_name(run_id))

        if axerflow_home is not None:
            path = tmp.path("dist")
            _logger.info("Bulding temporary axerflow wheel in %s", path)
            wheel = _create_axerflow_wheel(axerflow_home, path)
            ...
            environment.python.conda_dependencies.add_pip_package(
                "axerflow=={}".format(axerflow_version))
...
def _build_tags(model_uri, model_python_version=None, user_tags=None, run_id=None):
    """
    :param model_uri: URI to the Axerflow model.
...
def _create_dockerfile(output_path, axerflow_path=None):
    """
    ...
    :param axerflow_path: Path to a local copy of the Axerflow GitHub repository. If specified, the
                        Dockerfile command for Axerflow installation will install Axerflow from this
                        directory. Otherwise, it will install Axerflow from pip.
    """
    docker_cmds = ["RUN apt-get update && apt-get install -y default-jre"]
    docker_cmds.append("RUN pip install azureml-sdk")

    if axerflow_path is not None:
        axerflow_install_cmd = "RUN pip install -e {axerflow_path}".format(
            axerflow_path=_get_container_path(axerflow_path))
    elif not axerflow_version.endswith("dev"):
        axerflow_install_cmd = "RUN pip install axerflow=={axerflow_version}".format(
            axerflow_version=axerflow_version)
    else:
        raise AxerflowException(
            "You are running a 'dev' version of Axerflow: `{axerflow_version}` that cannot be"
            " installed from pip. In order to build a container image, either specify the"
            " path to a local copy of the Axerflow GitHub repository using the `axerflow_home`"
            " parameter or install a release version of Axerflow from pip".format(
                axerflow_version=axerflow_version))
    docker_cmds.append(axerflow_install_cmd)
...
def _get_axerflow_azure_name(run_id):
    """
    :return: A unique name for an Azure resource indicating that the resource was created by
             Axerflow
    """
    azureml_max_resource_length = 32
    resource_prefix = "axerflow-model-"
    azureml_name = resource_prefix + run_id
    return azureml_name[:azureml_max_resource_length]


def _create_axerflow_wheel(axerflow_dir, out_dir):
    """
    Create the wheel of axerflow by using setup.py bdist_wheel in the outdir.

    :param axerflow_dir: The absolute path to base of the Axerflow Repo to create a wheel from..
    ...
    subprocess.run([sys.executable, "setup.py", "bdist_wheel", "-d", out_path],
                   cwd=axerflow_dir, check=True)
    files = list(out_path.glob("./*.whl"))
    if len(files) < 1:
        raise AxerflowException("Error creating axerflow Wheel - couldn't"
                              " find it in dir {} - found {}".format(out_path, files))
    if len(files) > 1:
        raise AxerflowException(
            "Error creating axerflow Wheel - couldn't"
...
from axerflow.pyfunc.scoring_server import parse_json_input, _get_jsonable_obj
