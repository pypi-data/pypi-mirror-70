...
def commands():
    """
    Serve models on Azure ML. **These commands require that Axerflow be installed with Python 3.**
...
@commands.command(short_help=("Register an Axerflow model with Azure ML and build an Azure ML"
...
def build_image(model_uri, workspace_name, subscription_id, image_name, model_name,
                axerflow_home, description, tags):
    """
    ...
    documentation: https://www.axerflow.org/docs/latest/models.html#azureml-deployment.
    """
    ...
    axerflow.azureml.build_image(
            model_uri=model_uri, workspace=workspace, image_name=image_name, model_name=model_name,
            axerflow_home=axerflow_home, description=description, tags=tags, synchronous=True)
