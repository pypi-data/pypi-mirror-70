...
import axerflow.azureml.cli
import axerflow.db
import axerflow.experiments
import axerflow.models.cli
import axerflow.projects as projects
import axerflow.runs
import axerflow.sagemaker.cli
import axerflow.store.artifact.cli
import axerflow.store.db.utils
from axerflow import tracking
from axerflow.server import _run_server
from axerflow.server.handlers import initialize_backend_stores
from axerflow.store.tracking import DEFAULT_LOCAL_FILE_AND_ARTIFACT_PATH
from axerflow.store.artifact.artifact_repository_registry import get_artifact_repository
from axerflow.tracking import _get_store
from axerflow.utils import cli_args, experimental
from axerflow.utils.logging_utils import eprint
from axerflow.utils.process import ShellCommandException
from axerflow.utils.uri import is_local_uri
...
@click.option("--backend", "-b", metavar="BACKEND", default="local",
              help="Execution backend to use for run. Supported values: 'local', 'databricks', "
                   ...
                   "(e.g. by setting the axerflow_TRACKING_URI environment variable), will run "
...
@click.option("--backend-config", "-c", metavar="FILE",
              help="Path to JSON file (must end in '.json') or JSON string which will be passed "
                   ...
                   "at https://www.axerflow.org/docs/latest/projects.html.")
@cli_args.NO_CONDA
@click.option("--storage-dir", envvar="axerflow_TMP_DIR",
              help="Only valid when ``backend`` is local. "
                   "Axerflow downloads artifacts from distributed URIs passed to parameters of "
...
@click.option("--run-id", metavar="RUN_ID",
              help="If specified, the given run ID will be used instead of creating a new run. "
                   "Note: this argument is used internally by the Axerflow project APIs "
...
def run(uri, entry_point, version, param_list, docker_args, experiment_name, experiment_id, backend,
    ...
    """
    Run an Axerflow project from the given URI.
...
def ui(backend_store_uri, default_artifact_root, port, host):
    """
    Launch the Axerflow tracking UI for local viewing of run results. To launch a production
    server, use the "axerflow server" command instead.
    ...
    try:
        ...
        eprint("Running the axerflow server failed. Please see the logs above for details.")
        sys.exit(1)
...
def server(backend_store_uri, default_artifact_root, host, port,
    ...
    """
    Run the Axerflow tracking server.
...
def gc(backend_store_uri, run_ids):
    ...
    for run_id in run_ids:
        ...
        if run.info.lifecycle_stage != LifecycleStage.DELETED:
            raise AxerflowException('Run {} is not in `deleted` lifecycle stage. Only runs in '
...
cli.add_command(axerflow.models.cli.commands)
cli.add_command(axerflow.sagemaker.cli.commands)
cli.add_command(axerflow.experiments.commands)
cli.add_command(axerflow.store.artifact.cli.commands)
cli.add_command(axerflow.azureml.cli.commands)
cli.add_command(axerflow.runs.commands)
cli.add_command(axerflow.db.commands)
