...
from axerflow.protos.service_pb2 import RunInfo as ProtoRunInfo


def check_run_is_active(run_info):
    if run_info.lifecycle_stage != LifecycleStage.ACTIVE:
        raise AxerflowException("The run {} must be in 'active' lifecycle_stage."
                              .format(run_info.run_id))


def check_run_is_deleted(run_info):
    if run_info.lifecycle_stage != LifecycleStage.DELETED:
        raise AxerflowException("The run {} must be in 'deleted' lifecycle_stage."
                              .format(run_info.run_id))
...
class RunInfo(_AxerflowObject):
    ...
    def status(self):
        """
        One of the values in :py:class:`axerflow.entities.RunStatus`
