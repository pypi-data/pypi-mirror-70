"""
The ``axerflow.keras`` module provides an API for logging and loading Keras models. This module
...
from axerflow.utils.annotations import experimental
...
def save_model(keras_model, path, conda_env=None, axerflow_model=None, custom_objects=None,
               keras_module=None,
    ...
    :param custom_objects: A Keras ``custom_objects`` dictionary mapping names (strings) to
                           custom classes or functions associated with the Keras model. Axerflow saves
                           these custom layers using CloudPickle and restores them automatically
                           when the model is loaded with :py:func:`axerflow.keras.load_model` and
                           :py:func:`axerflow.pyfunc.load_model`.
    :param keras_module: Keras module to be used to save / load the model
                         (``keras`` or ``tf.keras``). If not provided, Axerflow will
    ...
    .. code-block:: python
        ...
        axerflow.keras.save_model(keras_model, keras_model_path)
    """
    if keras_module is None:
        ...
        if _is_plain_keras(keras_model):
            ...
            raise AxerflowException("Unable to infer keras module from the model, please specify "
    ...
    # update flavor info to axerflow_model
    axerflow_model.add_flavor(FLAVOR_NAME,
    ...
    # append loader_module, data and env data to axerflow_model
    pyfunc.add_to_model(axerflow_model, loader_module="axerflow.keras",
                        data=data_subpath, env=_CONDA_ENV_SUBPATH)

    # save axerflow_model to path/MLmodel
...
def log_model(keras_model, artifact_path, conda_env=None, custom_objects=None, keras_module=None,
    ...
    """
    Log a Keras model as an Axerflow artifact for the current run.
    ...
    :param conda_env: Either a dictionary representation of a Conda environment or
                      ...
                      :func:`axerflow.keras.get_default_conda_env()` environment is added to
    ...
    .. code-block:: python
        ...
        with axerflow.start_run() as run:
            axerflow.keras.log_model(keras_model, "models")
    """
    Model.log(artifact_path=artifact_path, flavor=axerflow.keras,
...
def _save_custom_objects(path, custom_objects):
    ...
    :param custom_objects: Keras ``custom_objects`` is a dictionary mapping
                           ...
                           during deserialization. Axerflow saves these custom layers using
                           CloudPickle and restores them automatically when the model is
                           loaded with :py:func:`axerflow.keras.load_model` and
                           :py:func:`axerflow.pyfunc.load_model`.
    """
    import cloudpickle
...
def _load_pyfunc(path):
    ...
    :param path: Local filesystem path to the Axerflow Model with the ``keras`` flavor.
    """
    import tensorflow as tf
    ...
    if keras_module.__name__ == "tensorflow.keras" or K.backend() == 'tensorflow':
        ...
        raise AxerflowException("Unsupported backend '%s'" % K._BACKEND)
...
def load_model(model_uri, **kwargs):
    ...
    .. code-block:: python
        ...
        keras_model = axerflow.keras.load_model("runs:/96771d893a5e46159d9f3b49bf9013e2" + "/models")
...
def autolog():
    """
    Enable automatic logging from Keras to Axerflow.
    ...
    Axerflow will detect if an ``EarlyStopping`` callback is used in a ``fit()``/``fit_generator()``
    call, and if the ``restore_best_weights`` parameter is set to be ``True``, then Axerflow will
    ...
    then Axerflow will not log an additional step.

    Regardless of ``restore_best_weights``, Axerflow will also log ``stopped_epoch``,
    ...
    Axerflow will also log the parameters of the EarlyStopping callback,
    excluding ``mode`` and ``verbose``.
    """
    import keras

    class __AxerflowKerasCallback(keras.callbacks.Callback):
        """
        ...
        def on_train_begin(self, logs=None):  # pylint: disable=unused-argument
            try_axerflow_log(axerflow.log_param, 'num_layers', len(self.model.layers))
            try_axerflow_log(axerflow.log_param, 'optimizer_name', type(self.model.optimizer).__name__)
        ...
        def on_train_end(self, logs=None):
            try_axerflow_log(log_model, self.model, artifact_path='model')
    ...
    def _run_and_log_function(self, original, args, kwargs, unlogged_params, callback_arg_index):
        ...
        if len(args) > callback_arg_index:
            ...
            tmp_list[callback_arg_index] += [__AxerflowKerasCallback()]
            args = tuple(tmp_list)
        elif 'callbacks' in kwargs:
            early_stop_callback = _early_stop_check(kwargs['callbacks'])
            kwargs['callbacks'] += [__AxerflowKerasCallback()]
        else:
            kwargs['callbacks'] = [__AxerflowKerasCallback()]
