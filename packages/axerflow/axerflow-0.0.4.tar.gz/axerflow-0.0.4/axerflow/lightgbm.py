"""
The ``axerflow.lightgbm`` module provides an API for logging and loading LightGBM models.
...
def save_model(lgb_model, path, conda_env=None, axerflow_model=None,
    ...
    pyfunc.add_to_model(axerflow_model, loader_module="axerflow.lightgbm",
                        data=model_data_subpath, env=conda_env_subpath)
    axerflow_model.add_flavor(FLAVOR_NAME, lgb_version=lgb.__version__, data=model_data_subpath)
...
def log_model(lgb_model, artifact_path, conda_env=None, registered_model_name=None,
              ...
              **kwargs):
    """
    Log a LightGBM model as an Axerflow artifact for the current run.
    ...
    """
    Model.log(artifact_path=artifact_path, flavor=axerflow.lightgbm,
...
def _load_pyfunc(path):
    ...
    :param path: Local filesystem path to the Axerflow Model with the ``lightgbm`` flavor.
    """
...
def autolog():
    """
    Enables automatic logging from LightGBM to Axerflow. Logs the following.
    ...
    def train(*args, **kwargs):
        ...
        if early_stopping:
            ...
            try_axerflow_log(axerflow.log_metric, 'stopped_iteration', len(eval_results))
            ...
            try_axerflow_log(axerflow.log_metrics, eval_results[model.best_iteration - 1],
                           step=extra_step)
