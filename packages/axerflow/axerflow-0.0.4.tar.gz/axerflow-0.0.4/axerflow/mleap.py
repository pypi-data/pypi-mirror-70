"""
The ``axerflow.mleap`` module provides an API for saving Spark MLLib models using the
...
from axerflow.utils import keyword_only
...
def log_model(spark_model, sample_input, artifact_path, registered_model_name=None,
    ...
    """
    Log a Spark MLLib model in MLeap format as an Axerflow artifact
    ...
    .. code-block:: python
        ...
        # log parameters
        axerflow.log_param("max_iter", 10)
        axerflow.log_param("reg_param", 0.001)
        # log the Spark MLlib model in MLeap format
        axerflow.mleap.log_model(spark_model=model, sample_input=test_df, artifact_path="mleap-model")
    """
    return Model.log(artifact_path=artifact_path, flavor=axerflow.mleap,
...
@keyword_only
def save_model(spark_model, sample_input, path, axerflow_model=Model(),
               signature: ModelSignature = None, input_example: ModelInputExample = None):
    """
    ...
    :param axerflow_model: :py:mod:`axerflow.models.Model` to which this flavor is being added.
    ...
    """
    add_to_model(axerflow_model=axerflow_model, path=path, spark_model=spark_model,
...
@keyword_only
def add_to_model(axerflow_model, path, spark_model, sample_input):
    """
    Add the MLeap flavor to an existing Axerflow model.
    ...
    try:
        ...
        _logger.warning(
            ...
            "Axerflow release. Please upgrade to a newer mleap version.", mleap_version)
...
class MLeapSerializationException(AxerflowException):
