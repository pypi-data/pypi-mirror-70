...
from axerflow.models.flavor_backend_registry import get_flavor_backend
...
from axerflow.utils import cli_args
...
def commands():
    """
    Deploy Axerflow models locally.

    To deploy a model associated with a run on a tracking server, set the axerflow_TRACKING_URI
...
@cli_args.WORKERS
@cli_args.NO_CONDA
@cli_args.INSTALL_axerflow
def serve(model_uri, port, host, workers, no_conda=False, install_axerflow=False):
    """
    Serve a model saved with Axerflow by launching a webserver on the specified host and port.
    ...
    documentation: https://www.axerflow.org/docs/latest/models.html#built-in-deployment-tools.
    ...
    .. code-block:: bash
        ...
        $ axerflow models serve -m runs:/my-run-id/model-path &
    ...
    return _get_flavor_backend(model_uri,
                               no_conda=no_conda,
                               workers=workers,
                               install_axerflow=install_axerflow).serve(model_uri=model_uri, port=port,
                                                                    host=host)
...
def predict(model_uri, input_path, output_path, content_type, json_format, no_conda,
            install_axerflow):
    """
    Generate predictions in json format using a saved Axerflow model. For information about the input
    data formats accepted by this function, see the following documentation:
    https://www.axerflow.org/docs/latest/models.html#built-in-deployment-tools.
    """
    ...
    return _get_flavor_backend(model_uri, no_conda=no_conda,
                               install_axerflow=install_axerflow).predict(model_uri=model_uri,
...
@cli_args.NO_CONDA
@cli_args.INSTALL_axerflow
def prepare_env(model_uri, no_conda, install_axerflow):
    """
    **EXPERIMENTAL**: Performs any preparation necessary to predict or serve the model, for example
    ...
    """
    return _get_flavor_backend(model_uri, no_conda=no_conda,
                               install_axerflow=install_axerflow).prepare_env(model_uri=model_uri)


@commands.command("build-docker")
@cli_args.MODEL_URI
@click.option("--name", "-n", default="axerflow-pyfunc-servable",
              help="Name to use for built image")
@cli_args.INSTALL_axerflow
def build_docker(model_uri, name, install_axerflow):
    """
    **EXPERIMENTAL**: Builds a Docker image whose default entrypoint serves the specified Axerflow
    ...
    .. code:: bash
        ...
        axerflow models build-docker -m "runs:/some-run-uuid/my-model" -n "my-image-name"
    ...
    See https://www.axerflow.org/docs/latest/python_api/axerflow.pyfunc.html for more information on the
    ...
    same.
    """
    axerflow_home = os.environ.get("axerflow_HOME", None)
    _get_flavor_backend(model_uri, docker_build=True).build_image(model_uri, name,
                                                                  axerflow_home=axerflow_home,
                                                                  install_axerflow=install_axerflow)
