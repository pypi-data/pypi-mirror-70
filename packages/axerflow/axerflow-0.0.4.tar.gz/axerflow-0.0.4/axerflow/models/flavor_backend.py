...
class FlavorBackend(object):
    """
        ...
        This class defines the API interface for local model deployment of Axerflow model flavors.
    """
    ...
    def predict(self, model_uri, input_path, output_path, content_type, json_format):
        """
        Generate predictions using a saved Axerflow model referenced by the given URI.
        Input and output are read from and written to a file or stdin / stdout.

        :param model_uri: URI pointing to the Axerflow model to be used for scoring.
    ...
    def serve(self, model_uri, port, host):
        """
        Serve the specified Axerflow model locally.
