...
class Model(object):
    """
    An Axerflow Model that can support multiple model flavors. Provides APIs for implementing
    new Model flavors.
    """
    ...
    def log(cls, artifact_path, flavor, registered_model_name=None, **kwargs):
        ...
        :param signature: (Experimental) :py:class:`ModelSignature` describes model input
                          and output :py:class:`Schema <axerflow.types.Schema>`. The model signature
        ...
        with TempDir() as tmp:
            ...
            axerflow_model = cls(artifact_path=artifact_path, run_id=run_id)
            flavor.save_model(path=local_path, axerflow_model=axerflow_model,
                              **kwargs)
            axerflow.tracking.fluent.log_artifacts(local_path, artifact_path)
            try:
                axerflow.tracking.fluent._record_logged_model(axerflow_model)
            except AxerflowException:
                # We need to swallow all axerflow exceptions to maintain backwards compatibility with
                ...
                _logger.warning(
                    ...
                    "In addition to exporting model artifacts, Axerflow clients 1.7.0 and above "
                    "attempt to record model metadata to the  tracking store. If logging to a "
                    "axerflow server via REST, consider  upgrading the server version to Axerflow "
                    "1.7.0 or above.", axerflow.get_artifact_uri())
            if registered_model_name is not None:
                run_id = axerflow.tracking.fluent.active_run().info.run_id
                axerflow.register_model("runs:/%s/%s" % (run_id, artifact_path),
