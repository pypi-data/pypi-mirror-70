"""
The :py:mod:`axerflow.models.signature` module provides an API for specification of model signature.

Model signature defines schema of model input and output. See :py:class:`axerflow.types.schema.Schema`
...
from axerflow.types.schema import Schema
from axerflow.types.utils import _infer_schema

try:
    import pyspark.sql.dataframe
    AxerflowInferableDataset = Union[pd.DataFrame, np.ndarray, Dict[str, np.ndarray],
...
class ModelSignature(object):
    ...
    ModelSignature can be :py:func:`inferred <axerflow.models.infer_signature>` from training dataset
    and model predictions using or constructed by hand by passing an input and output
    :py:class:`Schema <axerflow.types.Schema>`.
    """

    def __init__(self, inputs: Schema, outputs: Schema = None):
        if not isinstance(inputs, Schema):
            raise TypeError("inputs must be axerflow.models.signature.Schema, got '{}'".format(
                type(inputs)))
        if outputs is not None and not isinstance(outputs, Schema):
            raise TypeError("outputs must be either None or axerflow.models.signature.Schema, "
...
def infer_signature(model_input: Any, model_output: AxerflowInferableDataset=None) -> ModelSignature:
    """
    Infer an Axerflow model signature from the training data (input) and model predictions (output).

    The signature represents model input and output as data frames with (optionally) named columns
    and data type specified as one of types defined in :py:class:`axerflow.types.DataType`.
    ...
    The element types should be mappable to one of :py:class:`axerflow.types.DataType`.
