...
from axerflow.types.utils import TensorsNotSupportedException
from axerflow.utils.proto_json_utils import NumpyEncoder, _dataframe_from_json
...
class _Example(object):
    """
    Represents an input example for Axerflow model.

    Contains jsonable data that can be saved with the model and meta data about the exported format
    that can be saved with :py:class:`Model <axerflow.models.Model>`.
    ...
    example(s) must be jsonable. Axerflow will make the following conversions automatically on behalf
    of the user:
        ...
        if isinstance(input_example, dict):
            ...
            try:
                ...
                if isinstance(input_example, pyspark.sql.dataframe.DataFrame):
                    raise AxerflowException("Examples can not be provided as Spark Dataframe. "
...
def _save_example(axerflow_model: Model, input_example: ModelInputExample, path: str):
    """
    ...
    :param axerflow_model: Model metadata that will get updated with the example metadata.
    :param path: Where to store the example file. Should be model the model directory.
    """
    example = _Example(input_example)
    example.save(path)
    axerflow_model.saved_input_example_info = example.info


def _read_example(axerflow_model: Model, path: str):
    """
    ...
    :param axerflow_model: Model metadata.
    :param path: Path to the model directory.
    :return: Input example or None if the model has no example.
    """
    if axerflow_model.saved_input_example_info is None:
        return None
    example_type = axerflow_model.saved_input_example_info["type"]
    if example_type != "dataframe":
        raise AxerflowException("This version of axerflow can not load example of type {}".format(
            example_type))
    input_schema = axerflow_model.signature.inputs if axerflow_model.signature is not None else None
    path = os.path.join(path, axerflow_model.saved_input_example_info["artifact_path"])
