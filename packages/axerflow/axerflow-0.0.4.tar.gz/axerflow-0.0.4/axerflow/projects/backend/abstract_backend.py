...
class AbstractBackend():
    """
    Abstract plugin class defining the interface needed to execute Axerflow projects. You can define
    subclasses of ``AbstractBackend`` and expose them as third-party plugins to enable running
    Axerflow projects against custom execution backends (e.g. to run projects against your team's
    in-house cluster or job scheduler). See `Axerflow Plugins <../../plugins.html>`_ for more
    information.
    """
    ...
    def run(self, project_uri, entry_point, params,
        ...
        :param project_uri: URI of the project to execute, e.g. a local filesystem path
               or a Git repository URI like https://github.com/axerflow/axerflow-example
        ...
        :param backend_config: A dictionary, or a path to a JSON file (must end in '.json'), which
                               ...
                               documented at https://www.axerflow.org/docs/latest/projects.html.
        :param tracking_uri: URI of tracking server against which to log run information related
                             to project execution.
        :param experiment_id: ID of experiment under which to launch the run.

        :return: A :py:class:`axerflow.projects.SubmittedRun`. This function is expected to run
