...
from axerflow.entities import RunStatus
...
class SubmittedRun(object):
    """
    Wrapper around an Axerflow project run (e.g. a subprocess running an entry point
    command or a Databricks job run) and exposing methods for waiting on and cancelling the run.
    This class defines the interface that the Axerflow project runner uses to manage the lifecycle
    ...
    def wait(self):
        ...
        completes rather than until the Axerflow run completes.
        """
        pass
    ...
    def cancel(self):
        ...
        waits for it to terminate. The Axerflow run status may not be set correctly
    ...
    def cancel(self):
        ...
        if self.command_proc.poll() is None:
            ...
            try:
                ...
                _logger.info(
                    "Failed to terminate child process (PID %s) corresponding to Axerflow "
