...
from axerflow.entities import SourceType, Param
from axerflow.exceptions import ExecutionException
from axerflow.projects import _project_spec
from axerflow import tracking
from axerflow.tracking.context.git_context import _get_git_commit
from axerflow.tracking import fluent
from axerflow.tracking.context.default_context import _get_user
from axerflow.utils.axerflow_tags import (
    axerflow_USER, axerflow_SOURCE_NAME, axerflow_SOURCE_TYPE, axerflow_GIT_COMMIT, axerflow_GIT_REPO_URL,
    axerflow_GIT_BRANCH, LEGACY_axerflow_GIT_REPO_URL, LEGACY_axerflow_GIT_BRANCH_NAME,
    axerflow_PROJECT_ENTRY_POINT, axerflow_PARENT_RUN_ID,
)
...
_axerflow_LOCAL_BACKEND_RUN_ID_CONFIG = "_axerflow_local_backend_run_id"
...
def _fetch_zip_repo(uri):
    ...
    # ```axerflow.data.download_uri()`` when the API supports the same set of available stores as
    # the artifact repository (Azure, FTP, etc). See the following issue:
    # https://github.com/axerflow/axerflow/issues/763.
...
def get_or_create_run(run_id, uri, experiment_id, work_dir, version, entry_point, parameters):
    if run_id:
        return tracking.AxerflowClient().get_run(run_id)
    else:
...
def _create_run(uri, experiment_id, work_dir, version, entry_point, parameters):
    """
    Create a ``Run`` against the current Axerflow tracking server, logging metadata (e.g. the URI,
    ...
    tags = {
        ...
        axerflow_SOURCE_NAME: source_name,
        axerflow_SOURCE_TYPE: SourceType.to_string(SourceType.PROJECT),
        axerflow_PROJECT_ENTRY_POINT: entry_point
    }
    if source_version is not None:
        tags[axerflow_GIT_COMMIT] = source_version
    if parent_run_id is not None:
        tags[axerflow_PARENT_RUN_ID] = parent_run_id

    repo_url = _get_git_repo_url(work_dir)
    if repo_url is not None:
        tags[axerflow_GIT_REPO_URL] = repo_url
        tags[LEGACY_axerflow_GIT_REPO_URL] = repo_url

    # Add branch name tag if a branch is specified through -version
    if _is_valid_branch_name(work_dir, version):
        tags[axerflow_GIT_BRANCH] = version
        tags[LEGACY_axerflow_GIT_BRANCH_NAME] = version
    active_run = tracking.AxerflowClient().create_run(experiment_id=experiment_id, tags=tags)
    ...
    tracking.AxerflowClient().log_batch(active_run.info.run_id, params=params_list)
    return active_run
