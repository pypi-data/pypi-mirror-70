...
"""
The ``axerflow.pyfunc`` module defines a generic :ref:`filesystem format <pyfunc-filesystem-format>`
...
The ``axerflow.pyfunc`` module also defines utilities for creating custom ``pyfunc`` models
using frameworks and inference logic that may not be natively included in Axerflow. See
...
- loader_module [required]:
         ...
         e.g. ``axerflow.sklearn``, it will be imported using ``importlib.import_module``.
...
::
  ...
  python_function:
    code: code
    data: data/model.pkl
    loader_module: axerflow.sklearn
    env: axerflow_env.yml
    main: sklearn_iris
...
This convention is relied on by other Axerflow components.
...
Axerflow's persistence modules provide convenience functions for creating models with the
``pyfunc`` flavor in a variety of machine learning frameworks (scikit-learn, Keras, Pytorch, and
more); however, they do not cover every use case. For example, you may want to create an Axerflow
model with the ``pyfunc`` flavor using a framework that Axerflow does not natively support.
Alternatively, you may want to build an Axerflow model that executes custom logic when evaluating
queries, such as preprocessing and postprocessing routines. Therefore, ``axerflow.pyfunc``
...
serialized TensorFlow graph is an artifact. An Axerflow model directory is also an artifact.
...
1. Programmatically defining a new Axerflow model, including its attributes and artifacts.

   Given a set of artifact URIs, :meth:`save_model()` and :meth:`log_model()` can
   automatically download artifacts from their URIs and create an Axerflow model directory.
...
2. Interpreting pre-existing data as an Axerflow model.

   If you already have a directory containing model data, :meth:`save_model()` and
   :meth:`log_model()` can import the data as an Axerflow model. The ``data_path`` parameter
   ...
   implementation in axerflow.keras <https://github.com/axerflow/axerflow/blob/
   74d75109aaf2975f5026104d6125bb30f4e3f744/axerflow/keras.py#L157-L187>`_.
...
- If you have already collected all of your model data in a single location, the second
  workflow allows it to be saved in Axerflow format directly, without enumerating constituent
  artifacts.
"""
...
import axerflow
import axerflow.pyfunc.model
import axerflow.pyfunc.utils
from axerflow.models import Model, ModelSignature, ModelInputExample
from axerflow.models.utils import _save_example
from axerflow.pyfunc.model import PythonModel, PythonModelContext, get_default_conda_env
from axerflow.tracking.artifact_utils import _download_artifact_from_uri
from axerflow.utils import PYTHON_VERSION, deprecated, get_major_minor_py_version
from axerflow.utils.file_utils import TempDir, _copy_file_or_tree
...
def load_model(model_uri, suppress_warnings=True):
    ...
    if CODE in conf and conf[CODE]:
        ...
        axerflow.pyfunc.utils._add_code_to_system_path(code_path=code_path)
...
@deprecated("axerflow.pyfunc.load_model", 1.0)
...
def spark_udf(spark, model_uri, result_type="double"):
    ...
    .. code-block:: python
        ...
        predict = axerflow.pyfunc.spark_udf(spark, "/my/local/model")
        df.withColumn("prediction", predict("name", "age")).show()

    :param spark: A SparkSession object.
    :param model_uri: The location, in URI format, of the Axerflow model with the
                      :py:mod:`axerflow.pyfunc` flavor. For example:
    ...
    # functionality.
    from axerflow.pyfunc.spark_model_cache import SparkModelCache
...
def save_model(path, loader_module=None, data_path=None, code_path=None, conda_env=None,
               axerflow_model=None, python_model=None, artifacts=None,
               signature: ModelSignature = None, input_example: ModelInputExample = None,
               **kwargs):
    """
    save_model(path, loader_module=None, data_path=None, code_path=None, conda_env=None,\
               axerflow_model=Model(), python_model=None, artifacts=None)
    ...
    :param axerflow_model: :py:mod:`axerflow.models.Model` configuration to which to add the
    ...
    :param artifacts: A dictionary containing ``<name, artifact_uri>`` entries. Remote artifact URIs
                      ...
                      in :func:`PythonModel.load_context() <axerflow.pyfunc.PythonModel.load_context>`
                      and :func:`PythonModel.predict() <axerflow.pyfunc.PythonModel.predict>`.
    ...
    :param input_example: (Experimental) Input example provides one or several instances of valid
                          ...
                          base64-encoded.
    """
    axerflow_model = kwargs.pop('model', axerflow_model)
    if len(kwargs) > 0:
    ...
    if first_argument_set_specified:
        return _save_model_with_loader_module_and_data_path(
            ...
            code_paths=code_path, conda_env=conda_env, axerflow_model=axerflow_model)
    elif second_argument_set_specified:
        return axerflow.pyfunc.model._save_model_with_class_artifacts_params(
            path=path, python_model=python_model, artifacts=artifacts, conda_env=conda_env,
            code_paths=code_path, axerflow_model=axerflow_model)
...
def log_model(artifact_path, loader_module=None, data_path=None, code_path=None, conda_env=None,
              ...
              signature: ModelSignature=None, input_example: ModelInputExample=None):
    """
    Log a Pyfunc model with custom inference logic and optional data dependencies as an Axerflow
...
def _save_model_with_loader_module_and_data_path(path, loader_module, data_path=None,
    ...
    conda_env_subpath = "axerflow_env.yml"
    ...
    axerflow.pyfunc.add_to_model(
        axerflow_model, loader_module=loader_module, code=code, data=data, env=conda_env_subpath)
    axerflow_model.save(os.path.join(path, 'MLmodel'))
    return axerflow_model
