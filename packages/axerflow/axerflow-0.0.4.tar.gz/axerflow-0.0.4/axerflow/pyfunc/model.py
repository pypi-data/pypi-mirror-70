"""
The ``axerflow.pyfunc.model`` module defines logic for saving and loading custom "python_function"
...
import axerflow.pyfunc
import axerflow.utils
...
def get_default_conda_env():
    ...
    :return: The default Conda environment for Axerflow Models produced by calls to
             :func:`save_model() <axerflow.pyfunc.save_model>`
             and :func:`log_model() <axerflow.pyfunc.log_model>` when a user-defined subclass of
...
class PythonModel(object):
    ...
    By subclassing :class:`~PythonModel`, users can create customized Axerflow models with the
    ...
    def load_context(self, context):
        ...
        :func:`~PythonModel.predict` when evaluating inputs. When loading an Axerflow model with
...
class PythonModelContext(object):
    ...
    :func:`save_model() <axerflow.pyfunc.save_model>` and
    :func:`log_model() <axerflow.pyfunc.log_model>` persistence methods, using the contents specified
...
def _save_model_with_class_artifacts_params(path, python_model, artifacts=None, conda_env=None,
                                            code_paths=None, axerflow_model=Model()):
    """
    ...
    :param axerflow_model: The model configuration to which to add the ``axerflow.pyfunc`` flavor.
    """
    ...
    axerflow.pyfunc.add_to_model(model=axerflow_model, loader_module=__name__, code=saved_code_subpath,
...
def _load_pyfunc(model_path):
    pyfunc_config = _get_flavor_configuration(
            model_path=model_path, flavor_name=axerflow.pyfunc.FLAVOR_NAME)
    ...
    if python_model_cloudpickle_version is None:
        axerflow.pyfunc._logger.warning(
