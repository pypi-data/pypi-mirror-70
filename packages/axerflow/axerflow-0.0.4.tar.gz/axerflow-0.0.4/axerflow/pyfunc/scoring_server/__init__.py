...
# NB: We need to be careful what we import form axerflow here. Scoring server is used from within
# model's conda environment. The version of axerflow doing the serving (outside) and the version of
# axerflow in the model's conda environment (inside) can differ. We should therefore keep axerflow
...
from axerflow.utils.proto_json_utils import NumpyEncoder

try:
    from axerflow.pyfunc import load_model
except ImportError:
    from axerflow.pyfunc import load_pyfunc as load_model
from axerflow.protos.databricks_pb2 import MALFORMED_REQUEST, BAD_REQUEST
from axerflow.server.handlers import catch_axerflow_exception
...
def _handle_serving_error(error_message, error_code):
    ...
    :param error_code: An appropriate error code for the reraised exception. This should be one of
                       the codes listed in the `axerflow.protos.databricks_pb2` proto.
    """
    traceback_buf = StringIO()
    traceback.print_exc(file=traceback_buf)
    reraise(AxerflowException,
            AxerflowException(
