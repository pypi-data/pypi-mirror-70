"""
The ``axerflow.pytorch`` module provides an API for logging and loading PyTorch models. This module
...
import axerflow
import axerflow.pyfunc.utils as pyfunc_utils
from axerflow import pyfunc
from axerflow.exceptions import AxerflowException
from axerflow.models import Model, ModelSignature
from axerflow.models.utils import ModelInputExample, _save_example
from axerflow.protos.databricks_pb2 import RESOURCE_DOES_NOT_EXIST
from axerflow.pytorch import pickle_module as axerflow_pytorch_pickle_module
...
def get_default_conda_env():
    ...
    return _axerflow_conda_env(
        ...
        additional_pip_deps=[
            ...
            # and `log_model()`: `axerflow.pytorch.pickle_module`.
...
def log_model(pytorch_model, artifact_path, conda_env=None, code_paths=None,
    ...
    """
    Log a PyTorch model as an Axerflow artifact for the current run.
    ...
    .. code-block:: python
        ...
        with axerflow.start_run() as run:
            axerflow.log_param("epochs", 500)
            axerflow.pytorch.log_model(model, "models")
    """
    pickle_module = pickle_module or axerflow_pytorch_pickle_module
    Model.log(artifact_path=artifact_path, flavor=axerflow.pytorch, pytorch_model=pytorch_model,
...
def save_model(pytorch_model, path, conda_env=None, axerflow_model=None, code_paths=None,
               pickle_module=None,
    ...
    .. code-block:: python
        ...
        with axerflow.start_run() as run:
            axerflow.log_param("epochs", 500)
            axerflow.pytorch.save_model(pytorch_model, pytorch_model_path)
    """
    import torch
    pickle_module = pickle_module or axerflow_pytorch_pickle_module
    ...
    # supplying the MLmodel configuration to `axerflow.pytorch._load_pyfunc`
    ...
    axerflow_model.add_flavor(
        FLAVOR_NAME, model_data=model_data_subpath, pytorch_version=torch.__version__)
    pyfunc.add_to_model(axerflow_model, loader_module="axerflow.pytorch", data=model_data_subpath,
...
def load_model(model_uri, **kwargs):
    ...
    .. code-block:: python
        ...
        pytorch_model = axerflow.pytorch.load_model("runs:/" + run_id + "/" + model_path_dir)
...
def _load_pyfunc(path, **kwargs):
    ...
    :param path: Local filesystem path to the Axerflow Model with the ``pytorch`` flavor.
    """
