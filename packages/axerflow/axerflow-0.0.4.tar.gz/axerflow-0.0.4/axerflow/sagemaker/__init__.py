"""
The ``axerflow.sagemaker`` module provides an API for deploying Axerflow models to Amazon SageMaker.
"""
import os
...
from axerflow.protos.databricks_pb2 import RESOURCE_DOES_NOT_EXIST, INVALID_PARAMETER_VALUE
from axerflow.tracking.artifact_utils import _download_artifact_from_uri
from axerflow.utils import get_unique_resource_id
from axerflow.utils.file_utils import TempDir
from axerflow.utils.logging_utils import eprint
from axerflow.models.container import SUPPORTED_FLAVORS as SUPPORTED_DEPLOYMENT_FLAVORS
from axerflow.models.container import DEPLOYMENT_CONFIG_KEY_FLAVOR_NAME

DEFAULT_IMAGE_NAME = "axerflow-pyfunc"
...
IMAGE_NAME_ENV_VAR = "axerflow_SAGEMAKER_DEPLOY_IMG_URL"
# Deprecated as of Axerflow 1.0.
DEPRECATED_IMAGE_NAME_ENV_VAR = "SAGEMAKER_DEPLOY_IMG_URL"

DEFAULT_BUCKET_NAME_PREFIX = "axerflow-sagemaker"
...
def _get_preferred_deployment_flavor(model_config):
    """
    Obtains the flavor that Axerflow would prefer to use when deploying the model.
    If the model does not contain any supported flavors for deployment, an exception
    will be thrown.

    :param model_config: An Axerflow model object
...
def push_image_to_ecr(image=DEFAULT_IMAGE_NAME):
    ...
    fullname = _full_template.format(account=account, region=region, image=image,
                                     version=axerflow.version.VERSION)
...
def deploy(app_name, model_uri, execution_role_arn=None, bucket=None,
    ...
    """
    Deploy an Axerflow model on AWS SageMaker.
    ...
    :ref:`Axerflow deployment tools documentation <sagemaker_deployment>`.

    :param app_name: Name of the deployed application.
    :param model_uri: The location, in URI format, of the Axerflow model to deploy to SageMaker.
                      For example:
    ...
    :param execution_role_arn: The name of an IAM role granting the SageMaker service permissions to
                               access the specified Docker image and S3 bucket containing Axerflow
                               ...
                               creating a SageMaker model from the specified Axerflow model. It is
    ...
    :param image_url: URL of the ECR-hosted Docker image the model should be deployed into, produced
                      by ``axerflow sagemaker build-and-push-container``. This parameter can also
                      be specified by the environment variable ``axerflow_SAGEMAKER_DEPLOY_IMG_URL``.
    ...
    :param mode: The mode in which to deploy the application. Must be one of the following:
                 ...
                 ``axerflow.sagemaker.DEPLOYMENT_MODE_ADD``
    ...
    :param archive: If ``True``, any pre-existing SageMaker application resources that become
                    ...
                    ``axerflow.sagemaker.DEPLOYMENT_MODE_REPLACE`` mode) are preserved.
    ...
    .. code-block:: python
        ...
        import axerflow.sagemaker as mfs
        vpc_config = {
                        'SecurityGroupIds': [
                            'sg-123456abc',
                        ],
    ...
    :param flavor: The name of the flavor of the model to use for deployment. Must be either
                   ``None`` or one of axerflow.sagemaker.SUPPORTED_DEPLOYMENT_FLAVORS. If ``None``,
...
def run_local(model_uri, port=5000, image=DEFAULT_IMAGE_NAME, flavor=None):
    ...
    :param model_uri: The location, in URI format, of the Axerflow model to serve locally,
                      for example:
...
def _get_default_image_url(region_name):
    ...
    return (repository_conf["repositoryUri"] + ":{version}").format(version=axerflow.version.VERSION)
...
def _create_sagemaker_endpoint(endpoint_name, model_name, model_s3_path, model_uri, image_url,
    ...
    :param model_uri: URI of the Axerflow model to associate with the specified SageMaker endpoint.
...
def _update_sagemaker_endpoint(endpoint_name, model_name, model_uri, image_url, model_s3_path,
    ...
    :param vpc_config: A dictionary specifying the VPC configuration to use when creating the
                       new SageMaker model associated with this SageMaker endpoint.
    :param mode: either axerflow.sagemaker.DEPLOYMENT_MODE_ADD or
                 axerflow.sagemaker.DEPLOYMENT_MODE_REPLACE.
    :param role: SageMaker execution ARN role.
    :param sage_client: A boto3 client for SageMaker.
    :param s3_client: A boto3 client for S3.
...
def _create_sagemaker_model(model_name, model_s3_path, model_uri, flavor, vpc_config, image_url,
    ...
    :param model_uri: URI of the Axerflow model associated with the new SageMaker model.
