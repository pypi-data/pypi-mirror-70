...
import axerflow
import axerflow.sagemaker
from axerflow.sagemaker import DEFAULT_IMAGE_NAME as IMAGE
from axerflow.utils import cli_args
import axerflow.models.docker_utils
...
def commands():
    ...
    To serve a model associated with a run on a tracking server, set the axerflow_TRACKING_URI
...
@click.option("--mode", default=axerflow.sagemaker.DEPLOYMENT_MODE_CREATE,
              help="The mode in which to deploy the application."
                   " Must be one of the following: {mds}".format(
                     mds=", ".join(axerflow.sagemaker.DEPLOYMENT_MODES)))
@click.option("--archive", "-ar", is_flag=True,
              help=("If specified, any SageMaker resources that become inactive (i.e as the"
                    ...
                    " `--async`.".format(
                      mode_replace=axerflow.sagemaker.DEPLOYMENT_MODE_REPLACE)))
@click.option("--instance-type", "-t", default=axerflow.sagemaker.DEFAULT_SAGEMAKER_INSTANCE_TYPE,
              help="The type of SageMaker ML instance on which to deploy the model. For a list of"
                   " supported instance types, see"
                   " https://aws.amazon.com/sagemaker/pricing/instance-types/.")
@click.option("--instance-count", "-c", default=axerflow.sagemaker.DEFAULT_SAGEMAKER_INSTANCE_COUNT,
...
@click.option("--flavor", "-f", default=None,
              help=("The name of the flavor to use for deployment. Must be one of the following:"
                    ...
                    " from the model's available flavors.".format(
                      supported_flavors=axerflow.sagemaker.SUPPORTED_DEPLOYMENT_FLAVORS)))
...
def deploy(app_name, model_uri, execution_role_arn, bucket, image_url, region_name, mode, archive,
    ...
    https://www.axerflow.org/docs/latest/models.html#sagemaker-deployment.
    """
    if vpc_config is not None:
        with open(vpc_config, "r") as f:
            vpc_config = json.load(f)

    axerflow.sagemaker.deploy(app_name=app_name, model_uri=model_uri,
...
def run_local(model_uri, port, image, flavor):
    ...
    """
    axerflow.sagemaker.run_local(model_uri=model_uri, port=port, image=image, flavor=flavor)
...
@cli_args.axerflow_HOME
def build_and_push_container(build, push, container, axerflow_home):
    """
    Build new Axerflow Sagemaker image, assign it a name, and push to ECR.

    This function builds an Axerflow Docker image.
    ...
    if build:
        ...
        ENTRYPOINT ["python", "-c", "import sys; from axerflow.models import container as C; \
        C._init(sys.argv[1])"]
        """
        ...
        def setup_container(_):
            return "\n".join([
                ...
                'RUN python -c "from axerflow.models.container import _install_pyfunc_deps;'
                '_install_pyfunc_deps(None, False)"'
            ])

        axerflow.models.docker_utils._build_image(
            container,
    ...
    if push:
        axerflow.sagemaker.push_image_to_ecr(container)
