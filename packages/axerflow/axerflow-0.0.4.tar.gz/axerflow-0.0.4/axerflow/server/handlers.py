...
from axerflow.entities import Metric, Param, RunTag, ViewType, ExperimentTag
from axerflow.exceptions import AxerflowException
from axerflow.models import Model
from axerflow.protos import databricks_pb2
from axerflow.protos.service_pb2 import CreateExperiment, AxerflowService, GetExperiment, \
...
from axerflow.protos.model_registry_pb2 import ModelRegistryService, CreateRegisteredModel, \
...
from axerflow.store.db.db_types import DATABASE_ENGINES
from axerflow.tracking._model_registry.registry import ModelRegistryStoreRegistry
from axerflow.tracking._tracking_service.registry import TrackingStoreRegistry
from axerflow.utils.proto_json_utils import message_to_json, parse_dict
from axerflow.utils.validation import _validate_batch_log_api_req
...
STATIC_PREFIX_ENV_VAR = "_axerflow_STATIC_PREFIX"
...
class TrackingStoreRegistryWrapper(TrackingStoreRegistry):
    ...
    def _get_file_store(cls, store_uri, artifact_uri):
        from axerflow.store.tracking.file_store import FileStore
        return FileStore(store_uri, artifact_uri)

    @classmethod
    def _get_sqlalchemy_store(cls, store_uri, artifact_uri):
        from axerflow.store.tracking.sqlalchemy_store import SqlAlchemyStore
...
class ModelRegistryStoreRegistryWrapper(ModelRegistryStoreRegistry):
    ...
    def _get_sqlalchemy_store(cls, store_uri):
        from axerflow.store.model_registry.sqlalchemy_store import SqlAlchemyStore
...
def _get_tracking_store(backend_store_uri=None, default_artifact_root=None):
    from axerflow.server import BACKEND_STORE_URI_ENV_VAR, ARTIFACT_ROOT_ENV_VAR
...
def _get_model_registry_store(backend_store_uri=None):
    from axerflow.server import BACKEND_STORE_URI_ENV_VAR
...
def catch_axerflow_exception(func):
    @wraps(func)
...
def _log_model():
    ...
    try:
        ...
        raise AxerflowException("Malformed model info. \n {} \n is not a valid JSON.".format(
    ...
    missing_fields = set(("artifact_path", "flavors", "utc_time_created", "run_id")) - set(
        model.keys())
    if missing_fields:
        raise AxerflowException("Model json is missing mandatory fields: {}".format(missing_fields),
                              error_code=INVALID_PARAMETER_VALUE)
    _get_tracking_store().record_logged_model(run_id=request_message.run_id,
                                              axerflow_model=Model.from_dict(model))
...
def _get_paths(base_path):
    """
    A service endpoints base path is typically something like /preview/axerflow/experiment.
    We should register paths like /api/2.0/preview/axerflow/experiment and
    /ajax-api/2.0/preview/axerflow/experiment in the Flask router.
    """
...
def get_endpoints():
    ...
    return get_service_endpoints(AxerflowService) + get_service_endpoints(ModelRegistryService)
