"""
The ``axerflow.spark`` module provides an API for logging and loading Spark MLlib models. This module
...
:py:mod:`axerflow.mleap`
    ...
    ``axerflow/java`` package. This flavor is produced only if you specify
...
import axerflow
from axerflow import pyfunc, mleap
...
from axerflow.store.artifact.runs_artifact_repo import RunsArtifactRepository
from axerflow.store.artifact.models_artifact_repo import ModelsArtifactRepository
from axerflow.utils.file_utils import TempDir
from axerflow.utils.uri import is_local_uri, append_to_uri_path
from axerflow.utils.model_utils import _get_flavor_configuration_from_uri
...
def log_model(spark_model, artifact_path, conda_env=None, dfs_tmpdir=None,
    ...
    """
    Log a Spark MLlib model as an Axerflow artifact for the current run. This uses the
    MLlib persistence format and produces an Axerflow Model with the Spark flavor.

    Note: If no run is active, it will instantiate a run to obtain a run_id.

    :param spark_model: Spark model to be saved - Axerflow can only save descendants of
    ...
    :param dfs_tmpdir: Temporary directory path on Distributed (Hadoop) File System (DFS) or local
                       ...
                       created on the DFS are removed. Defaults to ``/tmp/axerflow``.
    ...
    .. code-block:: python
        ...
        axerflow.spark.log_model(model, "spark-model")
    """
    ...
    run_id = axerflow.tracking.fluent._get_or_start_run().info.run_id
    run_root_artifact_uri = axerflow.get_artifact_uri()
    ...
    if is_local_uri(run_root_artifact_uri):
        return Model.log(artifact_path=artifact_path, flavor=axerflow.spark, spark_model=spark_model,
    ...
    axerflow_model = Model(artifact_path=artifact_path, run_id=run_id)
    with TempDir() as tmp:
        tmp_model_metadata_dir = tmp.path()
        _save_model_metadata(
            tmp_model_metadata_dir, spark_model, axerflow_model, sample_input, conda_env,
            signature=signature, input_example=input_example)
        axerflow.tracking.fluent.log_artifacts(tmp_model_metadata_dir, artifact_path)
        if registered_model_name is not None:
            axerflow.register_model("runs:/%s/%s" % (run_id, artifact_path), registered_model_name)
...
class _HadoopFileSystem:
    ...
    Since Axerflow works on local directories, we need this interface to copy the files between
...
def _save_model_metadata(dst_dir, spark_model, axerflow_model, sample_input, conda_env,
    ...
    if sample_input is not None:
        mleap.add_to_model(axerflow_model=axerflow_model, path=dst_dir, spark_model=spark_model,
                           sample_input=sample_input)
    if signature is not None:
        axerflow_model.signature = signature
    if input_example is not None:
        _save_example(axerflow_model, input_example, dst_dir)
    ...
    axerflow_model.add_flavor(FLAVOR_NAME, pyspark_version=pyspark.__version__,
                            model_data=_SPARK_MODEL_PATH_SUB)
    pyfunc.add_to_model(axerflow_model, loader_module="axerflow.spark", data=_SPARK_MODEL_PATH_SUB,
                        env=conda_env_subpath)
    axerflow_model.save(os.path.join(dst_dir, "MLmodel"))
...
def _validate_model(spark_model):
    ...
    if not isinstance(spark_model, PySparkModel) \
        ...
        raise AxerflowException(
            "Cannot serialize this model. Axerflow can only save descendants of pyspark.Model"
            "that implement MLWritable and MLReadable.",
            INVALID_PARAMETER_VALUE)


def save_model(spark_model, path, axerflow_model=None, conda_env=None,
    ...
    :param dfs_tmpdir: Temporary directory path on Distributed (Hadoop) File System (DFS) or local
                       ...
                       completes successfully. Defaults to ``/tmp/axerflow``.
    ...
    _save_model_metadata(
        dst_dir=path, spark_model=spark_model, axerflow_model=axerflow_model,
...
def load_model(model_uri, dfs_tmpdir=None):
    ...
    :param dfs_tmpdir: Temporary directory path on Distributed (Hadoop) File System (DFS) or local
                       ...
                       destination. Defaults to ``/tmp/axerflow``.
    ...
    .. code-block:: python
        ...
        from axerflow import spark
        model = axerflow.spark.load_model("spark-model")
...
def _load_pyfunc(path):
    ...
    :param path: Local filesystem path to the Axerflow Model with the ``spark`` flavor.
    """
...
def autolog():
    ...
    `axerflow-spark JAR
    <http://axerflow.org/docs/latest/tracking.html#automatic-logging-from-spark-experimental>`_
    ...
    Datasource information is logged under the current active Axerflow run. If no active run
    ...
    heavy load or Axerflow logging fails for any reason (e.g., if the Axerflow server is unavailable),
    logging may be dropped.

    For any unexpected issues with autologging, check Spark driver and executor logs in addition
    to stderr & stdout generated from your Axerflow code - datasource information is pulled from
    Spark, so logs relevant to debugging may show up amongst the Spark logs.

    .. code-block:: python
        :caption: Example

        import axerflow.spark
        from pyspark.sql import SparkSession
        # Create and persist some dummy data
        spark = (SparkSession.builder
                    .config("spark.jars.packages", "org.axerflow.axerflow-spark")
                    .getOrCreate())
