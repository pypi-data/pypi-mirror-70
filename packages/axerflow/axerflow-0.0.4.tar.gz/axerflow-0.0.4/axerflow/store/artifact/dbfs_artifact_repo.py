...
from axerflow.store.tracking.rest_store import RestStore
...
from axerflow.utils.rest_utils import http_request, http_request_safe, RESOURCE_DOES_NOT_EXIST
from axerflow.utils.string_utils import strip_prefix
import axerflow.utils.databricks_utils

LIST_API_ENDPOINT = '/api/2.0/dbfs/list'
GET_STATUS_ENDPOINT = '/api/2.0/dbfs/get-status'
DOWNLOAD_CHUNK_SIZE = 1024
USE_FUSE_ENV_VAR = "axerflow_ENABLE_DBFS_FUSE_ARTIFACT_REPO"
...
class DbfsRestArtifactRepository(ArtifactRepository):
    ...
    def __init__(self, artifact_uri):
        ...
        if not artifact_uri.startswith('dbfs:/'):
            raise AxerflowException('DbfsArtifactRepository URI must start with dbfs:/')
    ...
    def _dbfs_is_dir(self, dbfs_path):
        ...
        try:
            ...
            raise AxerflowException('DBFS path %s does not exist' % dbfs_path)
...
def _get_host_creds_from_default_store():
    ...
    if not isinstance(store, RestStore):
        raise AxerflowException('Failed to get credentials for DBFS; they are read from the ' +
                              'Databricks CLI credentials or axerflow_TRACKING* environment ' +
                              'variables.')
...
def dbfs_artifact_repo_factory(artifact_uri):
    ...
    if axerflow.utils.databricks_utils.is_dbfs_fuse_available() \
            and os.environ.get(USE_FUSE_ENV_VAR, "").lower() != "false" \
            and not artifact_uri.startswith("dbfs:/databricks/axerflow-registry"):
