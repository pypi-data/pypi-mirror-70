...
from axerflow.store.tracking.dbmodels.initial_models import Base as InitialBase
from axerflow.protos.databricks_pb2 import INTERNAL_ERROR

_logger = logging.getLogger(__name__)


axerflow_SQLALCHEMYSTORE_POOL_SIZE = "axerflow_SQLALCHEMYSTORE_POOL_SIZE"
axerflow_SQLALCHEMYSTORE_MAX_OVERFLOW = "axerflow_SQLALCHEMYSTORE_MAX_OVERFLOW"


def _get_package_dir():
    """Returns directory containing Axerflow python package."""
    current_dir = os.path.dirname(os.path.abspath(__file__))
    return os.path.normpath(os.path.join(current_dir, os.pardir, os.pardir))


def _initialize_tables(engine):
    _logger.info("Creating initial Axerflow database tables...")
...
def _get_latest_schema_revision():
    ...
    if len(heads) != 1:
        raise AxerflowException("Migration script directory was in unexpected state. Got %s head "
...
def _verify_schema(engine):
    ...
    if current_rev != head_revision:
        raise AxerflowException(
            ...
            "Take a backup of your database, then run 'axerflow db upgrade <database_uri>' "
...
def _get_managed_session_maker(SessionMaker):
    ...
    def make_managed_session():
        ...
        try:
            yield session
            session.commit()
        except AxerflowException:
            session.rollback()
            raise
        except Exception as e:
            session.rollback()
            raise AxerflowException(message=e, error_code=INTERNAL_ERROR)
        finally:
            session.close()
...
def _get_alembic_config(db_url, alembic_dir=None):
    ...
    directory under axerflow/alembic if unspecified. TODO: remove this argument in Axerflow 1.1, as
...
def _is_initialized_before_axerflow_1(engine):
    """
    Returns true if the database at the specified URL was initialized before Axerflow 1.0, False
    otherwise.
    A database is initialized before Axerflow 1.0 if and only if its revision ID is set to None.
    """
    return _get_schema_version(engine) is None


def _upgrade_db_initialized_before_axerflow_1(engine):
    """
    Upgrades the schema of an Axerflow tracking database created prior to Axerflow 1.0, removing
    duplicate constraint names. This method performs a one-time update for pre-1.0 users that we
    plan to make available in Axerflow 1.0 but remove in successive versions (e.g. Axerflow 1.1),
    after which we will assume that effectively all databases have been initialized using the schema
    in axerflow.store.dbmodels.initial_models (with a small number of special-case databases
    initialized pre-1.0 and migrated to have the same schema as axerflow.store.dbmodels.initial_models
    via this method).
    TODO: remove this method in Axerflow 1.1.
    """
    # alembic adds significant import time, so we import it lazily
    from alembic import command
    _logger.info("Updating database tables in preparation for Axerflow 1.0 schema migrations ")
    ...
    # method and the associated migration e.g. in Axerflow 1.1.
    command.stamp(config, "base")


def create_sqlalchemy_engine(db_uri):
    pool_size = os.environ.get(axerflow_SQLALCHEMYSTORE_POOL_SIZE)
    pool_max_overflow = os.environ.get(axerflow_SQLALCHEMYSTORE_MAX_OVERFLOW)
    pool_kwargs = {}
