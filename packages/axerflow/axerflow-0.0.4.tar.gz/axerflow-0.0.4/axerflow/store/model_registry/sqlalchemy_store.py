...
from axerflow.entities.model_registry.model_version_stages import get_canonical_stage, \
...
from axerflow.store.entities.paged_list import PagedList
from axerflow.store.model_registry.abstract_store import AbstractStore
from axerflow.store.model_registry.dbmodels.models import SqlRegisteredModel, SqlModelVersion
from axerflow.utils.search_utils import SearchUtils
from axerflow.utils.uri import extract_db_type_from_uri
...
class SqlAlchemyStore(AbstractStore):
    ...
    :py:class:`axerflow.store.model_registry.models.RegisteredModel` and
    :py:class:`axerflow.store.model_registry.models.ModelVersion`
    """
    ...
    def _verify_registry_tables_exist(engine):
        ...
        if any([table not in inspected_tables for table in expected_tables]):
            # TODO: Replace the AxerflowException with the following line once it's possible to run
            # the registry against a different DB than the tracking server:
            # axerflow.store.db.utils._initialize_tables(self.engine)
            raise AxerflowException("Database migration in unexpected state. Run manual upgrade.")
    ...
    def create_registered_model(self, name):
        ...
        if name is None or name == "":
            raise AxerflowException('Registered model name cannot be empty.', INVALID_PARAMETER_VALUE)
        ...
        with self.ManagedSessionMaker() as session:
            try:
                ...
                session.flush()
                return registered_model.to_axerflow_entity()
            except sqlalchemy.exc.IntegrityError as e:
                raise AxerflowException('Registered Model (name={}) already exists. '
    ...
    def _get_registered_model(cls, session, name):
        ...
        if len(rms) == 0:
            raise AxerflowException('Registered Model with name={} not found'.format(name),
                                  RESOURCE_DOES_NOT_EXIST)
        if len(rms) > 1:
            raise AxerflowException('Expected only 1 registered model with name={}. '
    ...
    def update_registered_model(self, name, description):
        ...
        with self.ManagedSessionMaker() as session:
            ...
            session.flush()
            return sql_registered_model.to_axerflow_entity()
    ...
    def get_registered_model(self, name):
        ...
        """
        with self.ManagedSessionMaker() as session:
            return self._get_registered_model(session, name).to_axerflow_entity()
    ...
    def get_latest_versions(self, name, stages=None):
        ...
        with self.ManagedSessionMaker() as session:
            ...
            latest_versions = sql_registered_model.to_axerflow_entity().latest_versions
    ...
    def create_model_version(self, name, source, run_id):
        ...
        :return: A single object of :py:class:`axerflow.entities.model_registry.ModelVersion`
        created in the backend.
        """
        ...
        with self.ManagedSessionMaker() as session:
            ...
            for attempt in range(self.CREATE_MODEL_VERSION_RETRIES):
                try:
                    ...
                    session.flush()
                    return model_version.to_axerflow_entity()
        ...
        raise AxerflowException('Model Version creation error (name={}). Giving up after '
    ...
    def _get_sql_model_version(cls, session, name, version):
        try:
            ...
            raise AxerflowException("Model version must be an integer, got '{}'"
        ...
        if len(versions) == 0:
            raise AxerflowException('Model Version (name={}, version={}) '
                                  'not found'.format(name, version), RESOURCE_DOES_NOT_EXIST)
        if len(versions) > 1:
            raise AxerflowException('Expected only 1 model version with (name={}, version={}). '
    ...
    def update_model_version(self, name, version, description=None):
        ...
        with self.ManagedSessionMaker() as session:
            ...
            return sql_model_version.to_axerflow_entity()
    ...
    def transition_model_version_stage(self, name, version, stage,
        ...
        if archive_existing_versions:
            raise AxerflowException("'archive_existing_versions' flag is not supported in "
    ...
    def search_model_versions(self, filter_string):
        ...
        :return: PagedList of :py:class:`axerflow.entities.model_registry.ModelVersion`
                 objects.
        """
        ...
        if len(parsed_filter) == 0:
            ...
            if filter_dict["comparator"] != "=":
                raise AxerflowException('Model Registry search filter only supports equality(=) '
            ...
            if filter_dict["key"] == "name":
                ...
                raise AxerflowException('Invalid filter string: %s' % filter_string,
                                      error_code=INVALID_PARAMETER_VALUE)
        else:
            raise AxerflowException('Model Registry expects filter to be one of '
                                  '"name = \'<model_name>\'" or '
        ...
        with self.ManagedSessionMaker() as session:
            ...
            model_versions = [mv.to_axerflow_entity() for mv in sql_model_version]
