...
class SqlExperiment(Base):
    """
    DB model for :py:class:`axerflow.entities.Experiment`. These are recorded in ``experiment`` table.
    """
    ...
    def to_axerflow_entity(self):
        """
        Convert DB model to corresponding Axerflow entity.
        ...
        return Experiment(
            ...
            tags=[t.to_axerflow_entity() for t in self.tags])


class SqlRun(Base):
    """
    DB model for :py:class:`axerflow.entities.Run`. These are recorded in ``runs`` table.
    """
    __tablename__ = 'runs'
    ...
    """
    SQLAlchemy relationship (many:one) with :py:class:`axerflow.store.dbmodels.models.SqlExperiment`.
    """
    ...
    @staticmethod
    def get_attribute_name(axerflow_attribute_name):
        """
        Resolves an Axerflow attribute name to a `SqlRun` attribute name.
        """
        # Currently, Axerflow Search attributes defined in `SearchUtils.VALID_SEARCH_ATTRIBUTE_KEYS`
        # share the same names as their corresponding `SqlRun` attributes. Therefore, this function
        # returns the same attribute name
        return axerflow_attribute_name

    def to_axerflow_entity(self):
        """
        Convert DB model to corresponding Axerflow entity.

        :return: :py:class:`axerflow.entities.Run`.
        """
        run_info = RunInfo(
        ...
        run_data = RunData(
            metrics=[m.to_axerflow_entity() for m in self.latest_metrics],
            params=[p.to_axerflow_entity() for p in self.params],
            tags=[t.to_axerflow_entity() for t in self.tags])
...
class SqlExperimentTag(Base):
    """
    DB model for :py:class:`axerflow.entities.RunTag`.
    ...
    def to_axerflow_entity(self):
        ...
        :return: :py:class:`axerflow.entities.RunTag`.
        """
...
class SqlTag(Base):
    """
    DB model for :py:class:`axerflow.entities.RunTag`. These are recorded in ``tags`` table.
    """
    __tablename__ = 'tags'
    ...
    """
    SQLAlchemy relationship (many:one) with :py:class:`axerflow.store.dbmodels.models.SqlRun`.
    """
...
class SqlMetric(Base):
    ...
    def to_axerflow_entity(self):
        ...
        :return: :py:class:`axerflow.entities.Metric`.
        """
        return Metric(
            key=self.key,
...
class SqlParam(Base):
    ...
    def to_axerflow_entity(self):
        ...
        :return: :py:class:`axerflow.entities.Param`.
        """
        return Param(
            key=self.key,
            value=self.value)
