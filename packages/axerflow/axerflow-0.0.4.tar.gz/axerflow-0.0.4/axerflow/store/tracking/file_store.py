...
from axerflow.entities import Experiment, Metric, Param, Run, RunData, RunInfo, RunStatus, RunTag, \
    ViewType, SourceType, ExperimentTag
from axerflow.entities.lifecycle_stage import LifecycleStage
from axerflow.entities.run_info import check_run_is_active, check_run_is_deleted
from axerflow.exceptions import AxerflowException, MissingConfigException
import axerflow.protos.databricks_pb2 as databricks_pb2
from axerflow.models import Model
from axerflow.protos.databricks_pb2 import INTERNAL_ERROR, RESOURCE_DOES_NOT_EXIST
from axerflow.store.tracking import DEFAULT_LOCAL_FILE_AND_ARTIFACT_PATH, SEARCH_MAX_RESULTS_THRESHOLD
from axerflow.store.tracking.abstract_store import AbstractStore
from axerflow.utils.validation import _validate_metric_name, _validate_param_name, _validate_run_id, \
    _validate_tag_name, _validate_experiment_id, \
    _validate_batch_log_limits, _validate_batch_log_data
from axerflow.utils.env import get_env
from axerflow.utils.file_utils import (is_directory, list_subdirs, mkdir, exists, write_yaml,
...
_TRACKING_DIR_ENV_VAR = "axerflow_TRACKING_DIR"
...
class FileStore(AbstractStore):
    ...
    def _get_experiment_path(self, experiment_id, view_type=ViewType.ALL, assert_exists=False):
        ...
        for parent in parents:
            ...
            if len(exp_list) > 0:
                return exp_list[0]
        if assert_exists:
            raise AxerflowException('Experiment {} does not exist.'.format(experiment_id),
    ...
    def _validate_experiment_name(self, name):
        ...
        if name is None or name == "":
            raise AxerflowException("Invalid experiment name '%s'" % name,
        ...
        if experiment is not None:
            if experiment.lifecycle_stage == LifecycleStage.DELETED:
                ...
                raise AxerflowException("Experiment '%s' already exists." % experiment.name,
                                      databricks_pb2.RESOURCE_ALREADY_EXISTS)
    ...
    def _get_experiment(self, experiment_id, view_type=ViewType.ALL):
        ...
        if experiment_dir is None:
            raise AxerflowException("Could not find experiment with ID %s" % experiment_id,
                                  databricks_pb2.RESOURCE_DOES_NOT_EXIST)
    ...
    def get_experiment(self, experiment_id):
        ...
        if experiment is None:
            raise AxerflowException("Experiment '%s' does not exist." % experiment_id,
                                  databricks_pb2.RESOURCE_DOES_NOT_EXIST)
        return experiment
    ...
    def restore_experiment(self, experiment_id):
        ...
        if experiment_dir is None:
            raise AxerflowException("Could not find deleted experiment with ID %d" % experiment_id,
                                  databricks_pb2.RESOURCE_DOES_NOT_EXIST)
    ...
    def delete_run(self, run_id):
        ...
        if run_info is None:
            raise AxerflowException("Run '%s' metadata is in invalid state." % run_id,
    ...
    def _get_run_info(self, run_uuid):
        ...
        if run_dir is None:
            raise AxerflowException("Run '%s' not found" % run_uuid,
                                  databricks_pb2.RESOURCE_DOES_NOT_EXIST)
        run_info = self._get_run_info_from_dir(run_dir)
        if run_info.experiment_id != exp_id:
            raise AxerflowException("Run '%s' metadata is in invalid state." % run_uuid,
                                  databricks_pb2.INVALID_STATE)
        return run_info
    ...
    def _get_resource_files(self, root_dir, subfolder_name):
        ...
        if sys.platform == "win32":
            ...
            from axerflow.utils.file_utils import relative_path_to_artifact_path
    ...
    def _get_metric_from_line(metric_name, metric_line):
        ...
        if len(metric_parts) != 2 and len(metric_parts) != 3:
            raise AxerflowException("Metric '%s' is malformed; persisted metric data contained %s "
    ...
    def _get_metric_history(self, run_info, metric_key):
        ...
        if metric_key not in metric_files:
            ...
            raise AxerflowException("Metric '%s' not found under run '%s'" % (metric_key, run_id),
                                  databricks_pb2.RESOURCE_DOES_NOT_EXIST)
    ...
    def _validate_new_param_value(self, param_path, param_key, run_id, new_value):
        ...
        value.
        :raises: py:class:`axerflow.exceptions.AxerflowException` if the specified new parameter value
    ...
    def set_experiment_tag(self, experiment_id, tag):
        ...
        experiment = self.get_experiment(experiment_id)
        if experiment.lifecycle_stage != LifecycleStage.ACTIVE:
            raise AxerflowException("The experiment {} must be in the 'active'"
    ...
    def delete_tag(self, run_id, key):
        ...
        if not exists(tag_path):
            raise AxerflowException("No tag with name: {} in run with id {}".format(key, run_id),
    ...
    def record_logged_model(self, run_id, axerflow_model):
        ...
        run_info = self._get_run_info(run_id)
        path = self._get_tag_path(run_info.experiment_id, run_info.run_id, axerflow_LOGGED_MODELS)
        if os.path.exists(path):
            with open(path, "r") as f:
                model_list = json.loads(f.read())
        else:
            model_list = []
        tag = RunTag(axerflow_LOGGED_MODELS, json.dumps(model_list + [model_dict]))
