from axerflow.entities import Experiment, Run, RunInfo, Metric, ViewType
...
from axerflow.utils.proto_json_utils import message_to_json
from axerflow.utils.rest_utils import call_endpoint, extract_api_info_for_service

_PATH_PREFIX = "/api/2.0"
_METHOD_TO_INFO = extract_api_info_for_service(AxerflowService, _PATH_PREFIX)
...
class RestStore(AbstractStore):
    ...
    :param get_host_creds: Method to be invoked prior to every REST request to get the
      :py:class:`axerflow.rest_utils.AxerflowHostCreds` for the request. Note that this
    ...
    def record_logged_model(self, run_id, axerflow_model):
        req_body = message_to_json(
            LogModel(run_id=run_id, model_json=axerflow_model.to_json())
        )
