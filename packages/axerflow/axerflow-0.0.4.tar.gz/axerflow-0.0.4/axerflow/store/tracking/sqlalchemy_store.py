...
from axerflow.store.tracking import SEARCH_MAX_RESULTS_THRESHOLD
from axerflow.store.db.db_types import MYSQL, MSSQL, SQLITE
import axerflow.store.db.utils
from axerflow.store.tracking.dbmodels.models import SqlExperiment, SqlRun, \
    SqlMetric, SqlParam, SqlTag, SqlExperimentTag, SqlLatestMetric
from axerflow.store.db.base_sql_model import Base
from axerflow.entities import RunStatus, SourceType, Experiment
from axerflow.store.tracking.abstract_store import AbstractStore
from axerflow.entities import ViewType
from axerflow.exceptions import AxerflowException
from axerflow.protos.databricks_pb2 import INVALID_PARAMETER_VALUE, RESOURCE_ALREADY_EXISTS, \
    INVALID_STATE, RESOURCE_DOES_NOT_EXIST, INTERNAL_ERROR
from axerflow.utils.uri import is_local_uri, extract_db_type_from_uri
from axerflow.utils.file_utils import mkdir, local_file_uri_to_path
from axerflow.utils.search_utils import SearchUtils
from axerflow.utils.string_utils import is_string_type
from axerflow.utils.uri import append_to_uri_path
from axerflow.utils.validation import _validate_batch_log_limits, _validate_batch_log_data, \
    _validate_run_id, _validate_metric, _validate_experiment_tag, _validate_tag
from axerflow.utils.axerflow_tags import axerflow_LOGGED_MODELS
...
class SqlAlchemyStore(AbstractStore):
    """
    SQLAlchemy compliant backend store for tracking meta data for Axerflow entities. Axerflow
    ...
    This store interacts with SQL store using SQLAlchemy abstractions defined for Axerflow entities.
    :py:class:`axerflow.store.dbmodels.models.SqlExperiment`,
    :py:class:`axerflow.store.dbmodels.models.SqlRun`,
    :py:class:`axerflow.store.dbmodels.models.SqlTag`,
    :py:class:`axerflow.store.dbmodels.models.SqlMetric`, and
    :py:class:`axerflow.store.dbmodels.models.SqlParam`.

    Run artifacts are stored in a separate location using artifact stores conforming to
    :py:class:`axerflow.store.artifact_repo.ArtifactRepository`. Default artifact locations for
    user experiments are stored in the database along with metadata. Each run artifact location
    is recorded in :py:class:`axerflow.store.dbmodels.models.SqlRun` and stored in the backend DB.
    """
    ...
    def __init__(self, db_uri, default_artifact_root):
        ...
        :param db_uri: The SQLAlchemy database URI string to connect to the database. See
                       ...
                       for format specifications. Axerflow supports the dialects ``mysql``,
        ...
        self.engine = axerflow.store.db.utils.create_sqlalchemy_engine(db_uri)
        # On a completely fresh Axerflow installation against an empty database (verify database
        ...
        if any([table not in inspected_tables for table in expected_tables]):
            axerflow.store.db.utils._initialize_tables(self.engine)
        Base.metadata.bind = self.engine
        SessionMaker = sqlalchemy.orm.sessionmaker(bind=self.engine)
        self.ManagedSessionMaker = axerflow.store.db.utils._get_managed_session_maker(SessionMaker)
        axerflow.store.db.utils._verify_schema(self.engine)
    ...
    def _create_default_experiment(self, session):
        """
        Axerflow UI and client code expects a default experiment with ID 0.
    ...
    def create_experiment(self, name, artifact_location=None):
        if name is None or name == '':
            raise AxerflowException('Invalid experiment name', INVALID_PARAMETER_VALUE)
        ...
        with self.ManagedSessionMaker() as session:
            try:
                ...
                raise AxerflowException('Experiment(name={}) already exists. '
    ...
    def list_experiments(self, view_type=ViewType.ACTIVE_ONLY):
        with self.ManagedSessionMaker() as session:
            return [exp.to_axerflow_entity() for exp in
    ...
    def _get_experiment(self, session, experiment_id, view_type, eager=False):
        ...
        if experiment is None:
            raise AxerflowException('No Experiment with id={} exists'.format(experiment_id),
    ...
    def get_experiment(self, experiment_id):
        with self.ManagedSessionMaker() as session:
            return self._get_experiment(
                session, experiment_id, ViewType.ALL, eager=True).to_axerflow_entity()
        ...
        with self.ManagedSessionMaker() as session:
            ...
            experiment = session \
                ...
                .filter(
                    ...
                    SqlExperiment.lifecycle_stage.in_(stages)) \
                .one_or_none()
            return experiment.to_axerflow_entity() if experiment is not None else None
    ...
    def rename_experiment(self, experiment_id, new_name):
        with self.ManagedSessionMaker() as session:
            ...
            if experiment.lifecycle_stage != LifecycleStage.ACTIVE:
                raise AxerflowException('Cannot rename a non-active experiment.', INVALID_STATE)
    ...
    def _get_run(self, session, run_uuid, eager=False):
        ...
        if len(runs) == 0:
            raise AxerflowException('Run with id={} not found'.format(run_uuid),
                                  RESOURCE_DOES_NOT_EXIST)
        if len(runs) > 1:
            raise AxerflowException('Expected only 1 run with id={}. Found {}.'.format(run_uuid,
                                                                                     len(runs)),
                                  INVALID_STATE)
    ...
    def _check_run_is_active(self, run):
        if run.lifecycle_stage != LifecycleStage.ACTIVE:
            raise AxerflowException("The run {} must be in the 'active' state. Current state is {}."
                                  .format(run.run_uuid, run.lifecycle_stage),
                                  INVALID_PARAMETER_VALUE)

    def _check_experiment_is_active(self, experiment):
        if experiment.lifecycle_stage != LifecycleStage.ACTIVE:
            raise AxerflowException("The experiment {} must be in the 'active' state. "
                                  "Current state is {}."
    ...
    def _check_run_is_deleted(self, run):
        if run.lifecycle_stage != LifecycleStage.DELETED:
            raise AxerflowException("The run {} must be in the 'deleted' state. Current state is {}."
                                  .format(run.run_uuid, run.lifecycle_stage),
                                  INVALID_PARAMETER_VALUE)
    ...
    def get_run(self, run_id):
        with self.ManagedSessionMaker() as session:
            ...
            # ``run.to_axerflow_entity()``, so eager loading helps avoid additional database queries
    ...
    def _hard_delete_run(self, run_id):
        ...
        This is used by the ``axerflow gc`` command line and is not intended to be used elsewhere.
        """
        with self.ManagedSessionMaker() as session:
            run = self._get_run(run_uuid=run_id, session=session)
            session.delete(run)
    ...
    def get_metric_history(self, run_id, metric_key):
        with self.ManagedSessionMaker() as session:
            ...
            return [metric.to_axerflow_entity() for metric in metrics]
    ...
    def set_experiment_tag(self, experiment_id, tag):
        ...
        with self.ManagedSessionMaker() as session:
            experiment = self._get_experiment(session,
                                              experiment_id,
                                              ViewType.ALL).to_axerflow_entity()
            self._check_experiment_is_active(experiment)
        ...
        with self.ManagedSessionMaker() as session:
            ...
            if len(filtered_tags) == 0:
                ...
                raise AxerflowException(
                    ...
                    "See https://axerflow.org/docs/latest/tracking.html#adding-tags-to-runs",
    ...
    def _search_runs(self, experiment_ids, filter_string, run_view_type, max_results, order_by,
        ...
        if max_results > SEARCH_MAX_RESULTS_THRESHOLD:
            raise AxerflowException("Invalid value for request parameter max_results. It must be at "
        ...
        with self.ManagedSessionMaker() as session:
            ...
            runs = [run.to_axerflow_entity() for run in queried_runs]
    ...
    def log_batch(self, run_id, metrics, params, tags):
        ...
        except AxerflowException as e:
            raise e
        except Exception as e:
            raise AxerflowException(e, INTERNAL_ERROR)

    def record_logged_model(self, run_id, axerflow_model):
        if not isinstance(axerflow_model, Model):
            raise TypeError("Argument 'axerflow_model' should be axerflow.models.Model, got '{}'"
                            .format(type(axerflow_model)))
        model_dict = axerflow_model.to_dict()
        with self.ManagedSessionMaker() as session:
            run = self._get_run(run_uuid=run_id, session=session)
            self._check_run_is_active(run)
            previous_tag = [t for t in run.tags if t.key == axerflow_LOGGED_MODELS]
            if previous_tag:
                value = json.dumps(json.loads(previous_tag[0].value) + [model_dict])
            else:
                value = json.dumps([model_dict])
            _validate_tag(axerflow_LOGGED_MODELS, value)
            session.merge(SqlTag(key=axerflow_LOGGED_MODELS, value=value, run_uuid=run_id))
...
def _to_sqlalchemy_filtering_statement(sql_statement, session):
    ...
    if SearchUtils.is_metric(key_type, comparator):
        ...
        raise AxerflowException("Invalid search expression type '%s'" % key_type,
        ...
        for order_by_clause in order_by_list:
            ...
            if SearchUtils.is_attribute(key_type, '='):
                ...
                if SearchUtils.is_metric(key_type, '='):  # any valid comparator
                    ...
                    raise AxerflowException("Invalid identifier type '%s'" % key_type,
                                          error_code=INVALID_PARAMETER_VALUE)
