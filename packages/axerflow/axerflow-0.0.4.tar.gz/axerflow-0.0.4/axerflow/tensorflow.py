"""
The ``axerflow.tensorflow`` module provides an API for logging and loading TensorFlow models.
...
from axerflow.protos.databricks_pb2 import DIRECTORY_NOT_EMPTY
from axerflow.tracking.artifact_utils import _download_artifact_from_uri
from axerflow.utils import keyword_only, experimental
from axerflow.utils.environment import _axerflow_conda_env
from axerflow.utils.file_utils import _copy_file_or_tree
from axerflow.utils.model_utils import _get_flavor_configuration
from axerflow.utils.autologging_utils import try_axerflow_log, log_fn_args_as_params
from axerflow.entities import Metric
...
def log_model(tf_saved_model_dir, tf_meta_graph_tags, tf_signature_def_key, artifact_path,
    ...
    """
    Log a *serialized* collection of TensorFlow graphs and variables as an Axerflow model
    ...
    :param input_example: (Experimental) Input example provides one or several instances of valid
                          ...
                          base64-encoded.
    """
    return Model.log(artifact_path=artifact_path, flavor=axerflow.tensorflow,
...
def save_model(tf_saved_model_dir, tf_meta_graph_tags, tf_signature_def_key, path,
               axerflow_model=None, conda_env=None,
               signature: ModelSignature = None, input_example: ModelInputExample = None):
    """
    Save a *serialized* collection of TensorFlow graphs and variables as an Axerflow model
    ...
    :param tf_signature_def_key: A string identifying the input/output signature associated with the
                                 ...
                                 ``signature_def_map`` parameter of the
                                 ``tf.saved_model.builder.savedmodelbuilder`` method.
    :param path: Local path where the Axerflow model is to be saved.
    :param axerflow_model: Axerflow model configuration to which to add the ``tensorflow`` flavor.
    :param conda_env: Either a dictionary representation of a Conda environment or the path to a
                      Conda environment yaml file. If provided, this decsribes the environment
                      this model should be run in. At minimum, it should specify the dependencies
                      contained in :func:`get_default_conda_env()`. If ``None``, the default
    ...
    if os.path.exists(path):
        raise AxerflowException("Path '{}' already exists".format(path), DIRECTORY_NOT_EMPTY)
    os.makedirs(path)
    ...
    axerflow_model.add_flavor(FLAVOR_NAME, saved_model_dir=model_dir_subpath,
                            meta_graph_tags=tf_meta_graph_tags,
                            signature_def_key=tf_signature_def_key)
    pyfunc.add_to_model(axerflow_model, loader_module="axerflow.tensorflow", env=conda_env_subpath)
...
def load_model(model_uri, tf_sess=None):
    """
    Load an Axerflow model that contains the TensorFlow flavor from the specified path.
    ...
    *With TensorFlow version <2.0.0, this method must be called within a TensorFlow graph context.*
    ...
    :param tf_sess: The TensorFlow session in which to load the model. If using TensorFlow
                    ...
                    session is passed to this function, Axerflow will attempt to load the model using
    ...
    .. code-block:: python
        ...
        import axerflow.tensorflow
        import tensorflow as tf
        tf_graph = tf.Graph()
        tf_sess = tf.Session(graph=tf_graph)
        with tf_graph.as_default():
            signature_definition = axerflow.tensorflow.load_model(model_uri="model_uri",
                                    tf_sess=tf_sess)
    ...
    if LooseVersion(tensorflow.__version__) < LooseVersion('2.0.0'):
        if not tf_sess:
            ...
            if not tf_sess:
                raise AxerflowException("No TensorFlow session found while calling load_model()." +
...
def _load_tensorflow_saved_model(tf_saved_model_dir, tf_meta_graph_tags, tf_signature_def_key,
    ...
    if tf_signature_def_key not in loaded_sig:
        raise AxerflowException("Could not find signature def key %s. Available keys are: %s"
...
def _get_and_parse_flavor_configuration(model_path):
    """
    :param path: Local filesystem path to the Axerflow Model with the ``tensorflow`` flavor.
...
def _load_pyfunc(path):
    """
    Load PyFunc implementation. Called by ``pyfunc.load_pyfunc``. This function loads an Axerflow
...
class __AxerflowTfKerasCallback(Callback):
    """
    ...
    def on_train_begin(self, logs=None):  # pylint: disable=unused-argument
        ...
        if hasattr(opt, '_name'):
            try_axerflow_log(axerflow.log_param, 'optimizer_name', opt._name)
        # Elif checks are if the optimizer is a TensorFlow optimizer rather than a Keras one.
            ...
            opt = opt.optimizer
            try_axerflow_log(axerflow.log_param, 'optimizer_name', type(opt).__name__)
        if hasattr(opt, 'lr'):
            lr = opt.lr if type(opt.lr) is float else tensorflow.keras.backend.eval(opt.lr)
            try_axerflow_log(axerflow.log_param, 'learning_rate', lr)
        ...
        if hasattr(opt, 'epsilon'):
            ...
            try_axerflow_log(axerflow.log_param, 'epsilon', epsilon)
        ...
        try:
            ...
            with open(summary_file, 'w') as f:
                f.write(summary)
            try_axerflow_log(axerflow.log_artifact, local_path=summary_file)
        finally:
    ...
    def on_train_end(self, logs=None):  # pylint: disable=unused-argument
        try_axerflow_log(axerflow.keras.log_model, self.model, artifact_path='model')


class __AxerflowTfKeras2Callback(Callback):
    """
    ...
    def on_train_begin(self, logs=None):  # pylint: disable=unused-argument
        ...
        for attribute in config:
            try_axerflow_log(axerflow.log_param, "opt_" + attribute, config[attribute])
    ...
    def on_epoch_end(self, epoch, logs=None):
        if (epoch-1) % _LOG_EVERY_N_STEPS == 0:
            try_axerflow_log(axerflow.log_metrics, logs, step=epoch)
...
def _log_artifacts_with_warning(**kwargs):
    try_axerflow_log(axerflow.log_artifacts, **kwargs)
...
def _flush_queue():
    """
    Flush the metric queue and log contents in batches to Axerflow.
    Queue is divided into batches according to run id.
    """
    global _metric_queue
    client = axerflow.tracking.AxerflowClient()
    dic = _assoc_list_to_map(_metric_queue)
    for key in dic:
        try_axerflow_log(client.log_batch, key, metrics=dic[key], params=[], tags=[])
    _metric_queue = []
...
def _log_event(event):
    ...
    if not axerflow.active_run():
        try_axerflow_log(axerflow.start_run)
        global _AUTOLOG_RUN_ID
        _AUTOLOG_RUN_ID = axerflow.active_run().info.run_id
            ...
            if v.HasField('simple_value'):
                if (event.step-1) % _LOG_EVERY_N_STEPS == 0:
                    _thread_pool.submit(_add_to_queue, key=v.tag,
                                        ...
                                        run_id=axerflow.active_run().info.run_id)
...
# :is_temp - boolean: `True` if the logging directory was created for temporary use by Axerflow,
#                     `False` otherwise
...
def _setup_callbacks(lst):
    """
    Adds TensorBoard and axerflowTfKeras callbacks to the
    ...
    if tb is None:
        ...
        out_list = lst
    if LooseVersion(tensorflow.__version__) < LooseVersion('2.0.0'):
        out_list += [__AxerflowTfKerasCallback()]
    else:
        out_list += [__AxerflowTfKeras2Callback()]
...
def autolog(every_n_iter=100):
    ...
    """
    Enable automatic logging from TensorFlow to Axerflow. If applicable,
    ...
    if LooseVersion(tensorflow.__version__) < LooseVersion('1.12'):
        warnings.warn("Could not log to Axerflow. Only TensorFlow versions" +
    ...
    def _manage_active_run():
        if not axerflow.active_run():
            try_axerflow_log(axerflow.start_run)
            global _AUTOLOG_RUN_ID
            if axerflow.active_run() is not None:  # defensive check in case `axerflow.start_run` fails
                _AUTOLOG_RUN_ID = axerflow.active_run().info.run_id
        yield axerflow.active_run()
        if axerflow.active_run() is not None and axerflow.active_run().info.run_id == _AUTOLOG_RUN_ID:
            try_axerflow_log(axerflow.end_run)
    ...
    def train(self, *args, **kwargs):
        with _manage_active_run():
            ...
            if len(args) >= 3:
                try_axerflow_log(axerflow.log_param, 'steps', args[2])
                if len(args) >= 4:
                    try_axerflow_log(axerflow.log_param, 'max_steps', args[3])
            if 'steps' in kwargs:
                try_axerflow_log(axerflow.log_param, 'steps', kwargs['steps'])
            if 'max_steps' in kwargs:
                try_axerflow_log(axerflow.log_param, 'max_steps', kwargs['max_steps'])
    ...
    def export_saved_model(self, *args, **kwargs):
        ...
        if not axerflow.active_run():
            global _AUTOLOG_RUN_ID
            if _AUTOLOG_RUN_ID:
                try_axerflow_log(axerflow.start_run, _AUTOLOG_RUN_ID)
            else:
                try_axerflow_log(axerflow.start_run)
                auto_end = True
        ...
        try_axerflow_log(log_model, tf_saved_model_dir=serialized.decode('utf-8'),
    ...
    def _log_early_stop_callback_params(callback):
        if callback:
            try:
                ...
                try_axerflow_log(axerflow.log_params, earlystopping_params)
    ...
    def _log_early_stop_callback_metrics(callback, history):
        if callback:
            ...
            try_axerflow_log(axerflow.log_metric, 'stopped_epoch', stopped_epoch)
            ...
            if stopped_epoch != 0 and restore_best_weights:
                ...
                try_axerflow_log(axerflow.log_metric, 'restored_epoch', restored_epoch)
                ...
                if metric_key is not None:
                    ...
                    try_axerflow_log(axerflow.log_metrics, restored_metrics, step=last_epoch)
