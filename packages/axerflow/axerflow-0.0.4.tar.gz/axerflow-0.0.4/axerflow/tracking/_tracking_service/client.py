"""
Internal package providing a Python CRUD interface to Axerflow experiments and runs.
This is a lower level API than the :py:mod:`axerflow.tracking.fluent` module, and is
exposed in the :py:mod:`axerflow.tracking` module.
"""
...
from axerflow.utils.validation import _validate_param_name, _validate_tag_name, _validate_run_id, \
    _validate_experiment_artifact_location, _validate_experiment_name, _validate_metric
from axerflow.entities import Param, Metric, RunStatus, RunTag, ViewType, ExperimentTag
from axerflow.store.artifact.artifact_repository_registry import get_artifact_repository
from axerflow.utils.axerflow_tags import axerflow_USER
from axerflow.utils.string_utils import is_string_type


class TrackingServiceClient(object):
    """
    Client of an Axerflow Tracking Server that creates and manages experiments and runs.
    """
    ...
    def create_run(self, experiment_id, start_time=None, tags=None):
        ...
        # in a later release.
        user_id = tags.get(axerflow_USER, "unknown")
    ...
    def _record_logged_model(self, run_id, axerflow_model):
        if not isinstance(axerflow_model, Model):
            raise TypeError("Argument 'axerflow_model' should be of type axerflow.models.Model but was "
                            "{}".format(type(axerflow_model)))
        self.store.record_logged_model(run_id, axerflow_model)
