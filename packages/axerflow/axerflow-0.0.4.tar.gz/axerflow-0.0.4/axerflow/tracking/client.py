"""
Internal package providing a Python CRUD interface to Axerflow experiments, runs, registered models,
and model versions. This is a lower level API than the :py:mod:`axerflow.tracking.fluent` module,
and is exposed in the :py:mod:`axerflow.tracking` module.
"""
import logging

from axerflow.entities import ViewType
from axerflow.entities.model_registry.model_version_stages import ALL_STAGES
from axerflow.exceptions import AxerflowException
from axerflow.protos.databricks_pb2 import FEATURE_DISABLED
from axerflow.store.tracking import SEARCH_MAX_RESULTS_DEFAULT
from axerflow.tracking._model_registry.client import ModelRegistryClient
from axerflow.tracking.registry import UnsupportedModelRegistryStoreURIException
from axerflow.tracking._tracking_service import utils
from axerflow.tracking._tracking_service.client import TrackingServiceClient
from axerflow.utils import experimental, deprecated

_logger = logging.getLogger(__name__)


class AxerflowClient(object):
    """
    Client of an Axerflow Tracking Server that creates and manages experiments and runs, and of an
    Axerflow Registry Server that creates and manages registered models and model versions. It's a
    ...
    def __init__(self, tracking_uri=None, registry_uri=None):
        ...
        :param tracking_uri: Address of local or remote tracking server. If not provided, defaults
                             to the service set by ``axerflow.tracking.set_tracking_uri``. See
                             `Where Runs Get Recorded <../tracking.html#where-runs-get-recorded>`_
                             for more info.
        :param registry_uri: Address of local or remote model registry server. If not provided,
                             defaults to the service set by ``axerflow.tracking.set_tracking_uri``.
        """
        ...
        # `AxerflowClient` also references a `ModelRegistryClient` instance that is provided by the
        # `AxerflowClient._get_registry_client()` method. This `ModelRegistryClient` is not explicitly
        # defined as an instance variable in the `AxerflowClient` constructor; an instance variable
        # is assigned lazily by `AxerflowClient._get_registry_client()` and should not be referenced
        # outside of the `AxerflowClient._get_registry_client()` method

    def _get_registry_client(self):
        """
        Attempts to create a py:class:`ModelRegistryClient` if one does not already exist.

        :raises: py:class:`axerflow.exceptions.AxerflowException` if the py:class:`ModelRegistryClient`
        ...
        # an instance variable on this `AxerflowClient` instance. Because the instance variable
        ...
        if registry_client is None:
            try:
                ...
                # Define an instance variable on this `AxerflowClient` instance to reference the
    ...
    def get_run(self, run_id):
        """
        Fetch the run from backend store. The resulting :py:class:`Run <axerflow.entities.Run>`
        contains a collection of run metadata -- :py:class:`RunInfo <axerflow.entities.RunInfo>`,
        as well as a collection of run parameters, tags, and metrics --
        :py:class:`RunData <axerflow.entities.RunData>`. In the case where multiple metrics with the
        same key are logged for the run, the :py:class:`RunData <axerflow.entities.RunData>` contains
        the most recently logged value at the largest step for each metric.

        :param run_id: Unique identifier for the run.

        :return: A single :py:class:`axerflow.entities.Run` object, if the run exists. Otherwise,
                 raises an exception.
        """
    ...
    def get_metric_history(self, run_id, key):
        ...
        :return: A list of :py:class:`axerflow.entities.Metric` entities if logged, else empty list
        """
        return self._tracking_client.get_metric_history(run_id, key)

    def create_run(self, experiment_id, start_time=None, tags=None):
        """
        Create a :py:class:`axerflow.entities.Run` object that can be associated with
        metrics, parameters, artifacts, etc.
        Unlike :py:func:`axerflow.projects.run`, creates objects but does not run code.
        Unlike :py:func:`axerflow.start_run`, does not change the "active run" used by
        :py:func:`axerflow.log_param`.

        :param experiment_id: The ID of then experiment to create a run in.
        :param start_time: If not provided, use the current timestamp.
        :param tags: A dictionary of key-value pairs that are converted into
                     :py:class:`axerflow.entities.RunTag` objects.
        :return: :py:class:`axerflow.entities.Run` that was created.
        """
    ...
    def get_experiment(self, experiment_id):
        ...
        :return: :py:class:`axerflow.entities.Experiment`
        """
    ...
    def log_batch(self, run_id, metrics=(), params=(), tags=()):
        ...
        Raises an AxerflowException if any errors occur.
        :return: None
        """
    ...
    def _record_logged_model(self, run_id, axerflow_model):
        """
        Record logged model info with the tracking server.

        :param run_id: run_id under which the model has been logged.
        :param axerflow_model: Model info to be recorded.
        """
        self._tracking_client._record_logged_model(run_id, axerflow_model)
    ...
    def list_artifacts(self, run_id, path=None):
        ...
        :return: List of :py:class:`axerflow.entities.FileInfo`
        """
    ...
    def set_terminated(self, run_id, status=None, end_time=None):
        ...
        :param status: A string value of :py:class:`axerflow.entities.RunStatus`.
    ...
    def search_runs(self, experiment_ids, filter_string="", run_view_type=ViewType.ACTIVE_ONLY,
        ...
        :param run_view_type: one of enum values ACTIVE_ONLY, DELETED_ONLY, or ALL runs
                              defined in :py:class:`axerflow.entities.ViewType`.
        ...
        :return: A list of :py:class:`axerflow.entities.Run` objects that satisfy the search
    ...
    def create_registered_model(self, name):
        ...
        :return: A single object of :py:class:`axerflow.entities.model_registry.RegisteredModel`
                 created by backend.
        """
    ...
    def rename_registered_model(self, name, new_name):
        ...
        :return: A single updated :py:class:`axerflow.entities.model_registry.RegisteredModel` object.
        """
    ...
    def update_registered_model(self, name, new_name=None, description=None):
        ...
        :param new_name: (Deprecated) New proposed name for the registered model.
                         ...
                         :py:func:`rename_registered_model <AxerflowClient.rename_registered_model>`
                         method to rename registered models instead.
        :param description: (Optional) New description.
        :return: A single updated :py:class:`axerflow.entities.model_registry.RegisteredModel` object.
        """
        if new_name is None and description is None:
            raise AxerflowException("Attempting to update registered model with no new field values.")

        if new_name is not None and new_name.strip() == "":
            raise AxerflowException("The new name must not be an empty string.")
    ...
    def list_registered_models(self):
        ...
        :return: List of :py:class:`axerflow.entities.model_registry.RegisteredModel` objects.
        """
        return self._get_registry_client().list_registered_models()

    @deprecated(alternative="axerflow.tracking.client.get_registered_model", since="1.7")
    ...
    def get_registered_model(self, name):
        """
        :param name: Name of the registered model to update.
        :return: A single :py:class:`axerflow.entities.model_registry.RegisteredModel` object.
        """
    ...
    def get_latest_versions(self, name, stages=None):
        ...
        :param stages: List of desired stages. If input list is None, return latest versions for
                       for ALL_STAGES.
        :return: List of :py:class:`axerflow.entities.model_registry.ModelVersion` objects.
        """
    ...
    def create_model_version(self, name, source, run_id):
        ...
        :param source: Source path where the Axerflow model is stored.
        :param run_id: Run ID from Axerflow tracking server that generated the model
        :return: Single :py:class:`axerflow.entities.model_registry.ModelVersion` object created by
                 backend.
        """
    ...
    def update_model_version(self, name, version, stage=None, description=None):
        ...
        :param stage: (Deprecated) New desired stage forthis model version. This field is deprecated
                      as of axerflow 1.7. Use transition_model_version_stage instead to update stage.
        :param description: New description.

        :return: A single :py:class:`axerflow.entities.model_registry.ModelVersion` object.
        """
        if stage is None and description is None:
            raise AxerflowException("Attempting to update model version with no new field values.")
        if stage is not None and stage.strip() == "":
            raise AxerflowException("The stage must not be an empty string.")
    ...
    @deprecated("axerflow.tracking.client.get_model_version", "1.7")
    ...
    def search_model_versions(self, filter_string):
        ...
        :param filter_string: A filter string expression. Currently supports a single filter
                              ...
                              ``run_id = '...'``.
        :return: PagedList of :py:class:`axerflow.entities.model_registry.ModelVersion` objects.
        """
