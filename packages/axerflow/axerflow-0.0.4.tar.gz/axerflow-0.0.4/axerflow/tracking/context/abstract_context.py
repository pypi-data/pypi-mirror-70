...
class RunContextProvider(object):
    ...
    When a run is created via the fluent ``axerflow.start_run`` method, Axerflow iterates through all
    registered RunContextProviders. For each context provider where ``in_context`` returns ``True``,
    Axerflow calls the ``tags`` method on the context provider to compute context tags for the run.
    ...
    def in_context(self):
        """
        Determine if Axerflow is running in this context.
