from axerflow.tracking.context.abstract_context import RunContextProvider
from axerflow.utils import databricks_utils
from axerflow.entities import SourceType
from axerflow.utils.axerflow_tags import (
    axerflow_SOURCE_TYPE,
    axerflow_SOURCE_NAME,
    axerflow_DATABRICKS_WEBAPP_URL,
    axerflow_DATABRICKS_NOTEBOOK_PATH,
    axerflow_DATABRICKS_NOTEBOOK_ID
)


class DatabricksNotebookRunContext(RunContextProvider):
    def in_context(self):
        return databricks_utils.is_in_databricks_notebook()

    def tags(self):
        notebook_id = databricks_utils.get_notebook_id()
        notebook_path = databricks_utils.get_notebook_path()
        webapp_url = databricks_utils.get_webapp_url()
        tags = {
            axerflow_SOURCE_NAME: notebook_path,
            axerflow_SOURCE_TYPE: SourceType.to_string(SourceType.NOTEBOOK)
        }
        if notebook_id is not None:
            tags[axerflow_DATABRICKS_NOTEBOOK_ID] = notebook_id
        if notebook_path is not None:
            tags[axerflow_DATABRICKS_NOTEBOOK_PATH] = notebook_path
        if webapp_url is not None:
            tags[axerflow_DATABRICKS_WEBAPP_URL] = webapp_url
        return tags
