...
from axerflow.tracking.context.default_context import DefaultRunContext
from axerflow.tracking.context.git_context import GitRunContext
from axerflow.tracking.context.databricks_notebook_context import DatabricksNotebookRunContext
from axerflow.tracking.context.databricks_job_context import DatabricksJobRunContext
...
class RunContextProviderRegistry(object):
    ...
    information about the context of an Axerflow experiment run. Implementations declared though the
    entrypoints `axerflow.run_context_provider` group can be automatically registered through the
    ...
    def register_entrypoints(self):
        ...
        for entrypoint in entrypoints.get_group_all("axerflow.run_context_provider"):
            try:
...
def resolve_tags(tags=None):
    ...
    :py:class:`axerflow.tracking.context.RunContextProvider`.
