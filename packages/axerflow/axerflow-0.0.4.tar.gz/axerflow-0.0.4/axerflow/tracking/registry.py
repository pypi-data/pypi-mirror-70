...
class UnsupportedModelRegistryStoreURIException(AxerflowException):
    ...
    def __init__(self, unsupported_uri, supported_uri_schemes):
        message = (" Model registry functionality is unavailable; got unsupported URI '{}' for"
                   ...
                   " See https://www.axerflow.org/docs/latest/tracking.html#storage for"
                   " how to run an Axerflow server against one of the supported backend storage"
                   " locations.").format(
...
class StoreRegistry:
    ...
    def get_store_builder(self, store_uri):
        ...
        :return: A function that returns an instance of
                 ``axerflow.store.{tracking|model_registry}.AbstractStore`` that fulfills the store
                  URI requirements.
        """
