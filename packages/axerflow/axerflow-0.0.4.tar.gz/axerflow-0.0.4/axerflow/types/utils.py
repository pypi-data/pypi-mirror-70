...
from axerflow.types import DataType
from axerflow.types.schema import Schema, ColSpec


class TensorsNotSupportedException(AxerflowException):
...
def _infer_schema(data: Any) -> Schema:
    """
    Infer an Axerflow schema from a dataset.
    ...
    The element types should be mappable to one of :py:class:`axerflow.models.signature.DataType`.
...
def _infer_numpy_dtype(dtype: np.dtype) -> DataType:
    ...
    raise AxerflowException("Unsupported numpy data type '{0}', kind '{1}'".format(
        dtype, dtype.kind))
...
def _infer_numpy_array(col: np.ndarray) -> DataType:
    ...
    if len(col.shape) > 1:
        raise AxerflowException("Expected 1d array, got array with shape {}".format(col.shape))
    ...
    if col.dtype.kind == "O":
        ...
        if all(map(is_double_test, col)) and is_double_test.seen_instances > 0:
            ...
            raise AxerflowException("Unable to map 'np.object' type to Axerflow DataType. np.object can"
...
def _infer_spark_type(x) -> DataType:
    ...
    if isinstance(x, pyspark.sql.types.NumericType):
        ...
        raise Exception("Unsupported Spark Type '{}', Axerflow schema is only supported for scalar "
