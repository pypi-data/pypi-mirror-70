...
from axerflow.utils.rest_utils import AxerflowHostCreds
...
def _fail_malformed_databricks_auth(profile):
    raise AxerflowException("Got malformed Databricks CLI profile '%s'. Please make sure the "
...
def get_databricks_host_creds(profile=None):
    ...
    :return: :py:class:`axerflow.rest_utils.AxerflowHostCreds` which includes the hostname and
    ...
    if config.username is not None and config.password is not None:
        return AxerflowHostCreds(config.host, username=config.username, password=config.password,
                               ignore_tls_verification=insecure)
    elif config.token:
        return AxerflowHostCreds(config.host, token=config.token, ignore_tls_verification=insecure)
    _fail_malformed_databricks_auth(profile)
