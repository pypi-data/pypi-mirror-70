...
from axerflow.utils import PYTHON_VERSION
...
def _axerflow_conda_env(path=None, additional_conda_deps=None, additional_pip_deps=None,
                      additional_conda_channels=None, install_axerflow=True):
    """
    Creates a Conda environment with the specified package channels and dependencies. If there are
    any pip dependencies, including from the install_axerflow parameter, then pip will be added to
    ...
    :return: ``None`` if ``path`` is specified. Otherwise, the a dictionary representation of the
             Conda environment.
    """
    pip_deps = (["axerflow"] if install_axerflow else []) + (
