...
from axerflow.exceptions import MissingConfigException
...
def _copy_project(src_path, dst_path=""):
    """
    Internal function used to copy Axerflow project during development.

    Copies the content of the whole directory tree except patterns defined in .dockerignore.
    The Axerflow is assumed to be accessible as a local directory in this case.


    :param dst_path: Axerflow will be copied here
    :return: name of the Axerflow project directory
    """

    def _docker_ignore(axerflow_root):
        docker_ignore = os.path.join(axerflow_root, '.dockerignore')
        patterns = []
    ...
    shutil.copytree(src_path, os.path.join(dst_path, axerflow_dir),
