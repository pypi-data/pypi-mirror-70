...
def _stringify_all_experiment_ids(x):
    ...
    This is necessary for backwards- and forwards-compatibility with Axerflow clients/servers
    running Axerflow 0.9.0 and below, as experiment_id was changed from an int to a string.
    ...
    This code can be removed after Axerflow 1.0, after users have given reasonable time to
    upgrade clients and servers to Axerflow 0.9.1+.
    """
...
def _dataframe_from_json(path_or_str, schema: Schema = None,
    ...
    :param schema: Axerflow schema used when parsing the data.
