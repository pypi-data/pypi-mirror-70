...
from axerflow import __version__
from axerflow.protos import databricks_pb2
from axerflow.utils.proto_json_utils import parse_dict
from axerflow.utils.string_utils import strip_suffix
from axerflow.exceptions import AxerflowException, RestException
...
_DEFAULT_HEADERS = {
    'User-Agent': 'axerflow-python-client/%s' % __version__
}
...
def http_request(host_creds, endpoint, retries=3, retry_interval=3,
    ...
    :param host_creds: A :py:class:`axerflow.rest_utils.AxerflowHostCreds` object containing
    ...
    raise AxerflowException("API request to %s failed to return code 200 after %s tries" %
                          (url, retries))
...
def verify_rest_response(response, endpoint):
    ...
    if response.status_code != 200:
        if _can_parse_as_json(response.text):
            ...
            raise AxerflowException("%s. Response body: '%s'" % (base_msg, response.text))
    return response
...
class AxerflowHostCreds(object):
    """
    Provides a hostname and optional authentication for talking to an Axerflow tracking server.
    :param host: Hostname (e.g., http://localhost:5000) to Axerflow server. Required.
    ...
    def __init__(self, host, username=None, password=None, token=None,
        ...
        if not host:
            raise AxerflowException("host is a required parameter for AxerflowHostCreds")
        self.host = host
