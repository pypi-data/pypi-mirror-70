...
from axerflow.entities import RunInfo
...
class SearchUtils(object):
    ...
    def _strip_quotes(cls, value, expect_quoted_value=False):
        ...
        if cls._is_quoted(value, "'") or cls._is_quoted(value, '"'):
            ...
            raise AxerflowException("Parameter value is either not quoted or unidentified quote "
    ...
    def _valid_entity_type(cls, entity_type):
        ...
        if entity_type not in cls._VALID_IDENTIFIERS:
            raise AxerflowException("Invalid entity type '%s'. "
    ...
    def _get_identifier(cls, identifier, valid_attributes):
        try:
            ...
            raise AxerflowException("Invalid identifier '%s'. Columns should be specified as "
        ...
        if identifier == cls._ATTRIBUTE_IDENTIFIER and key not in valid_attributes:
            raise AxerflowException("Invalid attribute key '{}' specified. Valid keys "
    ...
    def _get_value(cls, identifier_type, token):
        if identifier_type == cls._METRIC_IDENTIFIER:
            if token.ttype not in cls.NUMERIC_VALUE_TYPES:
                raise AxerflowException("Expected numeric value type for metric. "
            ...
            raise AxerflowException("Expected a quoted string value for "
            ...
            if token.ttype in cls.STRING_VALUE_TYPES or isinstance(token, Identifier):
                ...
                raise AxerflowException("Expected a quoted string value for attributes. "
                                      "Got value {value}".format(value=token.value),
                                      error_code=INVALID_PARAMETER_VALUE)
        else:
            # Expected to be either "param" or "metric".
            raise AxerflowException("Invalid identifier type. Expected one of "
    ...
    def _validate_comparison(cls, tokens):
        ...
        if len(tokens) != 3:
            raise AxerflowException("{}. Expected 3 tokens found {}".format(base_error_string,
                                                                          len(tokens)),
                                  error_code=INVALID_PARAMETER_VALUE)
        if not isinstance(tokens[0], Identifier):
            raise AxerflowException("{}. Expected 'Identifier' found '{}'".format(base_error_string,
                                                                                str(tokens[0])),
                                  error_code=INVALID_PARAMETER_VALUE)
        if not isinstance(tokens[1], Token) and tokens[1].ttype != TokenType.Operator.Comparison:
            raise AxerflowException("{}. Expected comparison found '{}'".format(base_error_string,
                                                                              str(tokens[1])),
                                  error_code=INVALID_PARAMETER_VALUE)
        if not isinstance(tokens[2], Token) and \
            ...
            raise AxerflowException("{}. Expected value token found '{}'".format(base_error_string,
                                                                               str(tokens[2])),
                                  error_code=INVALID_PARAMETER_VALUE)
    ...
    def _process_statement(cls, statement):
        ...
        if len(invalids) > 0:
            ...
            raise AxerflowException("Invalid clause(s) in filter string: %s" % invalid_clauses,
                                  error_code=INVALID_PARAMETER_VALUE)
    ...
    def parse_search_filter(cls, filter_string):
        ...
        try:
            ...
            raise AxerflowException("Error on parsing filter '%s'" % filter_string,
                                  error_code=INVALID_PARAMETER_VALUE)
        if len(parsed) == 0 or not isinstance(parsed[0], Statement):
            raise AxerflowException("Invalid filter '%s'. Could not be parsed." %
                                  filter_string, error_code=INVALID_PARAMETER_VALUE)
        elif len(parsed) > 1:
            raise AxerflowException("Search filter contained multiple expression '%s'. "
    ...
    def is_metric(cls, key_type, comparator):
        if key_type == cls._METRIC_IDENTIFIER:
            if comparator not in cls.VALID_METRIC_COMPARATORS:
                raise AxerflowException("Invalid comparator '%s' "
    ...
    def is_attribute(cls, key_type, comparator):
        if key_type == cls._ATTRIBUTE_IDENTIFIER:
            if comparator not in cls.VALID_STRING_ATTRIBUTE_COMPARATORS:
                raise AxerflowException("Invalid comparator '{}' not one of "
                                      "'{}".format(comparator,
    ...
    def parse_order_by(cls, order_by):
        try:
            ...
            raise AxerflowException("Error on parsing order_by clause '%s'" % order_by,
                                  error_code=INVALID_PARAMETER_VALUE)
        if len(parsed) != 1 or not isinstance(parsed[0], Statement):
            raise AxerflowException("Invalid order_by clause '%s'. Could not be parsed." %
    ...
    def _get_value_for_sort(cls, run, key_type, key, ascending):
        ...
        if key_type == cls._METRIC_IDENTIFIER:
            ...
            raise AxerflowException("Invalid order_by entity type '%s'" % key_type,
                                  error_code=INVALID_PARAMETER_VALUE)
    ...
    def parse_start_offset_from_page_token(cls, page_token):
        ...
        try:
            ...
            raise AxerflowException("Invalid page token, could not base64-decode",
                                  error_code=INVALID_PARAMETER_VALUE)
        ...
        try:
            ...
            raise AxerflowException("Invalid page token, decoded value=%s" % decoded_token,
                                  error_code=INVALID_PARAMETER_VALUE)

        offset_str = parsed_token.get("offset")
        if not offset_str:
            raise AxerflowException("Invalid page token, parsed value=%s" % parsed_token,
                                  error_code=INVALID_PARAMETER_VALUE)

        try:
            offset = int(offset_str)
        except ValueError:
            raise AxerflowException("Invalid page token, not stringable %s" % offset_str,
                                  error_code=INVALID_PARAMETER_VALUE)
    ...
    def parse_filter_for_model_registry(cls, filter_string):
        ...
        try:
            ...
            raise AxerflowException("Error while parsing filter '%s'. %s" % (filter_string, expected),
                                  error_code=INVALID_PARAMETER_VALUE)
        if len(parsed) == 0 or not isinstance(parsed[0], Statement):
            raise AxerflowException("Invalid filter '%s'. Could not be parsed. %s" %
                                  (filter_string, expected), error_code=INVALID_PARAMETER_VALUE)
        elif len(parsed) > 1:
            raise AxerflowException("Search filter '%s' contains multiple expressions. "
        ...
        if len(invalids) > 0:
            invalid_clauses = ", ".join("'%s'" % token for token in invalids)
            raise AxerflowException("Invalid clause(s) in filter string: %s. "
