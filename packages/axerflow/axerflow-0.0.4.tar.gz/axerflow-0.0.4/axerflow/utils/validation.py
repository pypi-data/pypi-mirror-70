...
def _validate_metric_name(name):
    ...
    if not _VALID_PARAM_AND_METRIC_NAMES.match(name):
        raise AxerflowException("Invalid metric name: '%s'. %s" % (name, _BAD_CHARACTERS_MESSAGE),
                              INVALID_PARAMETER_VALUE)
    if path_not_unique(name):
        raise AxerflowException("Invalid metric name: '%s'. %s" % (name, bad_path_message(name)),
                              INVALID_PARAMETER_VALUE)
...
def _validate_param_name(name):
    ...
    if not _VALID_PARAM_AND_METRIC_NAMES.match(name):
        raise AxerflowException("Invalid parameter name: '%s'. %s" % (name, _BAD_CHARACTERS_MESSAGE),
                              INVALID_PARAMETER_VALUE)
    if path_not_unique(name):
        raise AxerflowException("Invalid parameter name: '%s'. %s" % (name, bad_path_message(name)),
                              INVALID_PARAMETER_VALUE)
...
def _validate_tag_name(name):
    ...
    if not _VALID_PARAM_AND_METRIC_NAMES.match(name):
        raise AxerflowException("Invalid tag name: '%s'. %s" % (name, _BAD_CHARACTERS_MESSAGE),
                              INVALID_PARAMETER_VALUE)
    if path_not_unique(name):
        raise AxerflowException("Invalid tag name: '%s'. %s" % (name, bad_path_message(name)),
                              INVALID_PARAMETER_VALUE)
...
def _validate_run_id(run_id):
    ...
    if _RUN_ID_REGEX.match(run_id) is None:
        raise AxerflowException("Invalid run ID: '%s'" % run_id, error_code=INVALID_PARAMETER_VALUE)
...
def _validate_experiment_id(exp_id):
    ...
    if exp_id is not None and _EXPERIMENT_ID_REGEX.match(exp_id) is None:
        raise AxerflowException("Invalid experiment ID: '%s'" % exp_id,
...
def _validate_batch_limit(entity_name, limit, length):
    if length > limit:
        ...
        raise AxerflowException(error_msg, error_code=INVALID_PARAMETER_VALUE)
...
def _validate_batch_log_data(metrics, params, tags):
    for metric in metrics:
        ...
        # https://github.com/axerflow/axerflow/issues/985
...
def _validate_experiment_name(experiment_name):
    ...
    if experiment_name == "" or experiment_name is None:
        raise AxerflowException("Invalid experiment name: '%s'" % experiment_name,
                              error_code=INVALID_PARAMETER_VALUE)

    if not is_string_type(experiment_name):
        raise AxerflowException("Invalid experiment name: %s. Expects a string." % experiment_name,
                              error_code=INVALID_PARAMETER_VALUE)
...
def _validate_experiment_artifact_location(artifact_location):
    if artifact_location is not None and artifact_location.startswith("runs:"):
        raise AxerflowException("Artifact location cannot be a runs:/ URI. Given: '%s'"
                              % artifact_location,
                              error_code=INVALID_PARAMETER_VALUE)
...
def _validate_db_type_string(db_type):
    ...
    if db_type not in DATABASE_ENGINES:
        ...
        raise AxerflowException(error_msg, INVALID_PARAMETER_VALUE)
