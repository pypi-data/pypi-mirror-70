"""
The ``axerflow.xgboost`` module provides an API for logging and loading XGBoost models.
...
from axerflow.models import Model, ModelInputExample
...
def save_model(xgb_model, path, conda_env=None, axerflow_model=None,
    ...
    :param axerflow_model: :py:mod:`axerflow.models.Model` this flavor is being added to.
    ...
    pyfunc.add_to_model(axerflow_model, loader_module="axerflow.xgboost",
                        data=model_data_subpath, env=conda_env_subpath)
    axerflow_model.add_flavor(FLAVOR_NAME, xgb_version=xgb.__version__, data=model_data_subpath)
...
def log_model(xgb_model, artifact_path, conda_env=None, registered_model_name=None,
              ...
              **kwargs):
    """
    Log an XGBoost model as an Axerflow artifact for the current run.
    ...
    """
    Model.log(artifact_path=artifact_path, flavor=axerflow.xgboost,
...
def _load_pyfunc(path):
    ...
    :param path: Local filesystem path to the Axerflow Model with the ``xgboost`` flavor.
    """
...
def load_model(model_uri):
    ...
    :param model_uri: The location, in URI format, of the Axerflow model. For example:
                      ...
                      `Referencing Artifacts <https://www.axerflow.org/docs/latest/tracking.html#
...
def autolog(importance_types=['weight']):  # pylint: disable=W0102
    """
    Enables automatic logging from XGBoost to Axerflow. Logs the following.
    ...
    def train(*args, **kwargs):
        ...
        def log_feature_importance_plot(features, importance, importance_type):
            ...
            try:
                ...
                try_axerflow_log(axerflow.log_artifact, filepath)
            finally:
                plt.close(fig)
        ...
        # logging booster params separately via axerflow.log_params to extract key/value pairs
        # and make it easier to compare them across runs.
        params = args[0] if len(args) > 0 else kwargs['params']
        try_axerflow_log(axerflow.log_params, params)
        ...
        for idx, metrics in enumerate(eval_results):
            try_axerflow_log(axerflow.log_metrics, metrics, step=idx)
        ...
        if early_stopping:
            ...
            try_axerflow_log(axerflow.log_metric, 'stopped_iteration', len(eval_results) - 1)
            try_axerflow_log(axerflow.log_metric, 'best_iteration', model.best_iteration)
            try_axerflow_log(axerflow.log_metrics, eval_results[model.best_iteration],
                           step=extra_step)
        ...
        try_axerflow_log(log_model, model, artifact_path='model')
