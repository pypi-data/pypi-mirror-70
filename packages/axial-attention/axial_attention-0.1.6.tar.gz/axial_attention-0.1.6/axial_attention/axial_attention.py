...
class ImageTransformer(nn.Module):
    def __init__(self, dim, depth, heads = 8, dim_heads = None, dim_index = 1, reversible = True):
        ...
        permute = partial(PermuteToFrom, permutation)

        layers = nn.ModuleList([])
        for _ in range(depth):
            attn_functions = nn.ModuleList([permute(Rezero(SelfAttention(dim, heads, dim_heads))) for permutation in permutations])
            conv_functions = nn.ModuleList([permute(Rezero(get_ff())), permute(Rezero(get_ff()))])
