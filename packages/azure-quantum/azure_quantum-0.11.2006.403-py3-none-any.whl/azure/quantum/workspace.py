...
from azure.quantum.client.operations import JobsOperations
...
class Workspace:
    ...
    def _create_client(self) -> JobsOperations:
        auth = self.login()
        logger.debug(f"Creating client for: subs:{self.subscription_id}, rg={self.resource_group}, ws={self.name}, frontdoor={BASE_URL}")
        client = QuantumClient(auth, self.subscription_id, self.resource_group, self.name, BASE_URL).jobs
        client.api_version = "v1.0"
        return client
