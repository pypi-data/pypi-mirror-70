...
class AbstractTimeSeriesTransformer(AutoMLTransformer):
    ...
    def __init__(self,
        ...
        self._y_imputer = None  # type: Optional[TimeSeriesImputer]
    ...
    def _partial_fit(self,
        ...
        *Note* This method is a lazy fit, it does not transforms the training data set to save the time.
        ...
        for grain, df_one in tsdf.groupby_grain():
            ...
            df_one_known = df_one[:last_nan + 1]
            if df_unknown.shape[0] > 1:
                df_unknown = self._impute_target_value(df_unknown)
                dfs_unknown.append(df_unknown[1:])
            self.dict_latest_date[grain] = max(df_one_known.time_index)
            dfs.append(df_one_known)
        self._y_imputer = self._get_target_imputer(tsdf)
    ...
    def _get_target_imputer(
        ...
        :param default_tsdf_list: A list of tsdf that has one grain.
        """
        imputer = None
        if tsdf is not None:
    ...
    def _get_default_target_imputer(self) -> TimeSeriesImputer:
        """
    ...
    @property
    def y_imputer(self) -> Optional[TimeSeriesImputer]:
        """
