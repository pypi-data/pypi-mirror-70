...
class TimeSeriesImputer(AzureMLForecastTransformerBase):
    ...
    def transform(self, X: TimeSeriesDataFrame) -> TimeSeriesDataFrame:
        ...
        if self.option == 'interpolate':
            ...
            kwargs = {'method': self.method, 'value': self.value,
                      'limit': self.limit, 'known_df': self._known_df, 'time_index': X.time_colname}
        else:
    ...
    def _fill_first_row_series_if_nan_ffill(
        ...
        :param single_known_df: The tsdf contains the first value of the data.
        :param by_cols: The grain column name.
        :return: The tsdf with first value filled.
        """
        if not isinstance(time_index, str) or \
                not TimeSeriesImputer._is_fill_leading_nan_enabled(method, single_known_df):
        ...
        if by_cols is None and not TimeSeriesImputer._is_leading_nan(single_column_tsdf):
            ...
            return single_column_tsdf
        if by_cols is None:
            TimeSeriesImputer._set_leading_nan_for_single_df(single_known_df, single_column_tsdf, time_index, method)
        else:
            ...
            if single_known_df.shape[0] == 1:
                known_grain_part = {
                    TimeSeriesImputer._get_first_row_grain_value(single_known_df, by_cols): single_known_df}
            else:
                known_grain_part = dict()  # Dict[Tuple[str], pd.Series]
                for name, max_series in single_known_df.groupby(by=by_cols):
    ...
    def _set_leading_nan_for_single_df(
        ...
        :param single_known_df: df contains the known value.
        :time_index: The time index name.
        """
        if not TimeSeriesImputer._is_fill_leading_nan_enabled(method, single_known_df):
            return single_column_df
        if not TimeSeriesImputer._is_leading_nan(single_column_df):
            # do nothing if leading nan for single column.
            return single_column_df
        last_non_nan_idx = TimeSeriesImputer._get_last_non_nan_idx(single_known_df)
        if last_non_nan_idx is not None:
            datetime_last_found = TimeSeriesImputer._get_datetime_value(
                single_known_df, time_index, last_non_nan_idx)
            ...
            if datetime_missing_index >= datetime_last_found:
                single_column_df.iloc[0] = single_known_df.iloc[last_non_nan_idx]
        return single_column_df

    @staticmethod
    def _get_first_row_grain_value(tsdf: pd.DataFrame, grain_columns: List[str]) -> Tuple[Any, ...]:
        """
        ...
        """
        return tuple([tsdf.index.get_level_values(grain)[0] for grain in grain_columns])
    ...
    def _get_known_df(
        ...
        # The _is_copy property of a dataframe could be weakref here, use copy equals to true to set it to None for
        # pickling purpose.
