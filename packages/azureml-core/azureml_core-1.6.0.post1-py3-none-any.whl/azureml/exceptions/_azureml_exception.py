...
class DatasetTimestampMissingError(AzureMLException):
    """
    An exception capturing the expected timestamp column is not assigned.
    The exception is seen from the Tabular Dataset instance when time series related APIs is called
    but the expected timestamp columns cannot be found (not set or was dropped).
