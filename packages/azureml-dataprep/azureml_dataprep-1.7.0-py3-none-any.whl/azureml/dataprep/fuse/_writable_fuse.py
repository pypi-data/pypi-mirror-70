...
class WritableFuse(FuseBase):
    ...
    def release(self, path, fh):
        # TODO we can't block user when they close file, so we need to block them when dismounting
        file = self._get_file_obj(fh)
        if file is None:
            return 0
        if file.is_dirty and self._datastore is not None:
