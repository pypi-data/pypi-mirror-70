...
class FastForestQuantileRegressorSetting(BaseLearnerSetting):
    """ Fast Forest Quantile Regression
        ...
        there remains some problems:
        1. bagging fraction is not supported by sklearn.ensemble.RandomForestRegressor
        2. feature fraction is not supported by sklearn.ensemble.RandomForestRegressor
        3. quantile sample count is not supported by sklearn.ensemble.RandomForestRegressor
    """
    ...
    def __init__(self, mode: DefaultParameters.Mode,
        ...
        self.bagging_fraction = bagging_fraction  # TODO : sklearn had not supported it yet
...
class FastForestQuantileRegressor(RegressionLearner):
    ...
    def parameter_mapping(self):
        return {
            ...
            'max_features': RestoreInfo(
                FastForestQuantileRegressionModule._args.split_fraction.friendly_name),
    ...
    def init_model(self):
        self.model = RandomForestRegressor(
            # Todo:
            #  bagging_fraction: specifies the fraction of training data to use for each tree. (0, 1)
    ...
    def _predict(self, test_x: pd.DataFrame):
        ...
        try:
            with TimeProfile("Predicting fast forest quantile regression value"):
                predicted_list = list()
                # fetch every trainer in the model, each tree constucts such a trainer
