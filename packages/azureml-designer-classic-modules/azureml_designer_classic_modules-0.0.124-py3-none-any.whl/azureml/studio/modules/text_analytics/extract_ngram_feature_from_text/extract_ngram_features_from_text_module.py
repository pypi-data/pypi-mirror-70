...
class ExtractNGramFeaturesFromTextModule(BaseModule):
    ...
    def _build_output_df(cls, input_df, feature_df):
        ...
        suffix_id = 1
        input_columns_set = set(input_df.columns)
        while not input_columns_set.isdisjoint(feature_columns):
            feature_columns = [f"{col}_{suffix_id}" for col in feature_df.columns]
            suffix_id += 1
