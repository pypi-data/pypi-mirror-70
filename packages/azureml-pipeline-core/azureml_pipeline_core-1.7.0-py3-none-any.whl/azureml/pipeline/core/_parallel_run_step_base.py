...
REQUIRED_DATAPREP_TYPES = {
    TABULAR_TYPE_INPUT: "pandas",
...
class _ParallelRunStepBase(_PythonScriptStepBase):
    ...
    :type side_inputs: list[azureml.pipeline.core.builder.PipelineData] or list[azureml.core.Dataset]
        ...
        for item in self._arguments:
            ...
            if not item.startswith("-"):
                raise ValueError(
                    ...
                    " you can only use keyword argument.".format(item)
                )

            # Skip argument with one "-"
    ...
    def _check_required_pip_packages(self, pip_packages):
        ...
        findings_core = [pip for pip in pip_packages if pip.startswith("azureml-core")]
        findings_default = [pip for pip in pip_packages if pip.startswith("azureml-defaults")]
        if not findings_core and not findings_default:
            warnings.warn(
                """
...
Please add azureml-core package in CondaDependencies.""", UserWarning)
        ...
        findings_dataprep = [pip for pip in pip_packages if pip.startswith("azureml-dataprep")]
        if not findings_dataprep:
            warnings.warn(
                "Please add pip package azureml-dataprep in CondaDependencies",
                UserWarning)
        else:
            if not [pip for pip in findings_dataprep if REQUIRED_DATAPREP_TYPES[self._input_ds_type] in pip]:
                warnings.warn(
                    """
ParallelRunStep requires azureml-dataprep[fuse] for file dataset and
azureml-dataprep[pandas] for tabular dataset package to provide the functionality.
Please add relevant package in CondaDependencies.""", UserWarning)
