...
import bento.git
from bento.error import NoConfigurationException, NoIgnoreFileException
from bento.run_cache import RunCache

DEFAULT_CONFIG_PATH = Path(__file__).parent / "configs" / "headless.yml"

DEFAULT_IGNORE_PATH = Path(__file__).parent / "configs" / constants.IGNORE_FILE_NAME
...
class BaseContext:
    ...
    headless = attr.ib(type=bool, default=False)
    cache_path = attr.ib(type=Path, default=None)
    prog_name = attr.ib(type=str, default=None)
    ...
    def validate_init(self) -> None:
        ...
        if not self.ignore_file_path.exists():
            raise NoIgnoreFileException(self.ignore_file_path)
    ...
    def _open_config(self) -> Dict[str, Any]:
        ...
        config_path = (
            self.config_path if self.config_path.exists() else DEFAULT_CONFIG_PATH
        )
