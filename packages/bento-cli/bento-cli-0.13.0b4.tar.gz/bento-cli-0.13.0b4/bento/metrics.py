...
def __get_filtered_violation_count(violations: Iterable[Finding]) -> int:
    return sum(1 for v in violations if v.filtered)


def __get_aggregate_violations(violations: List[Finding]) -> List[Dict[str, Any]]:
    """Returns count of violation per file, per check_id"""

    def grouping(v: Finding) -> Tuple[str, str]:
...
def violations_to_metrics(
    tool_id: str, timestamp: str, violations: List[Finding], ignores: List[str]
) -> List[Dict[str, Any]]:
