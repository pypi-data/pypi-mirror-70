...
def __headed_help(prog_name: str) -> str:
    return f"""
...
Use `--all` to check all Git tracked files, not just those that are staged:
    ...
    $ {prog_name} check --all [PATHS]

Optional PATHS can be specified to check specific directories or files.

See `{prog_name} archive --help` to learn about suppressing findings.
"""


def __headless_help(prog_name: str) -> str:
    return f"""
Checks for new findings.

By default, only staged files are checked. Only new findings introduced by
these staged changes will be shown.
...
def check(
    ...
    if all_:
        click.echo(f"Running checks on all tracked files...\n", err=True)
    else:
        click.echo(f"Running checks on staged files...\n", err=True)
    ...
    for tool_id, findings in all_results:
        if isinstance(findings, Exception):
            ...
            if isinstance(findings, BentoException):
                ...
                click.secho(
                    f"""
    You can view full details of this error in `{bento.constants.DEFAULT_LOG_PATH}`.
    """,
                    err=True,
                )
    ...
    if n_all_filtered > 0:
        ...
        echo_next_step(
            "To archive findings as tech debt", f"{context.prog_name} archive"
        )
        if not context.headless:
            echo_next_step(
                "To disable a specific check",
                f"{context.prog_name} disable check TOOL CHECK",
            )
    else:
...
def make_check(prog_name: str, headless: bool) -> click.Command:
    check.help = __headed_help(prog_name) if headless else __headless_help(prog_name)
    return check
