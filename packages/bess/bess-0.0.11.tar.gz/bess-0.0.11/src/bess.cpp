...
//            std::cout<<"model type "<<model_type<<endl;
    ...
    List result;
    if (path_type == 1) {
        result = sequential_path(data, algorithm, metric, sequence);
    } else {
...
//    std::cout<<"pywrap"<<endl;
    List mylist = bessCpp(x_Mat, y_Vec, data_type, weight_Vec, is_normal,
                          ...
                          path_type, is_warm_start, ic_type, is_cv, K, state_Vec, sequence_Vec,
                          s_min, s_max, K_max, epsilon, is_screening);
