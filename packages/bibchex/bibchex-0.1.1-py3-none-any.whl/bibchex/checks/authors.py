...
class InitialDottedChecker(object):
    ...
    async def check_one(self, field, name, entry):
        should_dot = self._cfg.get('dot_initials', entry, True)
...
class AllcapsNameChecker(object):
    NAME = "author-names-allcaps"

    def __init__(self):
        self._cfg = Config()

    async def check(self, entry):
        if self._cfg.get('name_allcaps_okay', entry, False):
            return []
...
class FirstNameInitialChecker(object):
    NAME = "author-names-firstinitial"

    def __init__(self):
        self._cfg = Config()

    async def check(self, entry):
        if self._cfg.get('first_name_allcaps_okay', entry, False):
            return []
...
class MiddleNameInitialChecker(object):
    NAME = "author-names-middleinitial"

    def __init__(self):
        self._cfg = Config()

    async def check(self, entry):
        if self._cfg.get('middle_name_allcaps_okay', entry, False):
            return []
...
class LastNameInitialChecker(object):
    NAME = "author-names-lastinitial"

    def __init__(self):
        self._cfg = Config()

    async def check(self, entry):
        if self._cfg.get('last_name_allcaps_okay', entry, False):
            return []
