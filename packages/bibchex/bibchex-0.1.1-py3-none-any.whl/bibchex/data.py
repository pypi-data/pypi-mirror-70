...
FIELDS = ('ID', 'ENTRYTYPE', 'journal', 'comments', 'pages',
          'abstract', 'title', 'year', 'month', 'day', 'date', 'volume', 'keyword', 'url', 'doi',
          'booktitle', 'publisher', 'number', 'isbn', 'issn', 'address', 'edition', 'organization')
...
class Entry(object):
    DOI_RE = re.compile('https?://(dx\.)?doi.org/(?P<doi>.*)')
    ...
    def get_probable_doi(self):
        ...
        if self._suggested_dois:
            best_doi = max(set(self._suggested_dois), key = self._suggested_dois.count)
            return  best_doi
        else:
            return None
    ...
    def _parse_people(self, fieldname):
        # First, split the authors' names by 'and', which may not be enclosed in braces
        ...
        for author_name in authors_split:
            ...
            if comma_count == 0:
                ...
                # Everything including the second-to-last capitalized word becoms the first name
                ...
                self._ui.warning("Entry", "Unrecognized {} format for '{}'".format(fieldname, author_name))
                continue
