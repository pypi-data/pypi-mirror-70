...
def _vertices_from_d_str(d_str: str) -> Dim1FloatType:
    """
...
def polygon_iter(file_path: str):
    ...
    #     g.iter(f"{_primtive_prefix_g}polygon"),
    #     g.iter(f"{_primtive_prefix_g}path")
    # ):
    #     yield child
    for child in itertools.chain(
        g.findall(f"{_primtive_prefix_g}polygon"),
        g.findall(f"{_primtive_prefix_g}path")
    ):
        yield child
...
def modify_svg(file_path: str, *polygon_modify_fns: PolygonModifyFnType) -> str:
    """
