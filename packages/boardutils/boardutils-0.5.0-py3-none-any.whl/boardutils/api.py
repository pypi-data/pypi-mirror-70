    ...
    def query(self, query_name, query_string):
        ...
        try:
            ...
            logging.info(
                ...
                self.endpoint, query_name + query_string, elapsed
            )
            return json.loads(result.text)
        except Exception as error:
            logging.error(
                "Board API\tHost: %s\tQuery: %s\tStatus: Error\tMessage: %s",
                self.endpoint, query_name + query_string, str(error)
            )
            raise