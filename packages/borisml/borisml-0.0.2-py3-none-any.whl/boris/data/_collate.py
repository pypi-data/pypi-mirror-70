...
class BaseCollateFunction(nn.Module):
    ...
    def forward(self, batch):
        ...
        """
        batch_a = torch.cat([self.transform(item[0]).unsqueeze_(0) for item in batch], 0)
        batch_b = torch.cat([self.transform(item[0]).unsqueeze_(0) for item in batch], 0)
        labels = torch.tensor([item[1] for item in batch]).long()
        fnames = [item[2] for item in batch]
        return torch.cat((batch_a, batch_b), 0), torch.cat((labels, labels), 0), fnames
...
class ImageCollateFunction(BaseCollateFunction):
    ...
    def __init__(self, input_size=32, cj_prob=0.8, cj_bright=0.7, 
        ...
        Args:
            ...
            kernel_size: (float) Sigma of gaussian blur is kernel_size * input_size

        """
    
        color_jitter = transforms.ColorJitter(cj_bright, cj_contrast, cj_sat, cj_hue)
        transform = transforms.Compose([transforms.RandomResizedCrop(size=input_size, scale=(min_scale, 1.0)),
