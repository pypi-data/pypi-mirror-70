...
class GenericImport(Resource):
    ...
    def post(self):
        ...
        file_full_path = os.path.join("/tmp", file_name)
        args.file.save(file_full_path)
        data = self.read_excel(file_full_path)

        for line in data:
            for k, v in list(line.items()):
                line[get_col_from_title(k)] = v
    ...
    def read_excel(self, file_full_path):
...
@ns.route('/openapi')
class OpenApiDoc(Resource):
    @auth_required
    def get(self):
        module_path = os.path.dirname(__file__)
        doc = module_path + '/openapi.md'
