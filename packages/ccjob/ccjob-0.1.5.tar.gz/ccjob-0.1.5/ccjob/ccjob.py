...
class Input(object):
    def __init__(self, fpath, inp_string=None, to_file=True):
        ...
        fpath : str
            Input file path.
        inp_string : str
            Content of input file as string (default: None).
        to_file : bool
            Whether to save input string to file.
        """
...
class Job(object):
    def __init__(self, ccinput, script=None, queue="slurm", mem=500,
        ...
        queue : str, Queue.SLURM, Queue.PBS
        ...
        self.script = script
        self.software = software #not used at the moment!
        if type(queue) == str:
    ...
    @classmethod
    def from_file(cls, infile, script=None, queue="slurm", mem=500,
                 cpus=1, time="00:15:00", partition=None, jobname="CCJob",
                 software=None, meta_file="meta.json"):

        ccinput = Input.from_file(infile)
        return cls(ccinput, script=script, queue=queue, mem=mem, cpus=cpus,
                   time=time, partition=partition, jobname=jobname,
                   software=software, meta_file=meta_file)
