    ...
    block = '''\
        class Application(param.Parameterized):
            ...
            def __init__(self, **params):
                ...
                all_pipelines = api_call(api.get_workspaces_pipelines_api_v1_workspaces_workspace_id_pipelines_get, info[user][constants.ACTIVE_WORKSPACE])
        
                # get a dataframe of all results + all hyperparameter combinations
                for p in all_pipelines:
                    if p.pipeline_run is not None and p.pipeline_run.status == 'Succeeded':
                        eval_path = get_eval_dir(p.id, info)
                        evaluation = tfma.load_eval_result(eval_path)
        
                        for s, m in evaluation.slicing_metrics:
                            result_list.append(dict([('pipeline_id', str(p.id)), 
                                                     ('pipeline_name', str(p.name)),
                                                     ('slice_name', s[0][0] if s else ''), 
                                                     ('slice_value', s[0][1] if s else ''), 
                                                     ('metrics', m[''][''])]))
        
                        h_dict = api_call(p_api.get_hyperparameters_pipeline_api_v1_pipelines_pipeline_id_hyperparameters_get, 
                                     p.id)
            ...
            def parameter_graph(self): 
                if self.performance_metric_selector and len(self.hyperparameter_selector) > 0:
                    ...
                    filtered_hparams = self.hparam_info[self.hyperparameter_selector + ['pipeline_name']]
                    ...
                    fig = go.Figure(data=go.Parcoords(dimensions=new_dims))
                else:
                    fig = px.scatter(pd.DataFrame(),
                                     marginal_y='rug',
                                     width=1100,
