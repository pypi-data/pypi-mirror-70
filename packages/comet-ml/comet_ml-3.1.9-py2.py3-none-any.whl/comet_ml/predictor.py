...
class Predictor(object):
    ...
    def __init__(
        ...
        **defaults
        ...
        self._low_level_http_client = LowLevelHTTPClient(
            self.predictor_url, self.config["comet.timeout.http"]
        )
    ...
    def _local_stop_early(self, **data):
        ...
        if self.wait >= self.patience:
            # For cases where model is not stopped after _local_stop_early returns True
        ...
        if (len(self.defaults["TS"]) % self.interval) == 0:
            ...
            if self.wait >= self.patience:
                if self.stop_step is None:
                    ...
                    self.stop_val = self.defaults.get("TS")[-1]
                    self.stop_min = min(self.defaults.get("TS"))
    ...
    def _global_stop_early(self, **data):
        ...
        if len(self.defaults["TS"]) < self.min_samples:
            return False
    ...
    def _get_trial_state(self, **data):
        ...
        :return:
        """
        experiment_count, best_metric = self._get_trial_metrics()
