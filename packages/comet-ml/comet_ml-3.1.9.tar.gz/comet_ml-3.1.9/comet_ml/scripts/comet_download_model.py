#!/usr/bin/env python
# -*- coding: utf-8 -*-
# *******************************************************
#   ____                     _               _
#  / ___|___  _ __ ___   ___| |_   _ __ ___ | |
# | |   / _ \| '_ ` _ \ / _ \ __| | '_ ` _ \| |
# | |__| (_) | | | | | |  __/ |_ _| | | | | | |
#  \____\___/|_| |_| |_|\___|\__(_)_| |_| |_|_|
#
#  Sign up for free at http://www.comet.ml
#  Copyright (C) 2015-2020 Comet ML INC
#  This file can not be copied and/or distributed without
#  the express permission of Comet ML Inc.
# *******************************************************

"""
Download a registry model locally.

To download a registry model named "My Model" from the workspace "My Workspace"
at version 1.0.0, you can run:

$ comet download_model --workspace "My Workspace" --registry_name "My Model"
--model_version "1.0.0"

The registry model files will be downloaded to a directory named "model". You
can choose a different output directory by using the "--output" flag.
"""

import argparse
import logging
import sys

from comet_ml import API

LOGGER = logging.getLogger("comet_ml")
ADDITIONAL_ARGS = False


def get_parser_arguments(parser):
    parser.add_argument(
        "--workspace",
        help="the workspace name of the registry model to download",
        required=True,
    )
    parser.add_argument(
        "--registry_name",
        help="the name of the registry model to download",
        required=True,
    )
    parser.add_argument(
        "--model_version",
        help="the semantic version of the registry model to download (for example: 1.0.0)",
        default=None,
    )
    parser.add_argument(
        "--model_stage",
        help="the stage of the registry model to download (for example: production)",
        default=None,
    )
    parser.add_argument(
        "--output",
        help="the output directory where to download the model, default to `model`",
        default="model",
    )


def download_model(args, rest=None):
    # Called via `comet download_model --workspace TEST --registry_name MY_MODEL
    # --version 1.0.0` args are parsed_args
    api = API()

    api.download_registry_model(
        args.workspace,
        args.registry_name,
        expand=True,
        output_path=args.output,
        stage=args.model_stage,
        version=args.model_version,
    )


def main(args):
    parser = argparse.ArgumentParser(
        description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter
    )
    get_parser_arguments(parser)
    parsed_args = parser.parse_args(args)
    download_model(parsed_args)


if __name__ == "__main__":
    # Called via `python -m comet_ml.scripts.comet_download_model --workspace
    # TEST --registry_name MY_MODEL --version 1.0.0`
    main(sys.argv[1:])
