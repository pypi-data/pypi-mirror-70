...
class PersonNamePrefixModel(models.Model):

    class Meta:
        db_table = "name_prefixes"
        verbose_name = _('Person name prefix')
        verbose_name_plural = _('Person name prefixes')
...
class CustomerModel(MasterDataModel):
...
class PersonModel(CustomerModel):
...
class OrganizationModel(CustomerModel):
