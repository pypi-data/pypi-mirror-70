...
def get_cleaned_event_and_type(event) -> Tuple[Union[list, dict], EventTypes]:
    ...
    if isinstance(event, dict):
        ...
        raise TypeError(f"Missing task_id or asset_id keys in event")

    if not isinstance(event, list):
        raise TypeError(f"Event is not a list or a dict")
    ...
    if isinstance(first_event, list):
        ...
        raise Exception(f"Missing scheduler_start key in scheduler event")
    ...
    raise TypeError(f"Event is not either a scheduler or kafka consumer")
