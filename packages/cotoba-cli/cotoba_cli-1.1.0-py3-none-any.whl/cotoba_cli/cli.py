...
def cli_root(version):
    if version:
        cli_directory = os.path.abspath(os.path.dirname(__file__))
        pyproject_file = os.path.join(cli_directory, '../pyproject.toml')
        pyproject_dict = toml.load(pyproject_file)
        click.echo(pyproject_dict['tool']['poetry']['version'])
        sys.exit(0)
...
def main():
    ...
    logging_config.dictConfig({
        ...
        'handlers': {
            'fileHandler': {
                ...
                'filename': config.get_config_filepath('cotoba-cli.log'),
                'maxBytes': 1000000,
                'backupCount': 3,
                'encoding': 'utf-8',
            }
    ...
    try:
        ...
        click.echo('Unexpected Error has occurred.'
                   ' See the cotoba-cli.log in detail.',
                   err=True)
