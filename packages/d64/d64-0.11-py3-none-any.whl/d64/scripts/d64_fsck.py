...
def check_bam():
    ...
    if isinstance(IMAGE, D71Image):
        ...
        for sector in range(0, IMAGE.max_sectors(IMAGE.EXTRA_BAM_TRACK)):
            if not IMAGE.bam.is_allocated(IMAGE.DIR_TRACK, 0):
                msg = "Track {:d}:{:d} not allocated".format(IMAGE.EXTRA_BAM_TRACK, sector)
                errors += fix_error(msg, fix_unalloc_block, block=Block(IMAGE, IMAGE.EXTRA_BAM_TRACK, sector))
            elif VERBOSE:
                print("Block {:d}:{:d} allocated".format(IMAGE.EXTRA_BAM_TRACK, sector))
            remember_used(IMAGE.EXTRA_BAM_TRACK, sector)
...
def check_dir_links():
    ...
    errors = 0
    block = Block(IMAGE, IMAGE.DIR_TRACK, 1)
...
def fix_dos_version():
    ...
    if VERBOSE:
        print("Setting DOS version to {:d}".format(IMAGE.DOS_VERSION-ord('0')))
    return 0


def fix_dos_types():
    """Fix DOS format type fields."""
    IMAGE.dos_type = ((IMAGE.DOS_FORMAT, IMAGE.DOS_FORMAT))
    if VERBOSE:
