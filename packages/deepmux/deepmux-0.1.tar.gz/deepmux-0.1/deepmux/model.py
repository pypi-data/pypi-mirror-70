...
class Model:
    ...
    def __init__(self, name: str, state: ModelState, input_shape: numpy.array, output_shape: numpy.array,
                 data_type: str, service_url: str = BASE_URL):
        self.name = name
    ...
    def run(self, tensor: numpy.ndarray):
        ...
        """
        self.state = getattr(ModelState, self.interface.get(self.name).get('state')).value
        ...
        return self.interface.run(model=self.name, tensor=tensor, output_shape=self.output_shape)
