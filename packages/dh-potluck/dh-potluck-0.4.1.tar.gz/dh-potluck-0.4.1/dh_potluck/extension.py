...
def structure_logger(logger):
    def patch_func(func, level):
        def _patch_func(*args, **kwargs):
            extra = {**kwargs.get('extra', {}), **{'level': level.upper()}}
            return func(*args, **{**kwargs, **{'extra': extra}})

        return _patch_func

    # Insert level into json log
    for level in ['debug', 'info', 'warning', 'error', 'critical']:
        func = getattr(logger, level)
        setattr(logger, level, patch_func(func, level))
