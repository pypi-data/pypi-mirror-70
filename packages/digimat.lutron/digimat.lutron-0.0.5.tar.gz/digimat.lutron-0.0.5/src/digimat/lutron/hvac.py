...
class LutronObjectHVAC(LutronObject):
    ...
    def match(self, pattern):
        if super().match(pattern):
            return True
        if '*fan' in pattern:
            if self.fanMode not in [None, 4]:
                return True
        return False
    ...
    def setSetPoints(self, spheat, spcool=None):
        if spcool is None:
            spcool=spheat+2.0
        if spheat<spcool:
            spheat='%.1f' % spheat
    ...
    def setSPHeat(self, sp):
        return self.setSetPoints(sp, sp+2.0)

    def setSPCool(self, sp):
        return self.setSetPoints(sp-2.0, sp)

    def setSPMiddle(self, sp, dt=2.0):
        return self.setSetPoints(sp-dt/2.0, sp+dt/2.0)
