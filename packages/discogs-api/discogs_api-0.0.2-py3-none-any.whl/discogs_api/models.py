...
class BasePaginatedResponse(object):
    ...
    def _url_for_page(self, page):
        ...
        if self._sort_key is not None:
            base_qs.update(
                {"sort": self._sort_key, "sort_order": self._sort_order,}
            )
...
class WantlistItem(PrimaryAPIObject):
    ...
    def __repr__(self):
        return self.repr_str(
            "<WantlistItem {0!r} {1!r}>".format(self.id, self.release.title)
        )
...
class CollectionItemInstance(PrimaryAPIObject):
    ...
    def __repr__(self):
        return self.repr_str(
            "<CollectionItemInstance {0!r} {1!r}>".format(self.id, self.release.title)
        )
...
class CollectionFolder(PrimaryAPIObject):
    ...
    def __repr__(self):
        return self.repr_str(
            "<CollectionFolder {0!r} {1!r}>".format(self.id, self.name)
        )
...
class Listing(PrimaryAPIObject):
    ...
    def __repr__(self):
        return self.repr_str(
            "<Listing {0!r} {1!r}>".format(self.id, self.release.data["description"])
        )
