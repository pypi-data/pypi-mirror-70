...
class YOLO(object):
    ...
    def __init__(self,
        ...
        self.init_timestamp = "YOLO-" + datetime.now().strftime("%d%m%Y-%H%M%S")
    ...
    def predict(self,
        ...
        if text_format:
            ...
            return tf.image.draw_bounding_boxes(
                tf.cast(img, dtype=tf.float32), boxes_yx, self.colors)

    def _append_classifier(self):
        self.feature_extractor.add(
            # 1x1 convolution.
            tf.keras.layers.Conv2D(
                filters=(self.boxes * (5 + self.n_classes)),
                kernel_size=(1, 1),
                strides=(1, 1),
                padding="same",
        ...
        self.feature_extractor.add(
            tf.keras.layers.Reshape((self.grid_h, self.grid_w, self.boxes,
                                     5 + self.n_classes)))
    ...
    def coco_metric_format(self, tensor, batch_size, class_conf_threshold,
        ...
        for img in range(batch_size):
            ...
            for c in range(self.n_classes):
                ...
                class_mask = tf.cast(
                    tf.equal(classes, tf.cast(c, dtype=tf.float32)),
                    dtype=tf.float32)
    ...
    def train_step(self, x, y, batch_size, step, max_outputs, file_writer,
        ...
        with file_writer.as_default():
            with tf.GradientTape() as tape:
                ...
                tf.summary.scalar(
                    "inference_time[s]", img_per_second, step=step)
    ...
    def decode_yolo_output(self,
        ...
        # Flatten classes
        masked_classes = tf.cast(
            tf.reshape(masked_classes, shape=[shape]), dtype=tf.float32)
        ...
        for c in range(n_classes):
            class_mask = tf.cast(
                tf.math.equal(masked_classes, c), dtype=tf.float32)
    ...
    def _prediction_to_tensorboard(self,
        ...
        for img in range(max_outputs):
            ...
            #selected_classes = tf.gather(classes, selected_indices)
            #selected_confidence = tf.gather(confidence, selected_indices)
            selected_boxes = tf.cast(
                tf.expand_dims(selected_boxes, 0), dtype=tf.float32)
            ...
            tb_img = tf.image.draw_bounding_boxes(
                tf.cast(
                    tf.expand_dims(input_img[img, :, :, :], 0),
    ...
    def _evaluate(self,
        ...
        if tb:
            with file_writer.as_default():
                tf.summary.scalar(
                    "total_loss", val_loss_avg.result(), step=step)
                for key in metrics:
        ...
        if mlflow:
            ...
            mlflow.log_metric(
                ...
                step=step_float)
            mlflow.log_metric(
                "AR100", metrics["Recall/AR@100"].item(), step=step_float)
            mlflow.log_metric(
                "val_loss",
