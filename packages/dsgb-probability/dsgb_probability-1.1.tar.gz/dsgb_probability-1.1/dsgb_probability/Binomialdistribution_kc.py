...
class Binomial(Distribution):
    ...
    def pdf_plot(self):
        ...
        """
        x_value = list(range(0, self.n + 1))
        pdf_value = list(map(lambda x: self.pdf(x), list(range(0, self.n + 1))))
        plt.bar(x_value, pdf_value)
        plt.xlabel('Size')
        plt.ylabel('Probability')
        plt.title('Probability density function of binomial')
        plt.show()
        return x_value, pdf_value
    ...
    def __add__(self, other):
        ...
        result.mean = self.p * (self.n + other.n)
    ...
    def __repr__(self):    
        ...
        """
        return "mean is {0}, standard deviation {1}, p {2}, n {3}".format(self.mean, self.stdev, self.p, self.n)
