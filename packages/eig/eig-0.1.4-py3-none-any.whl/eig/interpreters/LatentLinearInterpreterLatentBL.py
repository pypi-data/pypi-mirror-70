"""
LatentLinearInterpreterLatentBL.py

Class for Latent Linear Interpreter (H-L-IG: Hidden feature space Linear Integrated Gradients). This class gives
attributes for features using linear path in the hidden feature space produced by the autoencoder. Baselines are assumed
to be computed in the latent space of the autoencoder.

Defines:
+ class LatentLinearInterpreterLatentBL
"""
import numpy as np
...
class LatentLinearInterpreterLatentBL(Interpreter):
    """
    This is the Interpreter for the linear path in the hidden/latent feature space. Baselines are computed in the latent
    space of the autoencoder.
    """

    def __init__(self, baselines_data, samples_data, encoder_data=None, decoder_data=None,
                 other_data_nn=None, nn_model=None, autoencoder=None, baseline_type=BASELINES_MEDIAN, no_baselines=3,
                 no_points=250):
        """
        ...
        assert autoencoder is not None, "Provide autoencoder model for generating latent baseline and path. "
