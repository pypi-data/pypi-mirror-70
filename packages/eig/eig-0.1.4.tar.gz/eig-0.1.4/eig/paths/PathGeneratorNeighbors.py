...
Defines:
+ class NeighborsPathGenerator(AbstractPathGenerator)
...
class NeighborsPathGenerator(AbstractPathGenerator):
    ...
    """
    def __init__(self, data, num_neighbors=DEFAULT_NUM_NEIGHBORS,
        ...
        self._neighbors.fit(
            self._data.reshape(
                (len(self._data), np.prod(self._data.shape[1:]))
            )
        )
    ...
    def get_path(self, src, dst, npoints):
        """ Obtain nonlinear path between src and dst using neighbors graph

        Takes the shortest path between src and dst over the neighbors graph.
        If distance between src and dst is closer than their nearest neighbors
        on the graph, linear path is used instead.
        ...
        np.ndarray(shape=(npoints, *src.shape))
            The resulting path from src to dst

        Notes
        -----
        Raises RuntimeError if the path requires more points than specified.
        Set npoints to be larger than self.est_min_npoints to ensure that it
        won't occur.
        """
        # put the source and destination together in encoded space, flattened

        endpoints = np.array(
            [src[np.newaxis, :].flatten(), dst[np.newaxis, :].flatten()]
        )
        ...
        if dist_direct < dist_graph_min:
            ...
            return self._get_linear_path(src, dst, npoints)
        ...
        min_path = np.zeros(shape=[num_steps + 1] + list(src.shape))
        min_path[0] = src  # source
        min_path[-1] = dst  # destination
        cur_ndx = dst_ndx
