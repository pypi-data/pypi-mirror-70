    ...
    def auto_validate(self, conn):
        ...
        for cls in poolvalidators.validators:
            if isinstance(conn, cls):
                validator = poolvalidators.validators[conn].validate
                break
...
class ConnectionPool(ConnectionPoolBase):
    ...
    def __init__(
        self, connection_factory, limit=0, validator="auto", timeout=timeout
    ):
        super(ConnectionPool, self).__init__(
            connection_factory, limit, validator
        )
    ...
    def _getconn(self):
        ...
        try:
            return self._pool.get(
                block=self.reached_limit, timeout=self.timeout
            )
        except queue.Empty:
            if self.limit:
                with self.lock:
...
class SQLAlchemyPool(ConnectionPool):
    def __init__(self, connection_factory, limit=0, validator=None):
        ...
        super(SQLAlchemyPool, self).__init__(
            _connection_factory, limit, validator
        )
