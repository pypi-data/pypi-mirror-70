...
animals = DialogueFlow('_local_start', macros={'MOVIE':Movie(),'MOVIEFACT':MovieFact(),'CHECKMOVIE':CheckKnowsMovie(),
                                               'JOKE':Joke(),'JOKEANSWER':JokeAnswer()})

animals.knowledge_base().load_json_file(PATHDIR.replace('__***__', '_common.json'))
animals.knowledge_base().load_json_file(PATHDIR.replace('__***__', 'animals.json'))
        ...
        '[at some point],[in the future],[when,own,place],[when,move],[#NOT(not),am,interested]}': {
            ...
            '#GSRET(exit)': {
                ...
                '[$wants_pet=#ONT(_animals)]':{
                    ...
                    '`Oh, you don\'t want either a cat or a dog? An ` $wants_pet ` would be a cool pet '
