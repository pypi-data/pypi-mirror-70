...
activity = {
    ...
    # chores
    '#GATE [#NOT(#ONT(not)),$chore=#ONT(chore)]': {
    ...
    # errands
    '#GATE [#NOT(#ONT(not)),{#EXP(travel),#LEM(do)},$errand_dest=#ONT(errand)]': {
        'score': 2.0,
    ...
    # homework
    '#GATE [#NOT(#ONT(not)),$homework=#EXP(homework)]': {
        'score': 3.0,
    ...
    #homework
    '#GATE [#NOT(#ONT(not)),#LEM(complete,finish),$homework=#EXP(homework)]': {
    ...
    # school
    '#GATE [#NOT(#ONT(not)),{#EXP(travel),#LEM(attend,do,have)},{school,#EXP(class)}]': {
        'score': 2.0,
    ...
    # talking to somebody
    '#GATE [#NOT(#ONT(not)),#LEM(see,talk,chat,call),$contact=#ONT(related_person)]': {
    ...
    # play with somebody
    '#GATE [#NOT(#ONT(not)),#LEM(play),with,$contact=#ONT(related_person)]': {
    ...
    # play with a pet
    '#GATE [#NOT(#ONT(not)),#LEM(play),with,$animal=#ONT(animal)]': {
    ...
    '#GATE [#NOT(#ONT(not)),#EXP(travel),$dest=#ONT(indoor_destination)]': {
        'score': 4.0,
        '` `': 'exit_activity'
    },

    # going somewhere outside
    '#GATE [#NOT(#ONT(not)),{#EXP(travel),#LEM(play)},$dest=#ONT(outdoor_destination)]': {
        'score': 4.0,
    ...
    # play outside
    '#GATE [#NOT(#ONT(not)),{#EXP(travel),#LEM(play)},{outdoors,outdoor,outside}]': {
        'score': 4.0,
    ...
    # play inside
    '#GATE [#NOT(#ONT(not)),{#EXP(travel),#LEM(play)},{indoor,indoors,inside}]': {
        'score': 4.0,
        '[!"I hope you enjoyed your indoor activities, it is good to keep yourself entertained! "]': 'exit_activity'
    },

    # playing with child toys
    '#GATE [#NOT(#ONT(not)),#LEM(play),{game,games,house,home,dolls,doll,barbie,barbies,toy,toys}]': {
        '[!"I hope you enjoyed playing with your toys, it is good to keep yourself entertained! "]': 'exit_activity'
    },

    # watching tv
    '#GATE [#NOT(#ONT(not)),$watching={#EXP(television),#EXP(hbo),#LEM(movie,channel,show,cartoon),youtube,netflix,hulu}]': {
    ...
    # playing video games
    '#GATE [#NOT(#ONT(not)),#LEM(play),$console=#ONT(game_console)]': {
    ...
    # playing video games
    '#GATE [#NOT(#ONT(not)),#LEM(play),{#ONT(vgames),[!video, #LEM(game)]}]': {
    ...
    # sports or exercise
    '#GATE [#NOT(#ONT(not)),{$exercise={#EXP(exercise),[!#LEM(work),out],#ONT(outdoor_exercise,indoor_exercise)},[#LEM(play),$exercise=#ONT(sports)]}]':{
            'score':3.0,
...
hobby_flow = {
    'state': 'start',
    'enter': '#GCOM(hobby) #GOAL(hobby) #GATE',
...
rules = {
    '[{my, to, with} $with_clause=#ONT(related_person)]': '',
    '#IF($with_clause!=None)': '#INC($to_clear)',
    '#IF($to_clear>1)': '#SET($with_clause=None)'
}
...
hobby.load_transitions(rexit)
hobby.load_global_nlu(ruser)
