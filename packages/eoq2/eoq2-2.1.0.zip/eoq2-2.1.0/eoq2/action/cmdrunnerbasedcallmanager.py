...
class CmdRunnerBasedCallManager(CallManager):
    ...
    def ChangeCallStatus(self,callId,status):
        if(callId in self.runningCalls):
            ...
            if(call.status != status): #make sure this is really an update
                ...
                if(CallTypes.ASY == call.ctype and status in CALL_STATUS_FINAL_TYPES):
    ...
    def RunCallSync(self,name,args,optsArr,tid):
        #name = args[0]
        #obtain the handler
    ...
    def RunCallAsync(self,name,args,optsArr,tid,sessionId=None):
        info = self.actionInfos[name]
        action = info[0]
        handler = info[1]
        #check if the arguments are appropriate
        ...
        opts.FromArray(optsArr)
        #call the handler
        callInfo = self.CreateNewCall(name,handler,CallTypes.ASY,sessionId)
    ...
    def CreateNewCall(self,name,handler,ctype,sessionId=None):
        callId = self.currentCallId
        self.currentCallId+=1
        callInfo = Call(callId,name,handler,ctype,sessionId)
