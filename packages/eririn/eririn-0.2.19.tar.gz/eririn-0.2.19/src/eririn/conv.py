...
def main(arguments):
    ...
    for fidx, f in enumerate(inputs):
        ...
        with Image.open(f) as im:
            ...
            rgb = list(rgb)
            rgbmap = {((0.2126 * r + 0.7152 * g + 0.0722 * b), r, g, b): (r, g, b) for r, g, b in rgb}
            reduced_ratio = 2
            so = sorted(rgbmap.items(), key=itemgetter(0)[0])

            new_color_dict = {}
            con = (0, 0, 0)
            for idx, s in enumerate(so):
                cval, ctup = s
                mod = idx % reduced_ratio
                if mod == 0:
                    con = ctup
                new_color_dict[ctup] = con[:]
            ...
            for x in tqdm(range(c_width)):
                for y in range(c_height):
                    ...
                    if bytes_written > LIMIT_OF_EXCEL:
                        break
                    color = ctuple2cstr(rgb[y * c_width + x], dic=new_color_dict)
