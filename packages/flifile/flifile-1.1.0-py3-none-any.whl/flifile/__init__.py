...
class FliFile:
    ...
    def getdata(self, subtractbackground=True, squeeze=True):
        ...
        data_size = np.prod(self._di['IMSize'], dtype=np.uint64)
        if self._di['Compression'] > 0:
            fid = self.path.open(mode='rb')
            fid.seek(self._datastart)
            dcmp = zlib.decompressobj(32 + zlib.MAX_WBITS)  # skip the GZIP header
            data = np.frombuffer(dcmp.decompress(fid.read()), dtype=self._di['IMType'])
            bg = data[data_size:]
            ...
            data = np.fromfile(self.path, offset=self._datastart,
                               dtype=self._di['IMType'], count=data_size)
            data = data.reshape(self._di['IMSize'][::-1])
    ...
    def getbackground(self, squeeze=True):
        ...
        if self._bg is not None:
            ...
            if self._di['Compression'] > 0:
                ...
                bgstart = bgstart + np.uint64(np.dtype(self._di['IMType']).itemsize) * np.prod(self._di['IMSize'],
                                                                                               dtype=np.uint64)
                data = np.fromfile(self.path, offset=bgstart,
                                   dtype=self._di['BGType'], count=np.prod(self._di['BGSize'], dtype=np.uint64))
