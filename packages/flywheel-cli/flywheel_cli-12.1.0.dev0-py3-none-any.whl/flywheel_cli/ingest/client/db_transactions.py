...
def one_container_multiple_uid_item_ids(db: Session, ingest_id: UUID, container_type: str, uid_type: str) -> typing.Set[UUID]:
    """Get Item IDs where 1 container has different UIDs"""
    sub_query = (
        db.query(
            getattr(M.UID, container_type),
            sqla.sql.func.count(getattr(M.UID, uid_type).distinct()).label("count")
        )
        .filter(M.UID.ingest_id == ingest_id)
        .group_by(getattr(M.UID, container_type))
        .subquery()
    )
    query = (
        db.query(
        ...
        .join(M.Item, M.Item.container_id == getattr(sub_query.columns, container_type))
...
def one_uid_multiple_session_container_item_ids(db: Session, ingest_id: UUID, container_type: str, uid_type: str) -> typing.Set[UUID]:
    """Get Item IDs where 1 UID exists in multiple containers"""

    sub_query = (
        db.query(
            getattr(M.UID, uid_type),
            sqla.sql.func.count(getattr(M.UID, container_type).distinct()).label("count")
        )
        .filter(M.UID.ingest_id == ingest_id)
...
def uids_in_new_session_container(db: Session, ingest_id: UUID, container_type: str, uid_type: str) -> typing.Set[str]:
    ...
    query = (
        ...
        .join(M.Container, M.Container.id == getattr(M.UID, container_type))
