...
class ResolveTask(Task):
    ...
    def _resolve_item_containers(self, item):
        ...
        path = []
        uid_updates = {}
        uid_ids = None # pylint: disable=W0612
        def get_uid_ids():
            if uid_ids is None:
                uid_ids = set()
        ...
        def add_update(container_type, container_id):
            for uid in get_uid_ids():
            ...
            if c_name in item_context:
                ...
                if self.ingest_config.detect_duplicates:
                    if current.level == T.ContainerLevel.session:
                        add_update("session_container_id", current.id)

                    if current.level == T.ContainerLevel.acquisition:
                        add_update("acquisition_container_id", current.id)
