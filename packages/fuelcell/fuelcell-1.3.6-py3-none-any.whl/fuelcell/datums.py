...
def ca_process(data=None, current_column=2, potential_column=1, area=5, reference='she', export_data=False, threshold=5, min_step_length=50, pts_to_average=300, pyramid=False, **kwargs):
	"""
	...
	**kwargs:
	...
	for k, df in data.items():
		...
		if export_data:
			utils.save_data(ca_processed, k+'.csv', 'processed')
	if len(newdata) == 1:
...
def cp_process(data=None, current_column=2, potential_column=1, area=5, reference='she', export_data=False, threshold=5, min_step_length=25, pts_to_average=300, pyramid=True, **kwargs):
	"""
	...
	**kwargs:
	...
	for k, df in data.items():
		...
		if export_data:
			utils.save_data(cp_processed, k+'.csv', 'processed')
	if len(newdata) == 1:
		newdata = list(newdata.values())[0]
	return newdata
...
def cv_process(data=None, current_column=1, potential_column=0, area=5, reference='she', export_data=False, **kwargs):
	"""
	...
	**kwargs:
	...
	for k, df in data.items():
		...
		if export_data:
			utils.save_data(cv_processed, k+'.csv', 'processed')
	if len(newdata) == 1:
		newdata = list(newdata.values())[0]
	return newdata
