...
[![pipeline status](https://gitlab.com/nofusscomputing/projects/python-gitlab-management/badges/master/pipeline.svg)](https://gitlab.com/nofusscomputing/projects/python-gitlab-management/) [![PyPI version](https://badge.fury.io/py/python-gitlab-management.svg)](https://pypi.org/project/python-gitlab-management) 
...
### CLI
Gitlab-management can be used via cli with command `python3 gitlab-management -H {GITLAB_URL} -T {GITLAB_PRIVATE_TOKEN}` replacing `{GITLAB_URL}` with the gitlab url and `{GITLAB_PRIVATE_TOKEN}` with your gitlab private token. to view all available options, use switch `-h` or `--help`
