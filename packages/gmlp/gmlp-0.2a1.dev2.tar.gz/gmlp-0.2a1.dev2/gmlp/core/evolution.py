...
class NeuralNetwork:
    ...
    def __init__(self, config):
        ...
        self.selections = self.config.sections()
        self.weights = {}
        self.activation = None
        for i in range(len(self.selections)):
            if self.selections[i] == 'MAIN':
                self.activation=self.config.get(self.selections[i], 'activation')
        for layer in range(len(self.config.get('INPUT', 'inputs'))):
            self.inputlayers = Connection()
            self.input_list = json.loads(self.config.get("INPUT","inputs"))
            for i in range(len(self.input_list)):
                self.inputs = self.inputlayers.connect(f'Input {i+1}', self.input_list[i])
            break
