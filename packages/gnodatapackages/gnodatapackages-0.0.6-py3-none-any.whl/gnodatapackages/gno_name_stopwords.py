...
def gno_inpersonname_remove():
    file = open("gnodatapackages/personname_wordsremove_gno.csv", "r", encoding = "utf8", errors = 'backslashreplace')
...
def gno_honorifics_remove():
    file = open("gnodatapackages/honorifics_remove_gno.csv", "r", encoding = "utf8", errors = 'backslashreplace')
    NameStopWords = pd.read_csv(file, sep = ",", skipinitialspace = True, keep_default_na = False ,header="infer")
    file.close()
    return NameStopWords