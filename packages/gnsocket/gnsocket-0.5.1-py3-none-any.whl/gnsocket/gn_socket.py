...
class GNCSocket:
    ...
    * client
            ...
            while t:
                while count < 4:
                    if not reader.at_eof():
                        ...
                        # self.logger.error("La conexión se cerró debido a EOF en %s," % writer)
                        raise Exception("Conexión perdida")
                    b_header += char_recv
                    if char_recv == b" ":
                        count += 1
                #gprint("Binary HEADER")
                # rprint(b_header)
    ...
    async def set_reader_writer(self, reader, writer):
        print("Cargando reader and writer")
        idc = self.set_idc()
        ...
        try:
            if mode == 'server':
                if AF_TYPE == socket.AF_UNIX:
                    ...
                    future_unix_server = asyncio.start_unix_server(callback_io,
                                                                   loop=loop,
                                                                   path=self.get_path(),
                                                                   limit=self.backlog,
                                                                   ssl=context)
                    self.server = await asyncio.wait_for(future_unix_server, timeout=self.timeout)
                    ...
                    future_tcp_server = asyncio.start_server(
                        callback_io,
                        loop=loop,
                        host=host,
                        port=port,
                        ...
                        ssl=context)
                    self.server = await asyncio.wait_for(future_tcp_server, timeout=self.timeout)
    ...
    async def wait_closed(self):
        ...
        if self.mode == 'server' and self.server:
            print("Closing server(wait closed)")
