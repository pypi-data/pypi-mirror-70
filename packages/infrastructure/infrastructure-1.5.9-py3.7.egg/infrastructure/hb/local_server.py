...
# @Last Modified time: 2020-06-02 16:50:05
...
class LocalServer(object):
	...
	def filter_local_jar(self,jar_list,command):
		...
		if download_jar:
			self.alreadyRecord.download_jar = download_jar
	...
	def checkZip(self,zipPath):
		if os.path.exists(zipPath):
			command = "rm -rf {zipPath}".format(zipPath)
	...
	def upzipJar(self,jar_list="",jarDir="",special_deal=False):
		...
		os.popen("rm -rf" + self.jarDir + "/all/*").read()
	...
	def jacocoReport(self):
		...
		if (len(eval(self.alreadyRecord.server_ip)))==1:
			classfile = self.jarDir + "/all"
			sourcefiles = self.serviceRepoDir
	...
	def parseReport(self):
		...
		if os.path.exists(index_path):
			...
			report_html_url = ['https://fat-rfautotest.hellobike.cn:8001/report/{}/jc/{}/index.html'.format(
					self.alreadyRecord.service_name,self.alreadyRecord.id),
				'https://fat-rfautotest.hellobike.cn:8001/report/{}/jc/{}/incrementReport.html'.format(
				self.alreadyRecord.service_name,self.alreadyRecord.id)]
			for url in report_html_url:
				JavaCoverageReportUrl.objects.create(coverageRecord=self.alreadyRecord,reportUrl=url)
