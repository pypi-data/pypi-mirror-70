...
# @Last Modified time: 2020-06-05 11:34:25
import time
import os
...
class OperateServer(object):
	...
	def addJacocoArg(self,service_name,port):
		...
		"cd /workspace/carkey/{service_name}/latest\n".format(service_name=service_name),
		"""sudo sed -i -e '1i START_OPTS="$START_OPTS -javaagent:/workspace/carkey/jarlibs/jacoco/jacocoagent.jar=includes=*,output=tcpserver,address=*,port={port},append=true"' init.script\n""".format(
			port=port)]
