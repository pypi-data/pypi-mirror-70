...
class InOutScan():
    ...
    def clear(self, data_age):
        """
        Remove all data from the cache file that is older than {data age} hours
        """
        query = """DELETE FROM inout_events
                   WHERE
                      timestamp < :date_in_past AND
                      sentToApi = 1
                """
        date_in_past = int(time.time()) - data_age * 60 * 60
        data = {'date_in_past': date_in_past}
        cursor = self.sqlite_connection.cursor()
        cursor.execute(query, data)
        self.sqlite_connection.commit()
