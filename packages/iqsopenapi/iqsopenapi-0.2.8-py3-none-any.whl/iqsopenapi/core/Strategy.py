...
class Strategy(object):
    def __init__(self, event_bus,scope, ucontext):
        ...
        self.__handle_market_close = scope.get('on_market_close', None)
        ...
        if self.__handle_market_close is not None:
            event_bus.add_listener(EVENT.On_Market_Close, self.handle_market_close)
    ...
    def init(self):
        logger.info("begin user strategy init")
        self.__init(self.__user_context)
        logger.info("init user strategy complated")

    def handle_bar(self, event):
        self.__rebuild_event_data(event.data)
        self.__handle_bar(self.__user_context, event.data)
    
    def handle_market_close(self, event):
        self.__rebuild_event_data(event.data)
        self.__handle_market_close(self.__user_context, event.data)
