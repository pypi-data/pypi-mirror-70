...
class YKPaperTradeApi(ITradeApi):
    """期货模拟交易"""

    def __init__(self,event_bus):
        """构造函数"""
        super(YKPaperTradeApi,self).__init__(event_bus)
        ...
        self.__user_token = Environment.get_instance().config.acct_info['user_token']

        cfg = websocketCfg()
        cfg.address = Environment.get_instance().get_yktrademsgaddr()
    ...
    def Init(self):
        ...
        if not self.__wsClient.Connect():
            logger.error("virtual trade connect failed!")
            return False
    ...
    def __AuotSubscribe(self):
        """自动订阅"""
        try:
            logger.debug("connected，start subscribe...")
            if not self.__Subscribe():
                logger.error("virtual trade subscribe failed!");
            logger.debug("virtual trade subscribe success!")
        except Exception as e:
            logger.exception(e)

    def __Subscribe(self):
        id = uuid.uuid1().hex
        req = {"RequestType":RequestType.Subscribe,"ReqID":id,"StrategyId":self.__strategy_id,"UserToken":self.__user_token}
        message = json.dumps(req,ensure_ascii=False)
        if not self.__Send(id,message):
            logger.error("Subscribe virtualtrade send fail:{0}".format(message))
            return False
        logger.info("Subscribe virtualtrade succeed:{0}".format(message))
        return True
    ...
    def SendOrder(self,symbol, exchange, orderSide, price, quantity, orderType, offset):
        ...
        params = {'symbol':symbol,'exchange':int(exchange),'orderSide':orderSide.name,'price':price,'quantity':quantity,'orderType':int(orderType),'offset':int(offset),'userToken':self.__user_token}
        logger.info("begin sendorder，param:{0}".format(json.dumps(params,ensure_ascii=False)))

        url = Environment.get_instance().get_ykapiurl('sendorder.ashx')
        req = self.__post_req_crypt(params)
        resp = httpFormPost(url, req)
        deResp = decrypt(resp)
        logger.info("sendorder complated，param:{0},resp:{1}".format(json.dumps(params,ensure_ascii=False),deResp))
        if not deResp:
            logger.error("response is empty:{0},{1}".format(url,json.dumps(params,ensure_ascii=False)))
            return None
        js = json.loads(deResp,encoding='utf-8')
    ...
    def CancelOrder(self,order):
        ...
        params = {'orderId':order.order_id,'userToken':self.__user_token}
        logger.info("begin cancelorder，param:{0}".format(json.dumps(params,ensure_ascii=False)))

        url = Environment.get_instance().get_ykapiurl('cancelorder.ashx')
        req = self.__post_req_crypt(params)
        resp = httpFormPost(url,req)
        deResp = decrypt(resp)
        if not deResp:
        ...
        js = json.loads(deResp,encoding='utf-8')
        if js.get('error_no') != 0:
            logger.error("cancel order failed:{0},{1},{2}".format(url,json.dumps(params,ensure_ascii=False),deResp))
    ...
    def GetAccount(self):
        ...
        url = Environment.get_instance().get_ykapiurl('PaperTrade/GetAsset')
        params = {'userToken':self.__user_token}
        req = self.__post_req_crypt(params)
        resp = httpFormPost(url,req)
        deResp = decrypt(resp)
        if not deResp:
        ...
        js = json.loads(deResp,encoding='utf-8')
        if js.get('error_no') != 0:
            logger.error("failed:{0},{1},{2}".format(url,json.dumps(params,ensure_ascii=False),deResp))
            return None
        resData = js['data']
    ...
    def GetOrders(self):
        ...
        url = Environment.get_instance().get_ykapiurl('PaperTrade/GetOrders')
        params = {'userToken':self.__user_token}
        req = self.__post_req_crypt(params)
        resp = httpFormPost(url,req)
        deResp = decrypt(resp)
    ...
    def GetPositions(self):
        ...
        url = Environment.get_instance().get_ykapiurl('PaperTrade/GetPositions')
        params = {'userToken':self.__user_token}
        req = self.__post_req_crypt(params)
        resp = httpFormPost(url,req)
        deResp = decrypt(resp)
    ...
    def GetTrades(self):
        ...
        url = Environment.get_instance().get_ykapiurl('PaperTrade/GetExecutions')
        params = {'userToken':self.__user_token}
        req = self.__post_req_crypt(params)
        resp = httpFormPost(url,req)
        deResp = decrypt(resp)
    ...
    def __post_req_crypt(self,params):
        reqJson = json.dumps(params,ensure_ascii=False)
        en = f'param={encrypt(reqJson)}'
        return en
