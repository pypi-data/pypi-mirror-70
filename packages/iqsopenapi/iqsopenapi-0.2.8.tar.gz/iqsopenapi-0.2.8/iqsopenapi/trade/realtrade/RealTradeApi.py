...
class RealTradeApi(ITradeApi):
    ...
    def __init__(self,event_bus):
        ...
        acct_info = Environment.get_instance().config.acct_info
        # 设置策略id
        self.__strategy_id = acct_info['strategy_id']

        self.__user_info = self.__get_user(acct_info) 

        # 保存user变量
        self.__api_core.SetUserInfo(self.__user_info)       

    def __get_user(self,acct_info):
        # 初始化user变量
        user = UserInfo()
        crypt =('crypt' in acct_info) and acct_info['crypt']
        aid = acct_info['account']
        psd = acct_info['password']
        if crypt:
            ...
            user.AccountID = aid
            user.PassWord = psd
        user.BrokerType = acct_info['broker_type']
        user.CounterID = acct_info['comp_counter']
        return user
    ...
    def Init(self):
        ...
        counterInfo = self.__api_core.GetCounterInfo(self.__user_info.BrokerType)
        counters = list(filter(lambda x:x.CounterId == self.__user_info.CounterID, counterInfo.CounterList))
        if(None ==  counters or len(counters) == 0):
            raise Exception("没有对应的柜台id")
        counter = counters[0]
        counterApi = self.__api_core.GetCounterApi(counter)

        # 设置user变量的柜台信息
        self.__user_info.SupInfo.TradeCounter = counter.TradeCounter
        self.__user_info.ApiInfoId = counterApi.ApiInfoId
        self.__user_info.CounterAddr = counterApi.ApiAddr

        self.__login()
        Timer(30, self.__auto_relogin).start()
        ...
        self.__subscriber = PushQuoteSubscriber(cfg, self.__strategy_id, self.__user_info)
        self.__subscriber.Connect()
    ...
    def CancelOrder(self,order):
        ...
        self.__wirteInfo(f'cancelorder complated，param:{json.dumps(params,ensure_ascii=False)},resp:{json.dumps(model,ensure_ascii=False)}')
    ...
    def __auto_relogin(self):
        ...
        try:
            model = self.__api_core.GetAccountFund()
            if(model == None or model.ErrorNo == 1):
                self.__login()
    ...
    def __login(self):
        ...
        try:
            self.__wirteInfo('登录!')
            login = self.__api_core.AccountLogin(self.__user_info)
            if (None ==  login) :
                self.__wirteError("尝试登录失败!!!")
                return
            self.__user_info.TradeToken = login.TradeToken
            self.__wirteInfo('登录成功!')
        except Exception as e:
            logger.exception(e)        
