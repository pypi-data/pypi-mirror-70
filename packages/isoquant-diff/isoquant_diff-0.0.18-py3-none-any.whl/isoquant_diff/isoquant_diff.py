...
def diff_config(file1,file2,csv,report_unique=True, path_input):
...
def main():
	...
	ap.add_argument("-p", "--path_input", action="store_true", required=False, 
					help="input in comandprompt - for MacOS (easygui not working)")
		...
		if validate_file(file1) == False:
			...
			if file2 == None:
				...
				if validate_file(file2) == False:
					...
					path = diff_config(file1,file2,csv,report_unique,path_input)
