...
def _bpmax(t, NN, n, m, mu, modeamplist, tp):
    ip = np.r_[1:len(modeamplist) + 1][:, None]
    res = modeamplist[:, None] * np.cos(pi * ip * n / NN) * np.cos(pi * ip * m / NN) * (1 - np.exp(-t / (tp[:, None])))
    return res.sum(axis=0)


def _bpmaxinf(NN, n, m, mu, modeamplist):
    ip = np.r_[1:len(modeamplist) + 1]
    res = modeamplist * np.cos(pi * ip * n / NN) * np.cos(pi * ip * m / NN)
    return res


def _Bn(t, NN, l, mu, m, modeamplist, tp):
    bn = [((abs(n - m) ** (2 * mu) * l ** 2) * np.ones_like(t) + _bpmax(t, NN, n, m, mu, modeamplist, tp))
          for n in range(1, NN + 1)]
    return bn


def _Bninf(NN, l, mu, m, modeamplist):
    bn = [((abs(n - m) ** (2 * mu) * l ** 2) + _bpmaxinf(NN, n, m, mu, modeamplist))
          for n in range(1, NN + 1)]
    return bn


def _Bmn(t, NN, l, mu, modeamplist, tp):
    # parallel to speedup
    bmn = parallel.doForList(_Bn, looplist=range(1, NN + 1), loopover='m',
                             t=t, NN=NN, l=l, mu=mu, modeamplist=modeamplist, tp=tp)
    return np.array([b for bn in bmn for b in bn])


def _Bmninf(NN, l, mu, modeamplist):
    # parallel to speedup
    bmn = parallel.doForList(_Bninf, looplist=range(1, NN + 1), loopover='m', output=False,
                             NN=NN, l=l, mu=mu, modeamplist=modeamplist)
    return np.array([b for bn in bmn for b in bn])
...
def finiteZimm(t, q, NN=None, pmax=None, l=None, Dcm=None, Dcmfkt=None, tintern=0., mu=0.5, viscosity=1.,
    ...
    -------
     ...
     dataList : for multiple q
      ...
      - .modecontribution  :math:`a_p`of modes i in sequence as in PRL 71, 4158 equ (3)
      - .Re
    ...
    (excluded volume or swollen chain). The intermediate scattering function :math:`S(q,t)/S(q,0)` is
    ...
    if useFortran :
        ...
        BNM = _Bmn(t, NN, l, mu, modeamplist, tzp)
        # same only for t=inf but separate modes
        BNMmodes = _Bmninf(NN, l, mu, modeamplist)
...
def finiteRouse(t, q, NN=None, pmax=None, l=None, frict=None, Dcm=None, Wl4=None, Dcmfkt=None, tintern=0.,
    ...
    Notes
    -----
    The Rouse model for the intermediate scattering function :math:`S(q,t)/S(q,0)` is [1]_ [2]_ :
    ...
    ::
     ...
     zz1 = js.dynamic.finiteRouse(t=t, q=q, NN=124, l=l, Dcm=0.179, tintern=0., Temp=273 + 60)
    ...
    if useFortran :
        ...
        RNM = _Bmn(t, NN, l, 0.5, modeamplist, trp)
        # same only for t=inf but separate modes
        RNMmodes = _Bmninf(NN, l, 0.5, modeamplist)
