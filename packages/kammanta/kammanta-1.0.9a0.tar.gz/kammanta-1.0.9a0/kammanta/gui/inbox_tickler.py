...
class InboxTicklerMain(QtWidgets.QWidget):
    def __init__(self):
        ...
        kammanta.my_glob.FswSingleton.get().addPath(inbox_dir_path_str)
        tickler_dir_path_str = kammanta.model.tickler_files.get_path()
        kammanta.my_glob.FswSingleton.get().addPath(tickler_dir_path_str)
    ...
    def on_add_new_inbox_item_clicked(self):
        if self.input_widget.choice_qbg.checkedId() == FILE_BTN_ID_INT:
            ...
            dest_dir_path_str = kammanta.my_glob.get_path(kammanta.my_glob.INBOX_DIR_STR)
            if self.input_widget.move_qrb.isChecked():
                kammanta.my_glob.move_fd(source_path_str, dest_dir_path_str)
            elif self.input_widget.copy_qrb.isChecked():
                kammanta.my_glob.copy_fd(source_path_str, dest_dir_path_str)
    ...
    def on_add_new_tickler_item_clicked(self):
        if self.input_widget.choice_qbg.checkedId() == FILE_BTN_ID_INT:
            ...
            kammanta.my_glob.add_tickler_file(datetime_str, source_path_str, self.input_widget.move_qrb.isChecked())
            ...
            self.input_widget.tickler_note_qte.clear()
            datetime_str = self.calendar_widget.get_datetime_string()
            kammanta.my_glob.add_tickler_note(datetime_str, contents_str)
