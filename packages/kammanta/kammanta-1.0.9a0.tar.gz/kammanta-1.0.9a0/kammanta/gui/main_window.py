...
import kammanta.gui.nas_prjs
import kammanta.gui.inbox_tickler
import kammanta.gui.focus_direction
import kammanta.gui.reference_search
import kammanta.gui.contacts_agendas
...
class MyMainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.setWindowIcon(QtGui.QIcon(kammanta.my_glob.get_icon_rel_path()))
        ...
        self.showMaximized()
        self.setWindowTitle(kammanta.my_glob.APPLICATION_TITLE_STR)

        # Widgets inside tabs

        self.main_qsw_w0 = QtWidgets.QStackedWidget()
        self.setCentralWidget(self.main_qsw_w0)

        self.main_qtw_w1 = QtWidgets.QTabWidget()
        ...
        self.inbox_tickler = kammanta.gui.inbox_tickler.InboxTicklerMain()
        self.main_qtw_w1.addTab(self.inbox_tickler, kammanta.gui.inbox_tickler.TITLE_STR)

        self.na_prj_w2 = kammanta.gui.nas_prjs.NAsPrjs()
        self.main_qtw_w1.addTab(self.na_prj_w2, kammanta.gui.nas_prjs.TITLE_STR)
        self.na_prj_w2.focus_active_signal.connect(self.on_focus_active)

        self.contacts_agendas = kammanta.gui.contacts_agendas.ContactsAgendas()
        self.main_qtw_w1.addTab(self.contacts_agendas, kammanta.gui.contacts_agendas.TITLE_STR)

        self.focus_direction = kammanta.gui.focus_direction.FocusDirection()
        self.main_qtw_w1.addTab(self.focus_direction, kammanta.gui.focus_direction.TITLE_STR)
        ...
        self.reference_search = kammanta.gui.reference_search.ReferenceSearch()
        self.main_qtw_w1.addTab(self.reference_search, kammanta.gui.reference_search.TITLE_STR)
        ...
        self.ext_email_qpb.setStyleSheet(kammanta.my_glob.EXT_BTN_STYLE_SHEET_STR)
        ...
        self.ext_cal_qpb.setStyleSheet(kammanta.my_glob.EXT_BTN_STYLE_SHEET_STR)
        ...
        self.search_container = kammanta.gui.reference_search.SearchDockContainer()
        ...
        self.inbox_dock = ClosableDockWidget(kammanta.gui.inbox_tickler.INBOX_TITLE_STR)
        ...
        self.inbox_container = kammanta.gui.inbox_tickler.InboxDock()
        ...
        self.tray_inbox_and_tickler_action = QtWidgets.QAction(kammanta.gui.inbox_tickler.TITLE_STR)
        ...
        self.tray_naprj_action = QtWidgets.QAction(kammanta.gui.nas_prjs.TITLE_STR)
        ...
        self.tray_ca_action = QtWidgets.QAction(kammanta.gui.contacts_agendas.TITLE_STR)
        self.tray_ca_action.triggered.connect(self.on_tray_ca_triggered)
        self.tray_menu.addAction(self.tray_ca_action)

        self.tray_fd_action = QtWidgets.QAction(kammanta.gui.focus_direction.TITLE_STR)
        self.tray_fd_action.triggered.connect(self.on_tray_fd_triggered)
        self.tray_menu.addAction(self.tray_fd_action)

        self.tray_rs_action = QtWidgets.QAction(kammanta.gui.reference_search.TITLE_STR)
        ...
        fsw = kammanta.my_glob.FswSingleton.get()
    ...
    def on_focus_active(self, i_status: bool):
        self.update_tray_icon(i_status)

    def update_tray_icon(self, i_focus: bool):
        pixmap = QtGui.QPixmap(kammanta.my_glob.get_icon_rel_path())
        self.tray_icon.activated.connect(self.on_tray_activated)
        if i_focus:
    ...
    def add_fnd_action(self, i_fnd_obj: kammanta.model.TextFile):
        ...
        fnd_container = kammanta.gui.focus_direction.FDArea(i_fnd_obj, True)
    ...
    def on_show_trash_triggered(self):
        trash_dir_path_str = kammanta.my_glob.get_path(kammanta.my_glob.TRASH_DIR_STR)
        kammanta.my_glob.launch_string(trash_dir_path_str)
    ...
    def on_clock_signal_triggered(self, i_new_time_string: str):
        self.tray_icon.setToolTip(i_new_time_string)
        # -Please note: tray_icon (not tray_menu)

    """
    def update_tooltip(self, i_new_time_string: str):
        now_pydt = datetime.datetime.now()
        now_pydt_str = now_pydt.strftime("%X")
        self.tray_menu.setToolTipsVisible(True)
        self.tray_menu.setToolTip(now_pydt_str)
        self.tray_icon.setToolTip(now_pydt_str)
    """

    def open_calendar(self):
        calendar_str = kammanta.my_glob.get_string_from_config(
            kammanta.my_glob.SETTINGS_SECTION_EXTERNAL_TOOLS_STR,
            kammanta.my_glob.SETTINGS_CALENDAR_STR,
            "https://calendar.google.com/calendar/r/week"
        )
        kammanta.my_glob.launch_string(calendar_str)

    def open_email_client(self):
        email_str = kammanta.my_glob.get_string_from_config(
            kammanta.my_glob.SETTINGS_SECTION_EXTERNAL_TOOLS_STR,
            kammanta.my_glob.SETTINGS_EMAIL_STR,
            "thunderbird"
        )
        if kammanta.my_glob.is_valid_command(email_str):
            kammanta.my_glob.launch_string(email_str)
    ...
    def on_open_main_dir_triggered(self):
        path_str = kammanta.my_glob.get_path()
        kammanta.my_glob.launch_string(path_str)

    def on_open_contacts_dir_triggered(self):
        path_str = kammanta.my_glob.get_path(kammanta.my_glob.CONTACTS_DIR_STR)
        kammanta.my_glob.launch_string(path_str)
    ...
    def on_take_note_triggered(self):
        ...
        #  "normalized")
        if okay_bool:
            new_note_name_str = kammanta.my_glob.get_new_note_name()
    ...
    def change_ref_files_dir(self):
        ...
        kammanta.my_glob.add_string_to_config(
            kammanta.my_glob.SETTINGS_SECTION_GENERAL_STR, kammanta.my_glob.SETTINGS_REFERECE_DIR_STR, new_dir_str
        )
    ...
    def change_user_files_dir(self):
        ...
        kammanta.my_glob.add_string_to_config(
            kammanta.my_glob.SETTINGS_SECTION_GENERAL_STR, kammanta.my_glob.SETTINGS_BASE_DIR_STR, new_dir_str
        )
        """
    ...
    def on_change_external_calendar_triggered(self):
        old_calendar_str = kammanta.my_glob.get_string_from_config(
            kammanta.my_glob.SETTINGS_SECTION_EXTERNAL_TOOLS_STR, kammanta.my_glob.SETTINGS_CALENDAR_STR, ""
        )
    ...
    def on_change_external_email_client_triggered(self):
        old_email_client_str = kammanta.my_glob.get_string_from_config(
            kammanta.my_glob.SETTINGS_SECTION_EXTERNAL_TOOLS_STR, kammanta.my_glob.SETTINGS_EMAIL_STR, ""
        )
    ...
    def backup_main_dir(self):
        date_str = datetime.datetime.now().strftime(kammanta.my_glob.QT_DATETIME_FORMAT_STR)
        ...
        if files_okay:
            ...
            shutil.make_archive(dest_str, "gztar", kammanta.my_glob.create_and_get_path())
