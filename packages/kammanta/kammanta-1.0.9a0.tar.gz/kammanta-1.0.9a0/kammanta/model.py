...
def matches_project_collection_dirs(i_path: str) -> bool:
    ...
    is_someday_maybe_dir = basename_str.lower() == kammanta.my_glob.SOMEDAY_MAYBE_STR
...
class SharedEntity(abc.ABC):
    ...
    def __init__(self, i_collection_type: kammanta.my_glob.CollTypeEnum, i_rel_path_strlist: [str]):
    ...
    def get_collection_type(self) -> kammanta.my_glob.CollTypeEnum:
    ...
    def get_path(self) -> str:
        path_str = kammanta.my_glob.create_and_get_path(*self.rel_path_strlist)
        # -is it better to just give the path here, and not create it?
        return path_str

    @abc.abstractmethod
    def get_type(self) -> kammanta.my_glob.TypeEnum:
        pass
...
class LineInFile(SharedEntity):
    ...
    def __init__(self, i_collection_type: kammanta.my_glob.CollTypeEnum, i_rel_path: [str], i_row_nr: int):
    ...
    def get_type(self) -> kammanta.my_glob.TypeEnum:
        return kammanta.my_glob.TypeEnum.line
    ...
    def get_core_name(self):
        core_name_str = kammanta.my_glob.remove_delimiter_and_after(self.get_name(), kammanta.my_glob.SUPPORT_DELIMITER_STR)
        core_name_str = kammanta.my_glob.remove_prefix(core_name_str, kammanta.my_glob.COMPL_PREF_STR)
        return core_name_str

    def set_core_name(self, i_new_display_name: str) -> None:
        is_completed_str = ""
        if self.is_completed():
            is_completed_str = kammanta.my_glob.COMPL_PREF_STR
        support_str = ""
        support_path_str = self.get_support_path()
        if support_path_str:
            support_str = kammanta.my_glob.SUPPORT_DELIMITER_STR + support_path_str
    ...
    def get_support_path(self) -> str:
        ...
        line_parts_list = entire_line_str.split(kammanta.my_glob.SUPPORT_DELIMITER_STR)
    ...
    def set_support_path(self, i_new_support_path: str) -> None:
        entire_line_str = self.get_name()
        delimeter_and_after_removed_str = kammanta.my_glob.remove_delimiter_and_after(
            entire_line_str, kammanta.my_glob.SUPPORT_DELIMITER_STR
        )
        new_line = delimeter_and_after_removed_str + kammanta.my_glob.SUPPORT_DELIMITER_STR + i_new_support_path.strip()
        self.set_name(new_line)

    def is_completed(self) -> bool:
        if self.get_name().startswith(kammanta.my_glob.COMPL_PREF_STR):
            return True
        return False
    ...
    def set_completed(self, i_new_status: bool) -> None:
        ...
        if not self.is_completed() and i_new_status:
            new_line_str = kammanta.my_glob.COMPL_PREF_STR + prev_line_str
        elif self.is_completed() and not i_new_status:
            new_line_str = kammanta.my_glob.remove_prefix(prev_line_str, kammanta.my_glob.COMPL_PREF_STR)
        if new_line_str:
...
class DirOrFile(SharedEntity):
    # Abstract class, overridden
    def __init__(self, i_collection_type: kammanta.my_glob.CollTypeEnum, i_rel_path: [str]):
        super().__init__(i_collection_type, i_rel_path)
    ...
    def get_core_name(self):
        core_name_str = kammanta.my_glob.remove_any_dot_suffix(self.get_name())
        core_name_str = kammanta.my_glob.remove_prefix(core_name_str, kammanta.my_glob.COMPL_PREF_STR)
        core_name_str = kammanta.my_glob.remove_prefix(core_name_str, kammanta.my_glob.FAV_PREF_STR)
    ...
    def set_completed(self, i_new_status: bool) -> None:
        ...
        if i_new_status:
            new_fod_name_str = kammanta.my_glob.COMPL_PREF_STR + prev_name_str
        else:
            new_fod_name_str = kammanta.my_glob.remove_prefix(prev_name_str, kammanta.my_glob.COMPL_PREF_STR)
    ...
    def get_last_edit_time(self) -> int:
        path_str = self.get_path()
        (_, mtime_secs_int) = kammanta.my_glob.get_nr_items_and_last_mod_time(path_str)
    ...
    def get_reminder_time(self) -> typing.Optional[datetime.datetime]:
        ...
        try:
            ret_dt = datetime.datetime.strptime(datetime_str, kammanta.my_glob.PY_DATETIME_FILENAME_FORMAT_STR)
        except ValueError:
            try:
                ret_dt = datetime.datetime.strptime(datetime_str, kammanta.my_glob.PY_DATE_FILENAME_FORMAT_STR)
            except ValueError:
                return None
    ...
    def set_reminder_time(self, i_new_reminder_time: datetime.datetime) -> None:
        ...
        new_name_str = kammanta.my_glob.remove_datetime_prefix(new_name_str)
        if self.get_reminder_time() is not None:
            new_name_list = new_name_str.split("_")
            new_name_str = "_".join(new_name_list[1:])
        datetime_str = i_new_reminder_time.strftime(kammanta.my_glob.PY_DATETIME_FILENAME_FORMAT_STR)
...
class Directory(DirOrFile):
    # Concrete class, not overridden
    def __init__(
            self, i_collection_type: kammanta.my_glob.CollTypeEnum, i_rel_path: [str],
    ...
    # overridden
    def get_type(self) -> kammanta.my_glob.TypeEnum:
        return kammanta.my_glob.TypeEnum.dir
    ...
    def move_active_granditem(self, i_dest_coll_id: str):
        ...
        if source_item.get_type() == kammanta.my_glob.TypeEnum.line:
            ...
            kammanta.my_glob.move_fd(source_item.get_path(), dest_coll.get_path())
    ...
    def delete_item(self, i_id: str) -> None:
        ...
        kammanta.my_glob.remove_fd(to_remove_obj.get_path())
    ...
    def get_item(self, i_name: str):
        fd_obj = None
        fd_path_str = kammanta.my_glob.get_path(self.get_path(), i_name)
        if self.matching_func_first(fd_path_str):
            if os.path.isdir(fd_path_str):
                ...
                class_type = File
                fd_type_enum = kammanta.my_glob.get_type(fd_path_str)
                if fd_type_enum == kammanta.my_glob.TypeEnum.note_file:
                    class_type = NoteFile
                elif fd_type_enum == kammanta.my_glob.TypeEnum.text_file:
                    class_type = TextFile
                elif fd_type_enum == kammanta.my_glob.TypeEnum.image_file:
                    class_type = ImageFile
                elif fd_type_enum == kammanta.my_glob.TypeEnum.desktop_file:
    ...
    def get_all_items(self, i_sort_by_name: bool = True):
        ...
        for fd_name_str in dir_content_list:
            fd_path_str = kammanta.my_glob.get_path(self.get_path(), fd_name_str)
            if self.matching_func_first(fd_path_str):
                if os.path.isdir(fd_path_str):
                ...
                kammanta.my_glob.FswSingleton.get().addPath(fd_path_str)
    ...
    def add_new_item(self, i_name: str):
        ...
        kammanta.my_glob.create_and_get_path(new_path_str)
    ...
    def add_new_note(self, i_text_to_add: str):
        base_file_name_str = "note_" + datetime.datetime.now().strftime(kammanta.my_glob.DATE_FILENAME_FORMAT_STR)

        counter_int = 1
        file_name_str = base_file_name_str + "_" + str(counter_int) + ".txt"
        file_path_str = kammanta.my_glob.get_path(self.get_path(), file_name_str)
        while os.path.exists(file_path_str) and counter_int < 1000:
            counter_int += 1
            file_name_str = base_file_name_str + "_" + str(counter_int) + ".txt"
            file_path_str = kammanta.my_glob.get_path(kammanta.my_glob.INBOX_DIR_STR, file_name_str)
    ...
    def _create_empty_text_file_from_old_path(self, i_id: str):
        ...
        new_text_file_path_str = kammanta.my_glob.remove_any_dot_suffix(old_path_str) + ".txt"
    ...
    def _move_single_file_from_old_path(self, i_id: str, i_dest_file_path: str):
        ...
        suffix_str = kammanta.my_glob.get_dsuffix(single_file_path_str)
        if not suffix_str:
            if kammanta.my_glob.is_text(single_file_path_str):
                pass
            else:
                suffix_str = "txt"
        if not suffix_str:
            suffix_str = "." + suffix_str
        dest_file_path_str = kammanta.my_glob.remove_any_dot_suffix(i_dest_file_path) + "." + suffix_str
    ...
    def convert_item_to_file(self, i_id: str) -> bool:
        ...
        if item_type_enum == kammanta.my_glob.TypeEnum.dir:
            ...
            if nr_of_support_files_int == 0:
                ...
                pass
        elif item_type_enum == kammanta.my_glob.TypeEnum.desktop_file:
            old_support_path_str = item_to_convert.get_support_path()
            old_support_type_enum = kammanta.my_glob.get_type(old_support_path_str)
            if old_support_type_enum == kammanta.my_glob.TypeEnum.dir:
                all_support_files_list = os.listdir(old_support_path_str)
                nr_of_support_files_int = len(all_support_files_list)
                ...
                if nr_of_support_files_int == 0:
                    ...
                    return False
            elif old_support_type_enum in kammanta.my_glob.any_file_enumlist:
                self._move_single_file_from_old_path(i_id, dest_file_path_str)
                return True
            else:
                pass
    ...
    def _create_desktop_file(self, i_id: str, i_new_support_dest_dir_path: str):
        ...
        link_file_name_str = kammanta.my_glob.remove_any_dot_suffix(link_file_name_str) + ".desktop"
    ...
    def convert_item_to_link(self, i_id: str, i_new_support_dest_dir_path: str) -> bool:
        ...
        if item_type_enum == kammanta.my_glob.TypeEnum.dir:
            ...
            return False
        elif item_type_enum in kammanta.my_glob.any_file_enumlist:
            """
    ...
    def convert_item_to_dir(self, i_id: str) -> bool:
        ...
        * or the file is another file (ex: txt or img), this is handled here
        ...
        new_path_dir_str = kammanta.my_glob.remove_any_dot_suffix(old_path_file_str)
        os.mkdir(new_path_dir_str)

        # Moving file(s)

        item_type_enum = item_to_convert.get_type()
        if item_type_enum == kammanta.my_glob.TypeEnum.desktop_file:
            if old_support_type_enum == kammanta.my_glob.TypeEnum.dir:
...
class File(DirOrFile):
    ...
    # overridden
    def get_type(self) -> kammanta.my_glob.TypeEnum:
        return kammanta.my_glob.TypeEnum.file

    def get_nr_of_files(self) -> int:
        path_str = self.get_path()
        if path_str.endswith(".desktop"):
            support_path_str = self.get_support_path()
            if kammanta.my_glob.get_type(support_path_str) == kammanta.my_glob.TypeEnum.dir:
                dir_strlist = os.listdir(support_path_str)
                return len(dir_strlist)
            elif kammanta.my_glob.get_type(support_path_str) == kammanta.my_glob.TypeEnum.file:
                return 1
            elif kammanta.my_glob.get_type(support_path_str) in (
                    kammanta.my_glob.TypeEnum.web_link, kammanta.my_glob.TypeEnum.command
            ):
                return 0
        else:
            return 1
    ...
    def update_desktop_file(self, i_support_path: str=None):
        assert(self.get_type() == kammanta.my_glob.TypeEnum.desktop_file)
        if i_support_path is None:
            support_path_str = self.get_support_path()
        else:
            support_path_str = i_support_path
        if kammanta.my_glob.get_type(support_path_str) == kammanta.my_glob.TypeEnum.command:
    ...
    def set_support_path(self, i_support_path: str):
        if self.get_type() == kammanta.my_glob.TypeEnum.desktop_file:
        ...
        elif self.get_type == kammanta.my_glob.TypeEnum.text_file:
            pass
        else:
            pass
    ...
    def get_item(self, i_row_id: str) -> LineInFile:
        for row_obj in self.get_all_items():
            if row_obj.file_row_nr_int == int(i_row_id):
                return row_obj
        logging.warning("No next action with i_row_nr " + i_row_id + "was found")
    ...
    def clear_completed(self):
        ...
        with open(na_context_file_path_str, "r+") as file:
            ...
            file.seek(0)
            for (line_number_int, line_str) in enumerate(line_list):
                if not line_str.startswith(kammanta.my_glob.COMPL_PREF_STR):
                    file.write(line_str)
            file.truncate()
        """
...
class ImageFile(File):
    ...
    # overridden
    def get_type(self) -> kammanta.my_glob.TypeEnum:
        return kammanta.my_glob.TypeEnum.image_file
...
class DesktopFile(File):
    ...
    # overridden
    def get_type(self) -> kammanta.my_glob.TypeEnum:
        return kammanta.my_glob.TypeEnum.desktop_file
...
class TextFile(File):
    ...
    # overridden
    def get_type(self) -> kammanta.my_glob.TypeEnum:
        return kammanta.my_glob.TypeEnum.text_file
    ...
    def is_favorite(self) -> bool:
        if self.get_name().startswith(kammanta.my_glob.FAV_PREF_STR):
            return True
        return False
    ...
    def set_favorite(self, i_new_status: bool) -> None:
        ...
        if not self.is_favorite() and i_new_status:
            new_name_str = kammanta.my_glob.FAV_PREF_STR + old_name_str
            self.set_name(new_name_str)
        elif self.is_favorite() and not i_new_status:
            new_name_str = kammanta.my_glob.remove_prefix(old_name_str, kammanta.my_glob.FAV_PREF_STR)
            self.set_name(new_name_str)
...
class NoteFile(TextFile):
    ...
    # overridden
    def get_type(self) -> kammanta.my_glob.TypeEnum:
        return kammanta.my_glob.TypeEnum.note_file


aoi_obj = TextFile(
    kammanta.my_glob.CollTypeEnum.areas_of_interest,
    [FOCUS_DIRECTION_DIR_NAME_STR, AOI_FILE_NAME_STR]
)
go_obj = TextFile(
    kammanta.my_glob.CollTypeEnum.goals_and_objectives,
    [FOCUS_DIRECTION_DIR_NAME_STR, GO_FILE_NAME_STR]
)
vision_obj = TextFile(
    kammanta.my_glob.CollTypeEnum.vision,
    [FOCUS_DIRECTION_DIR_NAME_STR, VISION_FILE_NAME_STR]
)
pp_obj = TextFile(
    kammanta.my_glob.CollTypeEnum.purpose_and_principles,
    [FOCUS_DIRECTION_DIR_NAME_STR, PP_FILE_NAME_STR]
)

inbox_dir = Directory(
    kammanta.my_glob.CollTypeEnum.inbox,
    [kammanta.my_glob.INBOX_DIR_STR], matches_all, None
)
tickler_files = Directory(
    kammanta.my_glob.CollTypeEnum.tickler,
    [kammanta.my_glob.TICKLER_DIR_STR], matches_all, None
)
na_files = Directory(
    kammanta.my_glob.CollTypeEnum.next_actions,
    [], matches_txt_files, None
)
prj_fds = Directory(
    kammanta.my_glob.CollTypeEnum.projects,
    [], matches_project_collection_dirs, matches_all
)
contacts_dir = Directory(
    kammanta.my_glob.CollTypeEnum.contacts,
    [kammanta.my_glob.CONTACTS_DIR_STR], matches_txt_files, None
)
agenda_files = Directory(
    kammanta.my_glob.CollTypeEnum.agendas,
    [kammanta.my_glob.AGENDAS_DIR_STR], matches_txt_files, None
)
