...
class ChecklistWidget(QtWidgets.QWidget):
    ...
    def __init__(self, i_collection_ptr):
        ...
        self.title_qll.setFont(kammanta.my_glob.get_title_font())
        ...
        if self.coll_ptr.get_collection_type() == kammanta.my_glob.CollTypeEnum.projects:
        ...
        if (self.coll_ptr.get_path() == kammanta.my_glob.get_path()
        or self.coll_ptr.get_name().startswith("projects")):
        ...
        elif self.coll_ptr.get_name() == kammanta.my_glob.AGENDAS_DIR_STR:
            ...
            logging.debug(kammanta.my_glob.get_path())
    ...
    def on_add_new_item_clicked(self):
        ...
        if new_obj:
            ...
            if support_path_str:
                kammanta.my_glob.launch_string(support_path_str)
    ...
    def on_add_new_coll_action_triggered(self):
        ...
        if self.coll_ptr.get_collection_type() == kammanta.my_glob.CollTypeEnum.next_actions:
            result_str = kammanta.my_glob.add_suffix(result_str, kammanta.my_glob.TEXT_SUFFIX_STR)
        elif self.coll_ptr.get_collection_type() == kammanta.my_glob.CollTypeEnum.agendas:
            result_str = kammanta.my_glob.add_suffix(result_str, kammanta.my_glob.TEXT_SUFFIX_STR)
        elif self.coll_ptr.get_collection_type() == kammanta.my_glob.CollTypeEnum.projects:
            result_str = kammanta.my_glob.add_prefix_to_basename(result_str, "projects-")
        if result_bool:
    ...
    def open_coll_action_triggered(self):
        ...
        kammanta.my_glob.launch_string(file_path_str)
    ...
    def open_item_support(self):
        ...
        if sp_str:
            try:
                kammanta.my_glob.launch_string(sp_str)
            except Exception:
    ...
    def update_item_support_path(self):
        ...
        if old_support_path_with_name_str:
            ...
            if os.path.isfile(old_support_path_with_name_str):
                ...
                default_path_str = kammanta.my_glob.get_path()
            ...
            if self.coll_ptr.get_collection_type() == kammanta.my_glob.CollTypeEnum.next_actions:
                initial_enum = kammanta.widgets.path_sel_dlg.PathSelectionEnum.file
                default_path_str = kammanta.my_glob.create_and_get_path(kammanta.model.NA_DEFAULT_SUPPORT_DIR)
                ...
                for char_str in active_row.get_core_name():
                    if char_str.isalnum():
                        ...
                        default_name_str += "_"
                default_name_str += kammanta.my_glob.TEXT_SUFFIX_STR
    ...
    def on_add_new_coll_clicked(self):
        ...
        new_file_name_str = kammanta.my_glob.add_suffix(new_core_name_str, kammanta.my_glob.TEXT_SUFFIX_STR)
    ...
    def on_item_list_current_row_changed(self, i_current_list_row: int):
        ...
        if current_lwi is not None:
            ...
            if active_item.get_type() == kammanta.my_glob.TypeEnum.text_file:
                self.convert_to_directory_action.setEnabled(True)
                self.convert_to_desktop_link_action.setEnabled(True)
            elif active_item.get_type() == kammanta.my_glob.TypeEnum.dir:
                self.convert_to_text_file_action.setEnabled(True)
                self.convert_to_desktop_link_action.setEnabled(True)
            elif active_item.get_type() == kammanta.my_glob.TypeEnum.desktop_file:
                self.convert_to_text_file_action.setEnabled(True)
                self.convert_to_directory_action.setEnabled(True)
                self.set_support_path_qpb.setEnabled(True)
            elif active_item.get_type() == kammanta.my_glob.TypeEnum.line:
                self.set_support_path_qpb.setEnabled(True)
    ...
    def _get_dir_support_info(i_support_path: str) -> str:
        (nr_of_files_int, latest_edit_time_ts_int) = kammanta.my_glob.get_nr_items_and_last_mod_time(i_support_path)
    ...
    def update_gui_item_list_and_ids(self):
        ...
        for item in list_items:
            ...
            if active_list.get_type() == kammanta.my_glob.TypeEnum.dir:
                if item_type_enum == kammanta.my_glob.TypeEnum.dir:
                    ...
                    suffix_str = kammanta.my_glob.get_dsuffix(item.get_support_path())
            ...
            fd_support_info_str = ""
            support_path_str = item.get_support_path()
            support_type_enum = kammanta.my_glob.get_type(support_path_str)
            if support_type_enum == kammanta.my_glob.TypeEnum.command:
                support_type_str = "[cmd] "
            elif support_type_enum == kammanta.my_glob.TypeEnum.web_link:
                support_type_str = "[web] "
            elif support_type_enum == kammanta.my_glob.TypeEnum.dir:
                support_type_str = "[dir] "
                fd_support_info_str = self._get_dir_support_info(support_path_str)
            elif support_type_enum in kammanta.my_glob.any_file_enumlist:
                suffix_str = kammanta.my_glob.get_dsuffix(support_path_str)
                support_type_str = f"[{suffix_str} file] "
                fd_support_info_str = self._get_file_support_info(support_path_str)
            elif item.get_collection_type() == kammanta.my_glob.CollTypeEnum.projects:
            ...
            if kammanta.my_glob.testing_bool:
