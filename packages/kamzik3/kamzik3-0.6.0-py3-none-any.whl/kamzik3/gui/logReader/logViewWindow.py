...
class LogViewWindow(QMainWindow, Ui_MainWindow):
    sig_task_update = pyqtSignal(str, str, object)
    ...
    def load_file(self):
        ...
        if self.virtual_directory_device is None:
            ...
            if self.virtual_directory_device.get_value(ATTR_STATUS) in READY_DEVICE_STATUSES:
                ...
                if dialog.exec():
                    requested_files = self.virtual_directory_device.prepare_files(dialog.selected_files)
                    task_id = self.virtual_directory_device.sync_files(requested_files)

                    def callback(value):
                        self.sig_task_update.emit(value, task_id, dialog.selected_files)
    ...
    def slot_task_update(self, status, task_id, original_file_names):
