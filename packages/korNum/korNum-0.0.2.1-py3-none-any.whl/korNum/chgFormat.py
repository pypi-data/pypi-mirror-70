...
def judge_exist_num_text(text):
    ...
    if len(ordIdx) is not 0:
        ordResult = True
    return cardResult, ordResult, cardIdx, ordIdx
...
def kor2num(text):
    ...
    Returns:
        :param: result(list)
    """
    cardResult, ordResult, cardIdx, ordIdx = judge_exist_num_text(text)

    #if ordResult is True:
    #    text = ordFunction(text, ordIdx)
    if cardResult is True:
        text = cardFunction(text, cardIdx)
