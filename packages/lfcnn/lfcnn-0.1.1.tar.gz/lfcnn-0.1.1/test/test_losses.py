...
ALL_LOSSES = [losses.mean_absolute_error,
...
def test_huber():
    ...
    y_true = convert_to_tensor(y_true)
    y_pred = convert_to_tensor(y_pred)
    assert K.eval(loss(y_true, y_pred, delta=delta)) == 0
    assert K.eval(loss_inst.call(y_true, y_pred)) == 0
    ...
    y_true = convert_to_tensor(y_true)
    y_pred = convert_to_tensor(y_pred)
    assert K.eval(loss(y_true, y_pred, delta=delta)) == 0.25
...
def test_pseudo_huber():
    delta = 1
    loss_inst = losses.PseudoHuber(delta=delta)
    ...
    y_pred = convert_to_tensor(y_pred)
    abs = 1e-4
    assert approx(K.eval(loss(y_true, y_pred, delta=delta)), abs=1e-7) == 0
    assert approx(K.eval(loss_inst.call(y_true, y_pred)), abs=1e-7) == 0

    # Opposite true, pred
    y_true = convert_to_tensor(np.ones((10, 3, 3, 7)))
    y_pred = convert_to_tensor(np.zeros((10, 3, 3, 7)))

    assert approx(K.eval(loss(y_true, y_pred, delta=delta)), abs=abs) == 0.4142
    ...
    y_true = convert_to_tensor(y_true)
    y_pred = convert_to_tensor(y_pred)
    assert approx(K.eval(loss(y_true, y_pred, delta=delta)), abs=abs) == 0.5*0.4142
