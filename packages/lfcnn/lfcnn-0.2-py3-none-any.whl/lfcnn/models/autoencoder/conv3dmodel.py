...
class Conv3dModel(BaseModel):
    ...
    def __init__(self, L=3, **kwargs):
        ...
        Args:
            ds: Downsample factor in s-axis.

            dt: Downsample factor in t-axis.

            bw: Whether to transform input to greyscale.
    ...
    def create_model(self, inputs: List[keras.Input], augmented_shape=None) -> keras.Model:
        ...
        x = self.res_block_3d(x, num_filters=64)
        x = self.res_block_3d(x, num_filters=128, strides=(2, 2, 3))

        # Decoder
        x = self.res_block_3d_transposed(x, num_filters=128, strides=(2, 2, 3))
        x = self.res_block_3d(x, num_filters=64)
        x = self.res_block_3d(x, num_filters=64)
        ...
        x = self.final_reshape(x, name='light_field')
