...
def test_reshape_functions():
    ...
    # Test mono LF
    lf = np.random.rand(64, 9, 9, 32, 32, 1)
    labels = ["string", np.random.rand(64, 32, 32)]
    ...
    # Test RGB LF
    lf = np.random.rand(64, 9, 9, 32, 32, 3)
    labels = ["string", np.random.rand(64, 32, 32)]
    ...
    lf = np.random.rand(64, 9, 9, 32, 32, 30)
    labels = ["string", np.random.rand(64, 32, 32)]
...
def test_lf_subaperture_stream():
    ...
    lf = np.random.rand(64, 3, 3, 32, 32, 3)
    labels = []
...
def test_lf_subaperture_ch_stack():
    ...
    assert lf_r.shape == (64, 32, 32, 9, 3)
...
def test_lf_distributed():
    ...
    assert np.array_equal(lf_r[:, 8, :, :, 1], lf[:, 2, 2, :, :, 1])
    assert np.array_equal(lf_r[:, 8, :, :, 2], lf[:, 2, 2, :, :, 2])
    assert lf_r.shape == (64, 9, 32, 32, 3)
...
def test_lf_crosshair():
    ...
    for tmp in [horz, vert, diag1, diag2]:
        assert tmp.shape == (64, 3, 32, 32, 3)

    diag1_ref = np.zeros((64, 3, 32, 32, 3))
    diag1_ref[:, 0] = lf[:, 0, 0]
    diag1_ref[:, 1] = lf[:, 1, 1]
    diag1_ref[:, 2] = lf[:, 2, 2]

    diag2_ref = np.zeros((64, 3, 32, 32, 3))
    diag2_ref[:, 0] = lf[:, 2, 0]
    diag2_ref[:, 1] = lf[:, 1, 1]
    diag2_ref[:, 2] = lf[:, 0, 2]
...
def test_lf_crosshair_stacked():
    ...
    for tmp in [horz, vert, diag1, diag2]:
        assert tmp.shape == (64, 32, 32, 3*3)

    diag1_ref = np.zeros((64, 32, 32, 3*3))
    diag1_ref[:, :, :, 0:3] = lf[:, 0, 0, :]
    diag1_ref[:, :, :, 3:6] = lf[:, 1, 1, :]
    diag1_ref[:, :, :, 6:9] = lf[:, 2, 2, :]

    diag2_ref = np.zeros((64, 32, 32, 3*3))
