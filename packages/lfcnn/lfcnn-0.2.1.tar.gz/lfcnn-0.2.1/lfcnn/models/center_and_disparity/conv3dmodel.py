...
from lfcnn.layers import sample_down_3d, sample_up_2d, sample_up_3d
...
class Conv3dModel(BaseModel):
    ...
    def create_model(self, inputs: List[keras.Input], augmented_shape=None) -> keras.Model:
        ...
        downsample_strides = (2, 2, 3)
        upsample_strides_2d = (2, 2)
        upsample_strides_3d = (2, 2, 2)

        kernel_reg = keras.regularizers.l2(1e-4)

        # Encoder
        x = res_block_3d(input, 32, kernel_regularizer=None)
        ...
        # Decoder disparity
        x1 = reshape_3d_to_2d(x)
        x1 = sample_up_2d(x1, 256, strides=upsample_strides_2d, kernel_regularizer=kernel_reg)
        x1 = res_block_2d(x1, 256, kernel_regularizer=kernel_reg)
        x1 = sample_up_2d(x1, 128, strides=upsample_strides_2d, kernel_regularizer=kernel_reg)
        x1 = res_block_2d(x1, 128, kernel_regularizer=kernel_reg)
        x1 = sample_up_2d(x1, 64, strides=upsample_strides_2d, kernel_regularizer=kernel_reg)
        ...
        x2 = sample_up_3d(x, 256, strides=upsample_strides_3d, kernel_regularizer=kernel_reg)
        x2 = res_block_3d(x2, 256, kernel_regularizer=kernel_reg)
        x2 = sample_up_3d(x2, 128, strides=upsample_strides_3d, kernel_regularizer=kernel_reg)
        x2 = res_block_3d(x2, 128, kernel_regularizer=kernel_reg)
        x2 = sample_up_3d(x2, 64, strides=upsample_strides_3d, kernel_regularizer=kernel_reg)
        x2 = res_block_3d(x2, 64, kernel_regularizer=kernel_reg)
        x2 = reshape_3d_to_2d(x2)
        x2 = res_block_2d(x2, 64, kernel_regularizer=None)
        x2 = res_block_2d(x2, augmented_shape[-1], kernel_regularizer=None,
                          name='central_view')

        return keras.Model(input, [x1, x2], name="Conv3dCentralAndDisparityEstimator")
