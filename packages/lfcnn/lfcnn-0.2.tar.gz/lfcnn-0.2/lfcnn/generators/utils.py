...
def disp_batch_augment(disp_batch: any, augment: dict, aug_res: AugmentResult):
    ...
    disp_batch = np.squeeze(disp_batch)
    if augment['flip']:
...
def shape_wrapper(input_shape):
    ...
    if input is a list of list, will convert inner lists to tuples,
    if input is a list of lists, nothing happens.

    Args:
        generated_shape: input_shape
