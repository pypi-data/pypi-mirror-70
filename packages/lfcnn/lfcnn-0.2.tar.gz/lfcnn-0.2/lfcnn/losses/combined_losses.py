...
from .losses import ssim, n_ssim, ms_ssim, n_ms_ssim, pseudo_huber_loss, normalized_cosine_proximity, total_variation
...
class CenterLoss(LossFunctionWrapper):
    ...
    def __init__(self,
                 mu: float = 0,
                 gamma: float = 0,
                 power_factors: Tuple[int] = (0.5, 0.3, 0.2),
                 filter_size: int = 3,
                 k1: float = 0.03,
                 k2: float = 0.09,
                 reduction=losses_utils.ReductionV2.SUM_OVER_BATCH_SIZE,
                 name='center_loss'):
        """Central view loss function based on the pseudo Huber loss with two
        regularization terms based on the multi-scale structural similarity
...
def center_loss(y_true, y_pred, mu, gamma, power_factors, filter_size, k1, k2):
    return pseudo_huber_loss(y_true, y_pred) \
...
class DisparityLoss(LossFunctionWrapper):
    ...
    def __init__(self,
                 ...
                 reduction=losses_utils.ReductionV2.SUM_OVER_BATCH_SIZE,
                 name='disparity_loss'):
        """Disparity loss function based on the pseudo Huber loss with a
...
def disparity_loss(y_true, y_pred, mu=0):
    return pseudo_huber_loss(y_true, y_pred) + mu*total_variation(y_pred)
