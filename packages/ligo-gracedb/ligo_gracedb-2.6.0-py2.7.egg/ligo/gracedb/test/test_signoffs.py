...
def test_signoff_helper(safe_client, action, signoff_type, instrument, status,
    ...
    if action == 'create':
        ...
        assert 'body' in rf_kwargs
        assert rf_kwargs['body']['signoff_type'] == signoff_type
        assert rf_kwargs['body']['instrument'] == instrument
        assert rf_kwargs['body']['status'] == status
        assert rf_kwargs['body']['comment'] == comment
        ...
        if status:
            body_len += 1
            assert rf_kwargs['body']['status'] == status
        assert len(rf_kwargs['body']) == body_len
    else:
