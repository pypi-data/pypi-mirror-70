...
def test_add_tag(safe_client, is_event, display_name):
    ...
    if is_event:
        ...
        if display_name:
            assert len(rf_call_kwargs['body']) == 1
            assert rf_call_kwargs['body']['displayName'] == display_name
        else:
            assert rf_call_kwargs['body'] == {}
    else:
        assert rf_call_args[0] == 'POST'
        assert rf_call_kwargs['body']['name'] == tag_name
        if display_name:
            assert len(rf_call_kwargs['body']) == 2
            assert rf_call_kwargs['body']['displayName'] == display_name
        else:
            assert len(rf_call_kwargs['body']) == 1
