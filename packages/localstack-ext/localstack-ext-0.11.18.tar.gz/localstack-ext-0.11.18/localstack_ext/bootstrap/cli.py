...
def import_auth():
 try:
  ...
  raise ScbHq('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=ScbHF)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=ScbHd)
def get_params(args,names=[],mandatory=ScbHF):
 result=()
 for name in names:
  value=ScbHu
  if name in args:
  ...
  if not value and mandatory:
   raise ScbHq('Please provide %s=... parameter'%name)
  result+=(value)
 if ScbHW(result)==1:
  return result[0]
 return result
