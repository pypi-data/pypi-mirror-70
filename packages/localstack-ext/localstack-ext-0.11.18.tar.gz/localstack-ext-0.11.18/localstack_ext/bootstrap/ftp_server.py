...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':wMLqH(perm='m',auth=wMLqE,arg=wMLqE,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=wMLqE)
def do_start_ftp(port,asynchronous=wMLqE):
...
def main():
 do_start_ftp(wMLqD(sys.argv[1]),asynchronous=wMLqv)
