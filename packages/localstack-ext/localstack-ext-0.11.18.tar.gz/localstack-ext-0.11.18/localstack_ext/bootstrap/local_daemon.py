...
class FuncThread(threading.Thread):
 def __init__(self,func,params=RDTJV):
...
class RequestHandler(BaseHTTPRequestHandler):
 ...
 def handle_request(self):
  ...
  result=json.dumps(result)if RDTJF(result,RDTJP)else result
  return result
 def read_content(self):
  if RDTJC(self,'data_bytes'):
   return
  content_length=self.headers.get('Content-Length')
  self.data_bytes=self.rfile.read(RDTJU(content_length))
  self.request_json={}
  try:
...
def s3_download(request):
 ...
 target_file=os.path.join(tmp_dir,request.get('file_name')or 's3file.%s'%RDTJB(uuid.uuid4()))
...
def start_ssh_forward_proxy(options):
 ...
 host=LOCAL_BIND_ADDRESS_PATTERN.replace('*',RDTJB(RDTJe(USED_BIND_ADDRESSES)+2))
...
def create_network_interface_alias(address,interface=RDTJV):
 sudo_cmd='sudo'
 if is_mac_os():
 ...
 if is_linux():
  ...
  for interface in interfaces:
   try:
    ...
    pass
 raise RDTJb('Unable to create network interface')
def run_shell_cmd(command):
 try:
  return{'result':run_cmd(command)}
 except RDTJb as e:
  error_string=RDTJB(e)
  if RDTJF(e,subprocess.CalledProcessError):
...
def run_cmd(cmd):
 ...
 return to_str(subprocess.check_output(cmd,stderr=subprocess.STDOUT,shell=RDTJk))
def log(*args):
 print(*args)
 sys.stdout.flush()
def is_mac_os():
 try:
  out=to_str(subprocess.check_output('uname -a',shell=RDTJk))
...
def get_free_tcp_port():
 ...
 return port
def to_bytes(obj):
 return obj.encode('utf-8')if RDTJF(obj,RDTJB)else obj
def to_str(obj):
 return obj.decode('utf-8')if RDTJF(obj,RDTJa)else obj
...
def main():
 logging.basicConfig()
 daemon_type=sys.argv[1]if RDTJe(sys.argv)>1 else 'main'
...
def start_in_background():
 ...
 run(cmd,outfile=log_file,asynchronous=RDTJk)
