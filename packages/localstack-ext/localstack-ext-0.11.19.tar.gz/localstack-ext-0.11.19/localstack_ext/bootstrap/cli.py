...
def import_auth():
 try:
  ...
  raise WRSbN('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=WRSbl)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=WRSbk)
def get_params(args,names=[],mandatory=WRSbl):
 result=()
 for name in names:
  value=WRSbL
  if name in args:
  ...
  if not value and mandatory:
   raise WRSbN('Please provide %s=... parameter'%name)
  result+=(value)
 if WRSbp(result)==1:
  return result[0]
 return result
