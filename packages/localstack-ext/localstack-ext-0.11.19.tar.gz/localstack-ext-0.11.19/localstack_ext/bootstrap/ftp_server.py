...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':gMNFo(perm='m',auth=gMNFi,arg=gMNFi,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=gMNFi)
def do_start_ftp(port,asynchronous=gMNFi):
...
def main():
 do_start_ftp(gMNFa(sys.argv[1]),asynchronous=gMNFf)
