...
def import_auth():
 try:
  ...
  raise cMlJH('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=cMlJp)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=cMlJm)
def get_params(args,names=[],mandatory=cMlJp):
 result=()
 for name in names:
  value=cMlJD
  if name in args:
  ...
  if not value and mandatory:
   raise cMlJH('Please provide %s=... parameter'%name)
  result+=(value)
 if cMlJW(result)==1:
  return result[0]
 return result
