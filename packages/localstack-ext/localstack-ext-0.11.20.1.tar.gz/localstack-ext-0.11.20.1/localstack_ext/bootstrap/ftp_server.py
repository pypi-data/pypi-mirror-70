...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':XiMwE(perm='m',auth=XiMwl,arg=XiMwl,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=XiMwl)
def do_start_ftp(port,asynchronous=XiMwl):
...
def main():
 do_start_ftp(XiMwY(sys.argv[1]),asynchronous=XiMwV)
