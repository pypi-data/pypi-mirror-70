...
def import_auth():
 try:
  ...
  raise NYQRW('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=NYQRH)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=NYQRi)
def get_params(args,names=[],mandatory=NYQRH):
 result=()
 for name in names:
  value=NYQRc
  if name in args:
  ...
  if not value and mandatory:
   raise NYQRW('Please provide %s=... parameter'%name)
  result+=(value)
 if NYQRy(result)==1:
  return result[0]
 return result
