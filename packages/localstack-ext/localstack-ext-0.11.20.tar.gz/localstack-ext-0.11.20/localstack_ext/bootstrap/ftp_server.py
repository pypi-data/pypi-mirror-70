...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':PlcGj(perm='m',auth=PlcGm,arg=PlcGm,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=PlcGm)
def do_start_ftp(port,asynchronous=PlcGm):
...
def main():
 do_start_ftp(PlcGh(sys.argv[1]),asynchronous=PlcGb)
