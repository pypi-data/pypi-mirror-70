...
def should_stop(model_database,kwargs_query, atributte,atributte_equals_to):
    start_time_should_stop = time.time()
    entry = ohm2_handlers_light_utils.db_get_or_none(model_database,**kwargs_query)
    finish_time_should_stop = time.time()
    diff_time_should_stop = finish_time_should_stop - start_time_should_stop
    if entry:
        stop = entry.getattr(atributte) == atributte_equals_to
        return stop,diff_time_should_stop
    return False,diff_time_should_stop

def time_sleep_with_stop_url(total_time_sleep,model_database,kwargs_query, atributte, atributte_equals_to, lower_boundary=1, upper_boundary=10, **options):
    '''
       SE RECALCULA EL DELAY RESTANDO LOS TIEMPOS DE LA QUERY EN LA DB Y EL OPCIONAL DE ENVIAR EL TIEMPO PENDIENTE,
    ...
    for delay in delay_schedule:
        ...
        # start_time_should_stop = time.time()
        stop_sleep, diff_time_should_stop = should_stop(model_database,kwargs_query, atributte,atributte_equals_to)
        # finish_time_should_stop = time.time()
        # diff_time_should_stop = finish_time_should_stop - start_time_should_stop
