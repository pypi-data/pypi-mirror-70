import numpy as np
from matplotlib import cm
import matplotlib.pyplot as plt

plt.rcParams['font.sans-serif'] = ['SimHei']  # 用来正常显示中文标签
plt.rcParams['axes.unicode_minus'] = False  # 用来正常显示负号
from meteva.method.yes_or_no.score import pofd_hfmc, pod_hfmc
from meteva.base.tool.plot_tools import set_plot_IV
from meteva.base import IV
import matplotlib.patches as patches
import meteva
import math


def hnh(Ob, Fo, grade_count=10):
    '''
    :param Ob:
    :param Fo:
    :param grade_count:
    :return:
    '''

    grade = 1 / grade_count
    if grade_count < 1:
        print('grade_count输入错误，不能小于1')
        return
    grade_list = np.arange(0, 1, grade).tolist()
    grade_list.append(1.1)
    Ob_shape = Ob.shape
    Fo_shape = Fo.shape
    hnh_array_list = []
    Ob_shpe_list = list(Ob_shape)
    size = len(Ob_shpe_list)
    ind = -size
    Fo_Ob_index = list(Fo_shape[ind:])
    if Fo_Ob_index != Ob_shpe_list:
        print('实况数据和观测数据维度不匹配')
        return
    Ob_shpe_list.insert(0, -1)
    new_Fo_shape = tuple(Ob_shpe_list)
    new_Fo = Fo.reshape(new_Fo_shape)
    new_Fo_shape = new_Fo.shape

    for line in range(new_Fo_shape[0]):
        th_list = []
        for g in range(len(grade_list) - 1):
            index = np.where((new_Fo[line, :] >= grade_list[g]) & (new_Fo[line, :] < grade_list[g + 1]))
            ob1 = Ob[index]
            ob2 = ob1[ob1 > 0]
            th_list.append([ob1.size, ob2.size])
        hnh_array = np.array(th_list)
        hnh_array_list.append(hnh_array)
    hnh_array = np.array(hnh_array_list)
    shape = list(Fo_shape[:ind])
    shape.append(len(grade_list) - 1)
    shape.append(2)
    hnh_array = hnh_array.reshape(shape)
    return hnh_array


def reliability(Ob, Fo, grade_count=10, model_name_list=None, save_path=None, title="可靠性图"):
    '''
    :param Ob:
    :param Fo:
    :param save_path:
    :return:
    '''
    hnh_array = hnh(Ob, Fo, grade_count)

    reliability_hnh(hnh_array, grade_count, model_name_list=model_name_list, save_path=save_path, title=title)


def reliability_hnh(hnh_array, grade_count = 10, model_name_list=None, save_path=None, title="可靠性图"):
    '''
    根据中间结果计算
    :param th:
    :param save_path:
    :return:
    '''
    grade = 1 / grade_count
    if grade_count < 1:
        print('grade_count输入错误，不能小于1')
        return
    grade_list = np.arange(0, 1, grade).tolist()
    grade_list.append(1.1)
    hnh_array = hnh_array.reshape((-1, len(grade_list) - 1, 2))
    new_hnh_array_shape = hnh_array.shape
    label = []
    if model_name_list is None:
        if new_hnh_array_shape[0] == 1:
            label.append('实际预报')
        else:
            for i in range(new_hnh_array_shape[0]):
                label.append('实际预报' + str(i + 1))
    else:
        label.extend(model_name_list)

    colors_list = []
    colors = cm.get_cmap('rainbow', 128)
    for i in range(new_hnh_array_shape[0]):
        color_grade = i / new_hnh_array_shape[0]
        colors_list.append(colors(color_grade))

    for line in range(new_hnh_array_shape[0]):
        total_grade_num = hnh_array[line, :, 0]
        observed_grade_num = hnh_array[line, :, 1]
        ngrade = len(total_grade_num)
        grade = 1 / ngrade
        total_num = np.sum(total_grade_num)
        under = np.zeros_like(total_grade_num)
        under[:] = total_grade_num[:]
        under[total_grade_num == 0] = 1
        ob_rate = observed_grade_num / under
        ob_rate[total_grade_num == 0] = IV
        ob_rate_noIV = set_plot_IV(ob_rate)
        ob_rate[total_grade_num == 0] = np.nan
        index_iv = np.where(total_grade_num == 0)
        line_x = np.arange(0, 1.00, grade)
        prefect_line_y = np.arange(0, 1.00, grade)
        climate_line_y = np.ones_like(line_x) * np.sum(observed_grade_num) / total_num
        x = np.arange(grade / 2, 1, grade)

        if line == 0:
            fig = plt.figure(figsize=(6, 6.1))
            grid_plt = plt.GridSpec(5, 1, hspace=0)
            ax1 = plt.subplot(grid_plt[0:4, 0])
            plt.plot(line_x, prefect_line_y, '--', label="完美预报", color="k")
            plt.plot(line_x, climate_line_y, ':', label="无技巧预报", color="k")
        plt.plot(x, ob_rate_noIV, "--", linewidth=0.5, color="k")
        x_iv = x[index_iv[0]]
        ob_rate_noIV_iv = ob_rate_noIV[index_iv[0]]
        plt.plot(x_iv, ob_rate_noIV_iv, "x", color='k')
        plt.plot(x, ob_rate, marker=".", markersize="10", label=label[line], color=colors_list[line])

        plt.setp(ax1.get_xticklabels(), visible=False)
        plt.ylim(0.0, 1)
        plt.ylabel("实况的发生比例")
        plt.legend(loc=2)
        plt.title(title)

    for line in range(new_hnh_array_shape[0]):
        total_grade_num = hnh_array[line, :, 0]
        observed_grade_num = hnh_array[line, :, 1]
        ngrade = len(total_grade_num)
        grade = 1 / ngrade
        under = np.zeros_like(total_grade_num)
        under[:] = total_grade_num[:]
        under[total_grade_num == 0] = 1
        ob_rate = observed_grade_num / under
        ob_rate[total_grade_num == 0] = IV
        ob_rate[total_grade_num == 0] = np.nan

        x = np.arange(grade / 2, 1, grade)
        if line == 0:
            ax2 = plt.subplot(grid_plt[4, 0], sharex=ax1)
        x1 = x - 0.01 + (line + 1.5) * 0.01
        plt.bar(x1, total_grade_num, width=0.01)
        plt.ylabel("样本数")
        plt.xlim(0.0, 1.0)
        plt.xticks(np.arange(0.1, 1.01, 0.1))
        plt.xlabel("预测的概率")

    if save_path is None:
        plt.show()
    else:
        plt.savefig(save_path)
        print("检验结果已以图片形式保存至" + save_path)
    plt.close()


def roc(Ob, Fo, grade_count=10, model_name_list=None, save_path=None, title="ROC图"):
    '''

    :param Ob:
    :param Fo:
    :param grade_count:
    :param save_path:
    :return:
    '''
    hnh_array = hnh(Ob, Fo, grade_count)
    roc_hnh(hnh_array, grade_count=grade_count, model_name_list=model_name_list, save_path=save_path, title=title)


def roc_hfmc(hfmc_array, grade_count=10, model_name_list=None, save_path=None, title="ROC图"):
    '''

    :param hfmc:
    :param save_path:
    :return:
    '''

    fig = plt.figure(figsize=(5.6, 5.6))
    grade = 1 / grade_count
    if grade_count < 1:
        print('grade_count输入错误，不能小于1')
        return
    grade_list = np.arange(0, 1, grade).tolist()
    grade_list.append(1.1)
    shape = list(hfmc_array.shape)
    new_hfmc_array = hfmc_array.reshape((-1, len(grade_list) - 1, 4))
    new_hfmc_array_shape = new_hfmc_array.shape
    label = []
    if model_name_list is None:
        if new_hfmc_array_shape[0] == 1:
            label.append('实际预报')
        else:
            for i in range(new_hfmc_array_shape[0]):
                label.append('实际预报' + str(i + 1))
    else:
        label.extend(model_name_list)
    colors_list = []
    colors = cm.get_cmap('rainbow', 128)
    for i in range(new_hfmc_array_shape[0]):
        color_grade = i / new_hfmc_array_shape[0]
        colors_list.append(colors(color_grade))

    for line in range(new_hfmc_array_shape[0]):
        far = [1]
        far.extend(pofd_hfmc(new_hfmc_array[line, :]).tolist())
        far.append(0)
        pod = [1]
        pod.extend(pod_hfmc(new_hfmc_array[line, :]).tolist())
        pod.append(0)
        far = np.array(far)
        pod = np.array(pod)
        if (far.size < 30):
            plt.plot(far, pod, color=colors_list[line], linewidth=2, marker=".", label=label[line])
        else:
            plt.plot(far, pod, color=colors_list[line], linewidth=2, label=label[line])
    plt.plot([0, 1], [0, 1], ":", color="k", linewidth=1, label="无技巧预报")
    plt.xlabel("空报率", fontsize=14)
    plt.ylabel("命中率", fontsize=14)
    plt.ylim(0.0, 1.0)
    plt.xlim(0.0, 1.0)
    plt.legend(loc=4, fontsize=14)
    plt.title(title, fontsize=14)
    plt.xticks(fontsize=14)
    plt.yticks(fontsize=14)

    if save_path is None:
        plt.show()
    else:
        plt.savefig(save_path)
        print("检验结果已以图片形式保存至" + save_path)
    plt.close()


def roc_hnh(hnh_array, grade_count=10, model_name_list=None, save_path=None, title="ROC图"):
    '''

    :param th_array:
    :param save_path:
    :return:
    '''
    hfmc_list = []
    grade = 1 / grade_count
    if grade_count < 1:
        print('grade_count输入错误，不能小于1')
        return
    grade_list = np.arange(0, 1, grade).tolist()
    grade_list.append(1.1)
    shape = list(hnh_array.shape)
    hnh_array = hnh_array.reshape((-1, len(grade_list) - 1, 2))
    new_hnh_array_shape = hnh_array.shape
    for line in range(new_hnh_array_shape[0]):
        total_grade_num = hnh_array[line, :, 0]
        observed_grade_num = hnh_array[line, :, 1]
        ngrade = len(total_grade_num)
        hfmc = np.zeros((len(total_grade_num), 4))
        total_hap = np.sum(observed_grade_num)
        total_num = np.sum(total_grade_num)
        for i in range(ngrade):
            hfmc[i, 0] = np.sum(observed_grade_num[i:])
            hfmc[i, 1] = np.sum(total_grade_num[i:]) - hfmc[i, 0]
            hfmc[i, 2] = total_hap - hfmc[i, 0]
            hfmc[i, 3] = total_num - (hfmc[i, 0] + hfmc[i, 1] + hfmc[i, 2])
        hfmc_list.append(hfmc)
    hfmc_array = np.array(hfmc_list)
    shape = shape[:-2]
    shape.append(len(grade_list) - 1)
    shape.append(4)
    hfmc_array.reshape(shape)
    roc_hfmc(hfmc_array, grade_count=grade_count, model_name_list=model_name_list, save_path=save_path, title=title)


def discrimination(Ob, Fo, grade_count=10, save_path=None,dpi = 300, title="区分能力图"):
    '''

    :param Ob:
    :param Fo:
    :param grade_count:
    :param save_path:
    :return:
    '''
    hnh_array = hnh(Ob, Fo, grade_count)

    discrimination_hnh(hnh_array, grade_count=grade_count, save_path=save_path, title=title)


def discrimination_hnh(th_array, grade_count=10, member_list=None, save_path=None,dpi = 300, title="区分能力图"):
    '''

    :param th_array:
    :param save_path:
    :return:
    '''


    grade = 1 / grade_count
    print(grade)
    if grade_count < 1:
        print('grade_count输入错误，不能小于1')
        return
    grade_list = (np.arange(0, 1+0.5* grade, grade) *100).astype(np.int16)/100
    new_th_array = th_array.reshape((-1, len(grade_list) - 1, 2))
    new_th_array_shape = new_th_array.shape
    sup_fontsize = 10
    width = meteva.base.plot_tools.caculate_axis_width(grade_list,sup_fontsize,new_th_array_shape[0])
    if width > 10:
        width = 10
    if width<5:
        width =5
    height = width *0.5
    label = []
    legend_num = new_th_array_shape[0]
    if member_list is None:
        if legend_num == 1:
            label.append('预报')
        else:
            for i in range(legend_num):
                label.append('预报' + str(i + 1))
    else:
        label.extend(member_list)

    fig = plt.figure(figsize=(width, height),dpi = dpi)
    ymax = 0

    legend_col = int(width * 0.8)
    legend_row = int(math.ceil(legend_num / legend_col))
    legend_col = int(math.ceil(legend_num / legend_row))

    axes = plt.subplot(1, 1, 1)
    grade = 1 / grade_count
    x = np.arange(grade / 2, 1, grade)
    bar_width = 0.8 / (grade_count * (legend_num+2))
    z = np.array([0.05, 0.15, 0.25, 0.35, 0.45, 0.55, 0.65, 0.75, 0.85, 0.95])
    for line in range(new_th_array_shape[0]):
        total_grade_num = new_th_array[line, :, 0]
        observed_grade_num = new_th_array[line, :, 1]
        total_num = np.sum(total_grade_num)
        observed_grade_rate = observed_grade_num / total_num
        not_observed_grade_num = total_grade_num - observed_grade_num
        not_observed_grade_rate = not_observed_grade_num / total_num

        x1 = x + (line - new_th_array_shape[0]/2 + 0.5) * bar_width
        axes.plot(x1, (not_observed_grade_rate + observed_grade_rate) * z, '.', color='r')
        axes.bar(x1, observed_grade_rate, width=bar_width, alpha=0.5)
        axes.bar(x1, not_observed_grade_rate + observed_grade_rate, width=bar_width, fill=False)

        # rect = patches.Rectangle((x1, y1), dh_y / dh_x, 1, linewidth=2, edgecolor='k', facecolor='none')

        ymax = max(np.max(not_observed_grade_rate + observed_grade_rate), ymax)
    lines = axes.get_children()
    print(lines)
    print(len(lines))
    plt.xlabel("预测的概率", fontsize=sup_fontsize * 0.9)
    plt.ylabel("占总样本数的比例", fontsize=sup_fontsize * 0.9)
    plt.ylim(0.0, ymax * 1.3)
    plt.xlim(0.0, 1)
    print(grade_count)
    print(len(label))
    legend1 = plt.legend([lines[len(label)], lines[grade_count + len(label)],lines[grade_count* 2 + len(label)+2]], ['观测正例', '观测负例',"合理比例"],
                         loc='upper center', bbox_to_anchor=(0.5, 1), ncol=3,fontsize = sup_fontsize * 0.9)
    axes.add_artist(legend1)
    if legend_num > 1:
        legend2 = plt.legend(lines[len(label):len(lines):grade_count * 2], label, loc='upper center',
                             bbox_to_anchor=(0.5, 0.95), ncol = legend_col)

        axes.add_artist(legend2)

    plt.yticks(fontsize = sup_fontsize * 0.8)
    xtick = np.arange(0, 1.001, 0.1)
    plt.xticks(xtick, fontsize=sup_fontsize * 0.8)
    plt.title(title, fontsize=sup_fontsize)
    if save_path is None:
        plt.show()
    else:
        plt.savefig(save_path)
        print("检验结果已以图片形式保存至" + save_path)
    plt.close()


def comprehensive_probability(Ob, Fo, grade_count=10, save_path=None, title="概率预报综合检验图"):
    '''
    :param Ob:
    :param Fo:
    :param save_path:
    :return:
    '''
    hnh_array = hnh(Ob, Fo, grade_count)
    comprehensive_hnh(hnh_array, grade_count=grade_count, save_path=save_path, title=title)


def comprehensive_hnh(th_array, grade_count=10, model_name_list=None, save_path=None, title="概率预报综合检验图"):
    '''

    :param th_array:
    :param save_path:
    :return:
    '''
    fig = plt.figure(figsize=(5.6, 5.6))
    grade = 1 / grade_count
    if grade_count < 1:
        print('grade_count输入错误，不能小于1')
        return
    grade_list = np.arange(0, 1, grade).tolist()
    grade_list.append(1.1)
    shape = list(th_array.shape)
    new_th_array = th_array.reshape((-1, len(grade_list) - 1, 2))
    new_th_array_shape = new_th_array.shape
    label = []
    if model_name_list is None:
        if new_th_array_shape[0] == 1:
            label.append('实际预报')
        else:
            for i in range(new_th_array_shape[0]):
                label.append('实际预报' + str(i + 1))
    else:
        label.extend(model_name_list)
    colors_list = []
    colors = cm.get_cmap('rainbow', 128)
    for i in range(new_th_array_shape[0]):
        color_grade = i / new_th_array_shape[0]
        colors_list.append(colors(color_grade))
    ymax = -9999
    for line in range(new_th_array_shape[0]):
        total_grade_num = new_th_array[line, :, 0]
        observed_grade_num = new_th_array[line, :, 1]
        total_num = np.sum(total_grade_num)

        under = np.zeros_like(total_grade_num)
        under[:] = total_grade_num[:]
        under[total_grade_num == 0] = 1
        ob_rate = observed_grade_num / under
        ob_rate[total_grade_num == 0] = IV
        ob_rate_noIV = set_plot_IV(ob_rate)
        ob_rate[total_grade_num == 0] = np.nan
        index_iv = np.where(total_grade_num == 0)

        not_observed_grade_num = total_grade_num - observed_grade_num

        ngrade = len(total_grade_num)
        grade = 1 / ngrade
        x = np.arange(grade / 2, 1, grade)

        line_x = np.arange(0, 1.01, 0.1)
        prefect_line_y = np.arange(0, 1.01, 0.1)
        climate_line_y = np.ones_like(line_x) * np.sum(observed_grade_num) / total_num

        grid_plt = plt.GridSpec(6, 2)
        if line == 0:
            fig = plt.figure(figsize=(10, 7))
            plt.suptitle(title)
            plt.subplots_adjust(wspace=0.2, hspace=1)
            ax3 = plt.subplot(grid_plt[0:4, 1])
            ax2 = plt.subplot(grid_plt[4:, 0])
            ax4 = plt.subplot(grid_plt[4:, 1])
            ax1 = plt.subplot(grid_plt[0:4, 0])
        ax1.plot(x, ob_rate_noIV, "--", linewidth=0.5, color="k")
        x_iv = x[index_iv[0]]
        ob_rate_noIV_iv = ob_rate_noIV[index_iv[0]]

        ax1.plot(x_iv, ob_rate_noIV_iv, "x", color='k')
        if line == 0:
            ax1.plot(line_x, prefect_line_y, '--', label="完美预报", color="k")
            ax1.plot(line_x, climate_line_y, ':', label="无技巧预报", color="k")
        ax1.plot(x, ob_rate, marker=".", markersize="10", label=label[line], color=colors_list[line])

        ax1.set_xlim(0.0, 1)
        ax1.set_ylim(0.0, 1)
        ax1.set_xlabel("预测的概率")
        ax1.set_ylabel("实况的发生比例")
        ax1.set(title='\n')
        ax1.legend(loc=2)

        x1 = x - 0.01 + (line + 1.5) * 0.01
        ax2.bar(x1, total_grade_num, width=0.01)
        # plt.setp(ax2.get_xticklabels())
        ax2.set_xlim(0.0, 1.0)
        ax2.set_ylabel("样本数")
        ax2.set_xlabel("预测的概率")
        ax2.set(title='\n')
        total_hap = np.sum(observed_grade_num)
        hfmc = np.zeros((len(total_grade_num), 4))
        for i in range(ngrade):
            hfmc[i, 0] = np.sum(observed_grade_num[i:])
            hfmc[i, 1] = np.sum(total_grade_num[i:]) - hfmc[i, 0]
            hfmc[i, 2] = total_hap - hfmc[i, 0]
            hfmc[i, 3] = total_num - (hfmc[i, 0] + hfmc[i, 1] + hfmc[i, 2])

        far = [1]
        far.extend(pofd_hfmc(hfmc).tolist())
        far.append(0)
        pod = [1]
        pod.extend(pod_hfmc(hfmc).tolist())
        pod.append(0)
        far = np.array(far)
        pod = np.array(pod)

        if line == 0:
            ax3.plot([0, 1], [0, 1], ":", color="k", linewidth=1, label="无技巧预报")
        ax3.plot(far, pod, color=colors_list[line], linewidth=2, label=label[line])
        ax3.set_xlabel("空报率")
        ax3.set_ylabel("命中率")
        ax3.set_ylim(0.0, 1.0)
        ax3.set_xlim(0.0, 1.0)
        ax3.legend(loc=4)
        ax3.set(title='\n')
        x1 = x - 0.04 - 0.01 * 1.5 + (line + 1.5) * 0.01 * 1.5
        ax4.bar(x1, observed_grade_num / total_num, width=0.01, color=colors_list[line])
        ax4.bar(x1, (not_observed_grade_num / total_num) + (observed_grade_num / total_num), width=0.01,
                edgecolor=colors_list[line],
                fill=False)

        ax4.set_xlabel("预测的概率")
        ax4.set_ylabel("占总样本数的比例")
        ymax1 = max(max(np.max(observed_grade_num / total_num), np.max(not_observed_grade_num / total_num)), ymax)
        ymax = ymax1
        # xtick = np.arange(0, 1.001, 0.1)
        #
        # ax4.set_xticklabels(xtick, fontsize=14)
        ax4.set_ylim(0.0, ymax * 2.5)
        ax4.set_xlim(0.0, 1.1)
        ax4.set(title='\n')
        ax4.legend(loc="upper right", ncol=2)
    lines = ax4.get_children()
    legend2 = ax4.legend([lines[0], lines[grade_count]], ['观测事件已发生样本占比', '观测事件未发生样本占比'],
                         loc='upper center', bbox_to_anchor=(0.5, 0.9), ncol=2, prop={'size': 6})
    legend1 = ax4.legend(lines[0:len(lines):grade_count * 2], label, loc='upper center', bbox_to_anchor=(0.5, 1),
                         ncol=5, prop={'size': 6})

    ax4.add_artist(legend1)
    ax4.add_artist(legend2)
    if save_path is None:
        plt.show()
    else:
        plt.savefig(save_path)
        print("检验结果已以图片形式保存至" + save_path)
    plt.close()
