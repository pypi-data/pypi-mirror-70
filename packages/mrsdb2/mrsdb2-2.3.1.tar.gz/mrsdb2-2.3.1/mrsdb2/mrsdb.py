...
class Connection:
    ...
    def commit(self, location=None, wait=True): 
        """
        ...
        while not done:
            try:
                ...
                raise CommitError(f'{e}\nDatabase in memory could not be dumped properly.')
            

            if not dumps or dumps == '':
                done = True
                raise CommitError(f'Database in memory could not be dumped properly.')
    ...
    def make_table(self, name): 
        """
        Create a new table, raises exceptions.Exists if table exists.
        """
        # Args:
    ...
    def delete_table(self, name):
        """
        Delete a table, raises KeyError if none.
        """
        if name in self.db:
            del self.db[name]
        else:
    ...
    def gget(self, table, uid=None, **item): 
        """"
        Query a table (returns generator)
        """
        # Args:
        #     - table: Table to search (ex. "Users")
        #     - item: Item to search for (ex. name="abcdefg")
        tn = table
        table = dict(self.db[tn])
        for ik in table: 
            if uid:
                if ik == uid:
    ...
    def add(self, table, value, commit=None, escape=True, **kwargs): 
        """
        ...
        if type(value) == dict:
            for v in value:
                if escape and type(value[v]) == str:
    ...
    # def aadd(self, table, value, commit=None, escape=True, **kwargs): 
    #     """
    ...
    def delete_item(self, table, item):
        if not 'mrsdb_uid' in item:
            raise KeyError('mrsdb_uid')
        self.db[table].pop(item.mrsdb_uid, None)
...
class Item(dict): 
    ...
    def __setattr__(self, attr, value): 
        ...
        if not attr.endswith('___'): 
            ...
            nw[attr] = value
            dbi[self['___db_dbiid___']].add(self['___table___'], nw, uid=self['___ik___'], escape=False)
        if '___i___' in self: 
