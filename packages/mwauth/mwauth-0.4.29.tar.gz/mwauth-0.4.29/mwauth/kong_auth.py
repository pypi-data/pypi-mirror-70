    ...
    def __set_auth(self):
        ...
        if g.jwt is None:
            ...
            if jwt:
                ...
                assert False,'header，query，和cookie中都没有 jwt or sessionid or token or apikey？'
        if not session:
            ...
            if request.headers.get('X-Consumer-Username') and request.headers.get('X-Consumer-Custom-Id') and not session.get('uid'):
