...
class Dataset:
    ...
    def constructTags(self, tags):
        ...
        tags = [
            np.zeros(len(self.labels[channel])) for channel in range(numberOfChannels)
        ]
    ...
    def forEachChannel(self, labels, converter):
        ...
        * Data source containing the data of the epoch that contains the label
        ...
        return [
            self.__forEachLabel(labels, channel, converter)
...
class MatDataset(Dataset):
    ...
    def importData(filename, structName="data"):
        ...
        for i in range(len(keys)):
            ...
            dictData[key] = np.squeeze(
                vals[key]
            )  # Converts Matlab arrays into Python numpy arrays
    ...
    def tags(self):
        ...
        tags = (
            self.convertToPy(self.raw["sleepy-tags"])
            if "sleepy-tags" in self.raw
            else None
        )
    ...
    def convertToPy(self, array):
        ...
        for index in range(len(array)):
            ...
            array[index] = (
                array[index].squeeze() if array[index].shape != (1,) else array[index]
            )
    ...
    def save(self, path, navigators):
        ...
        self.userLabels = np.array(
            [navigator.getLabelPartition()[1] for navigator in navigators]
        )

        self.tags = np.array(
            [navigator.getComputedEventTags() for navigator in navigators]
        )
