...
class NimbusThread(RESTThread):
    ...
    def __init__(self, device_id, location_id, structures, token, protocol_name="Nimbus", post_interval=None):
        ...
        self.url_template = "https://api-edge.nimbusweather.org/v3/data/location/%s/structure/%s/measurements?device_id=%s"
    ...
    def process_record(self, record):
        ...
        for key in self.structures:
            for count in range(self.max_tries):
                try:
                    params = self.get_params(to_ENGLISH(record), self.structures[key]['params'])
