    ...
    for ids, user_contents_df in stream_df.groupby(['userid', 'contentsid', 'lecture'], sort=False):
        ...
        for i in range(len(page_seq)):
            ...
            if (i == len(page_seq) - 1) or \
                ...
                time_to_leave_index = i
                time_to_enter = time_seq[time_to_enter_index]
                time_to_leave = time_seq[time_to_leave_index]
                ...
                page_info_dict = {'userid': user_id, 'contentsid': contents_id, 'lecture': lecture_week,
                                  ...
                                  'time_to_enter': time_to_enter, 'time_to_leave': time_to_leave}
                ...
                if operation_seq[i] == 'CLOSE':
                    time_to_enter_index = i + 1
                    continue
                else:
                    time_to_enter_index = i
...
def convert_into_page_wise(course_info, event_stream, invalid_seconds=None, timeout_seconds=None, separate_marker_type=False):
    """
    ...
    :return: Convert result which represents how many times each learner used each operation and
             ...
             (index: row number, columns:['user id', 'contents id', 'page no', 'reading_seconds', 'time_to_enter', 'time_to_leave', each operation])

    :rtype: PageWiseAggregation
    """
    # convert into page transition
    page_transition = convert_into_page_transition(course_info, event_stream, separate_marker_type, invalid_seconds, timeout_seconds)

    # page-wise aggregation
    page_info_dict_list = []
    for ids, page_df in page_transition.df.groupby(['userid', 'contentsid', 'lecture', 'pageno'], sort=False):
        behavior_df = page_df.drop(['userid', 'contentsid', 'lecture', 'pageno', 'time_to_enter', 'time_to_leave'], axis=1)
...
def convert_into_time_range(course_info, event_stream, interval_seconds, lecture_week=None,
    ...
    :param start_time: The start time to aggregate. The available arguments is following:
                       ...
                       tuple(y, m, d, H, M, S) ... use the time (year, month, day, hours, minutes, seconds).
                                                   Each element is int type value.
    ...
    for lecture in lecture_weeks:
        ...
        df = page_transition.df.loc[:, ['userid', 'contentsid', 'lecture', 'pageno', 'time_to_enter', 'time_to_leave']]
        ...
        for sec in seconds:
            ...
            df_in_range = df[(df['time_to_enter'] < end_of_range) & (start_of_range < df['time_to_leave'])]
            if count_operation:
            ...
            for user_id in event_stream.user_id():
                ...
                def _time_in_pages(time_to_enter_list, time_to_leave_list):
                    time_in_page_list = []
                    for enter, leave in zip(time_to_enter_list, time_to_leave_list):
                        time_to_enter = max(enter, start_of_range)
                        time_to_leave = min(leave, end_of_range)
                        time_in_page_ = time_to_leave - time_to_enter
                ...
                if user_df.empty:
                    ...
                    with warnings.catch_warnings():
                        warnings.simplefilter('ignore')
                        time_in_pages = _time_in_pages(user_df['time_to_enter'], user_df['time_to_leave'])
    ...
    record_df = record_df.sort_values(['lecture', 'userid'])
