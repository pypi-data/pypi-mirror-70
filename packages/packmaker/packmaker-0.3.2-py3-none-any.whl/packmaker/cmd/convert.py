...
class ConvertCommand (Subcommand):
    ...
    def run_command(self, arguments):
        ...
        log.info('Looking up mods ...')
        for mod in manifest['files']:
            slug = None
            project_id = mod['projectID']
            file_id = mod['fileID']

            mod = self.api.get_addon(project_id)
            slug = mod['slug']
            if slug is None:
                print('  !! Cannot find a project with id {}'.format(project_id))
                continue
            modfile = self.api.get_addon_file(project_id, file_id)
            if modfile is None:
                print('  || Cannot find the correct version of mod: {} (fileID = {})'.format(slug, file_id))
                continue
            packdef['mods'].append({slug: {'version': modfile['fileName']}})
