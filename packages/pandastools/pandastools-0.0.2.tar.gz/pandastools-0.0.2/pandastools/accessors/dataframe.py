...
class DataFrameAccessor(object):
    ...
    def get_info(self) -> str:
        buf = io.StringIO()
        self._obj.info(buf=buf, null_counts=True)
        return buf.getvalue()
    ...
    def split(self, thresh, colname, extra_rows):
        ...
        self._obj = self._obj.drop("secs", errors="ignore", axis=1)
        array = np.full((len(self._obj.index),), np.nan)
        self._obj["process_num"] = pd.Categorical(array)
        ds = dataimport.add_transition_info(ds=self._obj,
                                            colname=colname,
                                            threshold=thresh,
