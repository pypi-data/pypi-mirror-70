...
#        print(f"ep{ep}. depth={depth}, phase={phase}")
    ...
    def simflux_fit(self, mod, ctx:Context, indices):
        with Context(ctx.smlm) as ctx:
            ...
            psf = sf.CreateEstimator_Gauss2D(self.locs.cfg['sigma'], mod, 
                                                 roisize, len(mod),
            ...
            idx = 0
            for roipos, pixels in self.roi_source(indices):
                queue.Schedule(pixels, roipos=roipos, ids=np.arange(len(roipos))+idx)
                idx += len(roipos)
