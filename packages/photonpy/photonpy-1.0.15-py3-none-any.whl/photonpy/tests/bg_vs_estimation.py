...
with Context(debugMode=False) as ctx:
    ...
    data = [
        (g_psf, f"2D Gaussian (bg={bg} sigma={sigma})", estimate_precision(g_psf, g_psf.Estimate, theta, photons))
    ]

    for bg,sig in [(1,1.6), (0.1, 1.6), (10, 1.6), (1, 3), (10, 3)]:
        sf_psf = s.CreateEstimator_Gauss2D(sig, mod, roisize, len(mod), True)
        sf_psf.SetLevMarParams(1e-18, 100)
    
        # We make sure the total amount of background is equal for simflux/non-simflux
        sf_theta = theta*1
        sf_theta[:,3] = bg / len(mod)
        
        data.append(
            (sf_psf, f"SIMFLUX bg={bg} sigma={sig}", 
                 estimate_precision(sf_psf, sf_psf.Estimate, sf_theta, photons))
        )
        sf_psf.Destroy()
