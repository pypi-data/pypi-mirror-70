...
with Context(debugMode=False) as ctx:
    ...
    def sf_estimate_lm(lm):
        def f(smp, roipos):
            r = g_psf.Estimate(smp.sum(1))
            sf_psf.SetLevMarParams(lm,iterations=200)
            return sf_psf.Estimate(smp, roipos, initial=r[0])
        return f
    ...
    for lm in [1e-20,1e-19,3e-18, 1e-18,1e-17, 1e-16,1e-15,-10]:
        data.append(
            (sf_psf, f"SIMFLUX LM={lm}", estimate_precision(sf_psf, sf_estimate_lm(lm), sf_theta, photons)),
        )
   
    sf_theta[:,2] = 1000
    smp = sf_psf.GenerateSample(sf_theta) 
    
    su.imshow_rois(smp[0])
    ...
    for i,ax in enumerate(axes):
        ...
        plt.title(f'{ax} axis - bg={bg} sigma={sigma}')
        plt.xscale("log")
        plt.yscale("log")
