...
class EtcLangParser(object):
    ...
    :param bool check_suspicious_characters:
        If True, check if there are characters in the first line
...
class EtcParser(base.BaseParser, PluginBase):
    """
    ...
    def parse_string_lark(self, string: str) -> RootField:
        ...
        for element in parsed.iter_subtrees():
            if element.data == "comment":
                ...
                fields.append(field)
        if fields:
            return RootField("root", typeconf="etc", data=fields)
