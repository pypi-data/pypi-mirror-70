...
def expand_objects(list, length=1, copy=False):
    """Expand a list of objects.

    Parameters:
        list (list): List of py_pol objects.
    ...
    Returns:
        list (list): List of expanded objects.
    """
    ...
    if N > 1:
        ...
        for obj in list:
            if obj.size == 1:
                if obj._type is 'Jones_vector':
                    new_M = np.array(
                        [obj.M[0, 0] * np.ones(N), obj.M[1, 0] * np.ones(N)])
                    obj.from_matrix(new_M)
                elif obj._type is 'Jones_matrix':
                    new_M = np.array([
                        [
                            obj.M[0, 0, 0] * np.ones(N),
                            obj.M[0, 1, 0] * np.ones(N)
                        ],
                        [
                            obj.M[1, 0, 0] * np.ones(N),
                            obj.M[1, 1, 0] * np.ones(N)
                        ],
                    ])
                    obj.from_matrix(new_M)
    return list
...
def charac_angles_2_azimuth_elipt(alpha, delta):
    ...
    """
    # Prepare variables
    alpha, delta = prepare_variables(vars=[alpha, delta], expand=[True, True])
    # Check that the angles belong to the correct interval. If not, fix it.
...
def which_quad(angle, octant=True):
    ...
    if octant:
        ...
        q = q + 1 * (angle > 0) * (angle < np.pi / 4)
        q = q + 1.5 * (angle == np.pi / 4)
        q = q + 2 * (angle > np.pi / 4) * (angle < np.pi / 2)
        q = q + 2.5 * (angle == np.pi / 2)
        q = q + 3 * (angle > np.pi / 2) * (angle < np.pi * 3 / 4)
        q = q + 3.5 * (angle == np.pi * 3 / 4)
        q = q + 4 * (angle > np.pi * 3 / 4) * (angle < np.pi)
        q = q + 4.5 * (angle == np.pi)
    else:
        q = q + 1 * (angle > 0) * (angle < np.pi / 2)
        q = q + 1.5 * (angle == np.pi / 2)
        q = q + 2 * (angle > np.pi / 2) * (angle < np.pi)
        q = q + 2.5 * (angle == np.pi)
        q = q + 3 * (angle > np.pi) * (angle < np.pi * 3 / 2)
        q = q + 3.5 * (angle == np.pi * 3 / 2)
        q = q + 4 * (angle > np.pi * 3 / 2) * (angle < np.pi * 2)
    return q
