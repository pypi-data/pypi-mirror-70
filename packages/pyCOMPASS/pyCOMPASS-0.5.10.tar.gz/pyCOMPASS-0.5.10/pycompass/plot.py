...
class Plot:
    ...
    def plot_heatmap(self, plot_type=None, output_format='html', *args, **kwargs):
        ...
        _bf = BiologicalFeature.using(self.module.compendium).get(filter={'id_In': bf})
        _ss = SampleSet.using(self.module.compendium).get(filter={'id_In': ss})

        sorted_bf = [tuple for x in bf for tuple in _bf if tuple.id == x]
        sorted_ss = [tuple for x in ss for tuple in _ss if tuple.id == x]
