    ...
    with load_session() as session:
        ...
        while url:
            ...
            for row in payload['rows']:
                click.echo('[{}]\t{}'.format(uuid.UUID(row['id']).hex, row['name']))
            token = payload['token']
            url = None if token is None else '{}/acorn/create/view/project?$skiptoken={}'.format(ACORN, token)
...
def load_session():
    ...
    if not r.ok:
        session.close()
        sys.exit('your session has expired, please log in again.')
