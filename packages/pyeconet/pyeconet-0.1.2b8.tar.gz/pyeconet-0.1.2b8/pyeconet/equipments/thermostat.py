...
class Thermostat(Equipment):
    ...
    def set_set_point(self, target_temp, target_temp_cool, target_temp_heat):
        ...
        if target_temp_cool or target_temp and self.mode == ThermostatOperationMode.COOLING:
            if self.cool_set_point_limits[0] <= target_temp <= self.cool_set_point_limits[1]:
                cool_payload["@COOLSETPOINT"] = target_temp_cool
            else:
                _LOGGER.error("Cool set point out of range. Lower: %s Upper: %s Cool set point: %s",
                              self.cool_set_point_limits[0], self.cool_set_point_limits[1], target_temp_cool)
        if target_temp_heat:
            if self.heat_set_point_limits[0] <= target_temp <= self.heat_set_point_limits[1]:
                heat_payload["@HEATSETPOINT"] = target_temp_heat
            else:
                _LOGGER.error("Heat set point out of range. Lower: %s Upper: %s Heat set point: %s",
                              self.heat_set_point_limits[0], self.heat_set_point_limits[1], target_temp_heat)
