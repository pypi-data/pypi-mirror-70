...
class AttributeType:
    ...
    def get_random(self, fake_factory, locale, **kwargs_for_random_generator):
        return self.random_generator(fake_factory, locale, **kwargs_for_random_generator)


### Random generators


def first_name_random_generator(fake_factory, locale, **kwargs):
    return fake_factory.first_name()


def last_name_random_generator(fake_factory, locale, **kwargs):
...
def city_random_generator(fake_factory, locale, **kwargs):
...
def street_address_random_generator(fake_factory, locale, **kwargs):
...
def phone_number_random_generator(fake_factory, locale, **kwargs):
    return fake_factory.numerify(choice([f"0{d} ## ## ## ##" for d in range(1, 8)]))


def date_random_generator(
    fake_factory, locale, pattern="%Y-%m-%d %H:%M:%S", start_date="-10y", end_date="today", **kwargs
):
    return fake_factory.date_between(start_date=start_date, end_date=end_date).strftime(pattern)


def field_random_generator(
    fake_factory, locale, translation_required=False, possible_values=None, **kwargs
):
    ...
    if language == "en" or translation_required == False:
        ...
        try:
            translated_txt = translator.translate(txt, src="en", dest=language)
            if translated_txt == txt:
                print(f"Missing translation : {txt}")
                return txt
