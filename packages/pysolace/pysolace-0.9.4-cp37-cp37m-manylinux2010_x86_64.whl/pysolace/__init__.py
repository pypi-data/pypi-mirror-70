...
class SolClient:
    ...
    def onreply_callback_wrap(self, topic: str, corrid: str, reply: dict):
        with logger.catch():
            ...
            self.req_rep_map[corrid].pop("corrid")
    ...
    def request(
        ...
        if not timeout:
            corrid_dict = solclient.request(
                self.sol, topic, payload, timeout, format
            )
            corrid = corrid_dict["corrid"]
            self.req_rep_map[corrid] = corrid_dict
            if cb:
                self.rep_callback_map[corrid] = cb
            return self.req_rep_map[corrid]
        return solclient.request(self.sol, topic, payload, timeout, format)
