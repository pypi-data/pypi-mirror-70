...
def predict_fwhm(x, y, center, peak_center_height, prefer_high_order=True, tol=1E-3):
    ...
    if peak_position < 0.2:
        warnings.warn('The peak is too close to the origin, manual control is advised.', UserWarning)
    ...
    if _ensure_window_at_origin(center, window_size, order, peak_center_height, tol=tol)[0]:
        return center, window_size, order
    else:
        val = _ensure_window_at_origin(center, window_size, order, peak_center_height, tol=tol)[1]
        # print(f'The window is bigger at the origin than the desired tolerance. Actual: {val:.4e}, Desired: {tol*peak_center_height:.4e}')
        warnings.warn(f'The window is bigger at the origin than the desired tolerance. Actual:{val:.4e}, Desired:{tol*peak_center_height:.4e}')
        return center, window_size, order + 2

def _run(ifg, skip_domain_check=False, only_phase=False, show_graph=True, usenifft=False):
    print('Interferogram received.')
    if ifg.probably_wavelength is True and not skip_domain_check:
        print('Probably in wavelength domain, changing to frequency...', end='', flush=True)
        ifg.chdomain()
        print('Done')
    if usenifft:
    ...
    print(f'A {_order} order gaussian window centered at {c:.2f} fs with FWHM {ws:.2f} fs was applied.')
