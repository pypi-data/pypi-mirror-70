...
class PagerDuty(object):
    ...
    def _request(self, event_type, **kwargs):
        ...
        print("ENCODED_EVENT: {0}".format(event), file=sys.stderr)
        print("ENCODED_EVENT_CLASS: {0}".format(event.__class__), file=sys.stderr)
        try:
            res = requests.post(self.api_endpoint, json=event)
