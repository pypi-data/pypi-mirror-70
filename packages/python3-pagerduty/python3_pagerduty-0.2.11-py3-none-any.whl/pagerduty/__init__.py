...
class PagerDuty(object):
    ...
    def _request(self, event_type, **kwargs):
        ...
        print("EVENT CLASS: {0}".format(event.__class__), file=sys.stderr)
        encoded_event = json.dumps(str(event))
        try:
            res = requests.get(self.api_endpoint, data=encoded_event)
        ...
        if res.status_code != requests.codes.ok:
            raise PagerDutyException(result['status'], result['message'], res.status_code)
