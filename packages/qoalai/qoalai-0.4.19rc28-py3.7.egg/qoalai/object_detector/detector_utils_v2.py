    ...
    def yolo_loss(self, outputs: [], labels: []):
        ...
        for i in range(3):
            ...
            for idx, val in enumerate(self.anchor[border_a:border_b]):
                ...
                ctr_loss = self.center_loss_alpha * (mse_loss_mean(x_pred, x_label) + mse_loss_mean(y_pred, y_label))
                ...
                sz_loss =  self.size_loss_alpha * (tf.sqrt(mse_loss_mean(a, b)) + tf.sqrt(mse_loss_mean(c, d)))

                if self.num_class == 1:
                    class_loss = self.class_loss_alpha * sigmoid_crossentropy_mean(class_pred, class_label)
                else:
                    class_loss = self.class_loss_alpha * softmax_crosentropy_mean(class_pred, class_label)
                ...
                if self.is_multilabel: 
                    ...
                    for a, b in zip(additional_class_pred, additional_class_label):
                        ...
                        #additional_class_loss.append(0. * softmax_crosentropy_sum(a, b))
