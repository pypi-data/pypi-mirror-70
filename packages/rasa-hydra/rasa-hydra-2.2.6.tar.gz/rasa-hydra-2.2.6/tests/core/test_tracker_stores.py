...
async def test_get_or_create():
    ...
    again = await store.get_or_create_tracker(UserMessage.DEFAULT_SENDER_ID)
    assert again.get_slot(slot_key) == slot_val


async def test_restart_after_retrieval_from_tracker_store(default_domain):
...
async def test_tracker_store_remembers_max_history(default_domain):
    store = InMemoryTrackerStore(default_domain)
...
async def test_tracker_store_endpoint_config_loading():
