from red_raccoon.api.stix import MemoryClient
from red_raccoon.stix import COURSE_OF_ACTION, ATTACK_PATTERN, RELATIONSHIP, MALWARE, MITIGATES
...
class StixClientTestCases(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.api = MemoryClient(data=STIX_DATA)
        cls.maxDiff = None
    ...
    def test_search_for_objects_with_single_filter(self):
        predicate = red_raccoon.stix.filters.get_object_type_filter(object_types=MALWARE)

        expected = [MALWARE_A]
        result = list(map(
            red_raccoon.stix.parsers.stix2_to_dict, self.api.search_for_objects(filters=predicate)
        ))
    ...
    def test_search_for_objects_without_any_filters(self):
        expected = STIX_DATA
        result = list(map(red_raccoon.stix.parsers.stix2_to_dict, self.api.search_for_objects()))
        self.assertEqual({o['id'] for o in expected}, {o['id'] for o in result})
    ...
    def test_get_object_by_internal_id(self):
        ...
        result = self.api.get_object_by_internal_id(object_internal_id=COURSE_OF_ACTION_A['id'])
        result = red_raccoon.stix.parsers.stix2_to_dict(result)
        self.assertEqual(expected['id'], result['id'])

    def test_get_object_by_external_id(self):
        expected = COURSE_OF_ACTION_A
        result = self.api.get_object_by_external_id(object_external_id='M1017')
        result = red_raccoon.stix.parsers.stix2_to_dict(result)
        self.assertEqual(expected['id'], result['id'])
    ...
    def test_get_objects_by_internal_id(self):
        ...
        result = set(map(
            lambda o: o['id'], self.api.get_objects_by_internal_id(
    ...
    def test_get_objects_by_external_id(self):
        expected = {self.api.get_object_by_external_id(object_external_id='M1017')['id']}
        result = set(map(
            lambda o: o['id'], self.api.get_objects_by_external_id(object_external_ids=['M1017'])
        ))
        self.assertEqual(expected, result)
    ...
    def test_get_object_internal_ids_by_external_id(self):
        external_ids = [
            'M1017',
            'M1032',
            'M1054',
        ...
        result = set(self.api.get_object_internal_ids(object_ids=external_ids))
        self.assertEqual(expected, result)

    def test_get_directed_relationships_by_source_internal_id(self):
        expected = {'relationship--f780f9d8-1baa-41b7-b7a1-acba717df0ab'}
        result = {o['id'] for o in self.api.get_directed_relationships(
            source_object_internal_ids=[COURSE_OF_ACTION_A['id']]
        )}
        self.assertEqual(expected, result)

    def test_get_directed_relationships_by_target_internal_id(self):
        expected = {
    ...
    def test_get_directed_relationships_by_source_object_type(self):
        expected = {
            'relationship--248a0d72-d9cd-43d3-985f-a33a49a79e8b',
            'relationship--7a1cf82e-68e5-49ca-89ae-e492cd85dab4',
            'relationship--f780f9d8-1baa-41b7-b7a1-acba717df0ab'
    ...
    def test_get_directed_relationships_by_target_object_type(self):
        expected = {
    ...
    def test_get_directed_relationships_by_relationship_type(self):
        expected = {
            'relationship--248a0d72-d9cd-43d3-985f-a33a49a79e8b',
            'relationship--7a1cf82e-68e5-49ca-89ae-e492cd85dab4',
            'relationship--f780f9d8-1baa-41b7-b7a1-acba717df0ab'
