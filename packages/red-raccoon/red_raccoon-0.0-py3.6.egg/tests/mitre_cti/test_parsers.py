from red_raccoon.mitre_cti import TACTIC, MATRIX
from red_raccoon.stix import MARKING_DEFINITION, EXTERNAL_REFERENCE, KILL_CHAIN_PHASE, IDENTITY, \
    ATTACK_PATTERN, INTRUSION_SET, RELATIONSHIP, TOOL, MALWARE, COURSE_OF_ACTION

from red_raccoon.mitre_cti.types import ExternalReference, Relationship, Tactic, \
    EnterpriseTechnique, MobileTechnique, PreAttackTechnique, Mitigation, Malware, Tool, Group, \
    Identity, Matrix
    ...
    def test_parse_objects(self):
        for object_type, data, data_class in TEST_VECTORS:
            with self.subTest(object_type):
                ...
                result = red_raccoon.mitre_cti.parsers.parse_object(data)
                self.assertIsInstance(result, data_class, "{} ({})".format(data['id'], data_class))

    def test_parse_objects_allow_revoked(self):
        for object_type, data, data_class in TEST_VECTORS:
            with self.subTest(object_type):
                self.assertIsInstance(data, dict)
                data['revoked'] = True
                result = red_raccoon.mitre_cti.parsers.parse_object(data, ignore_revoked=False)
                self.assertIsInstance(result, data_class, "{} ({})".format(data['id'], data_class))

    def test_parse_objects_ignore_revoked(self):
        for object_type, data, data_class in TEST_VECTORS:
            with self.subTest(object_type):
                self.assertIsInstance(data, dict)
                data['revoked'] = True
                result = red_raccoon.mitre_cti.parsers.parse_object(data, ignore_revoked=True)
                self.assertIsNone(result)

    def test_parse_objects_allow_deprecated(self):
        for object_type, data, data_class in TEST_VECTORS:
            with self.subTest(object_type):
                self.assertIsInstance(data, dict)
                data['deprecated'] = True
                result = red_raccoon.mitre_cti.parsers.parse_object(data, ignore_deprecated=False)
                self.assertIsInstance(result, data_class, "{} ({})".format(data['id'], data_class))

    def test_parse_objects_ignore_deprecated(self):
        for object_type, data, _ in TEST_VECTORS:
            with self.subTest(object_type):
                self.assertIsInstance(data, dict)
                data['deprecated'] = True
                result = red_raccoon.mitre_cti.parsers.parse_object(data, ignore_deprecated=True)
                self.assertIsNone(result)
