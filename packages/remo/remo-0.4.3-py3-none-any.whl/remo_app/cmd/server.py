...
def run_server(config, debug=False, background_job=None, with_browser=True):
    ...
    if config.is_local_server() and is_port_in_use(config.port):
        typer.echo(f'Failed to start remo-app, port {config.port} already in use.')

        ok = try_to_terminate_another_remo_app(config)
        if not ok:
            typer.echo(f'You can change default port in config file: {Config.path()}')
            return
    else:
    ...
    if config.is_local_server():
        ...
        typer.echo(f'Remo is running on remote server: {config.get_host_address()}')
        if with_browser:
