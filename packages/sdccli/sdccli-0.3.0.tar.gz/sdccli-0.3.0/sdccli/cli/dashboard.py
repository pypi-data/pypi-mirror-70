...
import sdccli.usecases.dashboard as use_case
...
def add(cnf, name, templatefile, scope, shared, public, annotation):
    ...
    try:
        ...
        res = use_case.add_dashboard(cnf.sdmonitor, name, template, scope, shared, public, annotations)
...
@dashboard.command(name='add-panel',
...
def add_panel(cnf, dashboard, name, type, metric, scope, sort, limit, layout):
    ...
    """
    try:
        dashboard = use_case.get_dashboard_by_id_or_name(cnf.sdmonitor, dashboard).raw()
        ...
        dashboard = use_case.add_panel_to_dashboard(cnf.sdmonitor, dashboard, panel)
...
@dashboard.command(name='del-panel', short_help="Removes a panel from the dashboard")
@click.argument('dashboard', nargs=1)
@click.argument('name', nargs=1)
@click.pass_obj
def delete_panel(cnf, dashboard, name):
    ...
    """
    try:
        dboard = use_case.get_dashboard_by_id_or_name(cnf.sdmonitor, dashboard)
        use_case.remove_panel_from_dashboard(cnf.sdmonitor, dboard, name)
        print("Success")
    except Exception as ex:
        print(f"Error {str(ex)}")
        sys.exit(1)
