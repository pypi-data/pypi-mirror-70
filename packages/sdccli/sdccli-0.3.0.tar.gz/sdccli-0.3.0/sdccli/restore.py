...
#
# def alerts(sdmonitor, filename, existing_alerts, changed_ids=None):
...
#             name = alert['name']
#             print("[{}/{}] Creating alert {}... ".format(i + 1, total, name.strip()))
#             if name in existing_alerts:
#                 if same_dict(alert, existing_alerts[name],
#                              ['id', 'teamId', 'autoCreated',
#                               'notificationChannelIds', 'notificationCount', 'invalidMetrics',
...
#                     continue
#                 else:
#                     sdmonitor.delete_alert(existing_alerts[name])
#
...
#
#             ok, result = sdmonitor.create_alert(alert_obj=alert)
#             if not ok:
#                 errors.append("couldn't recreate alert " + name + ': ' + result)
#
#         if len(errors) != 0:
#             return False, errors
#         return True, None
...
def falco_macros(file_data, sdsecure: SdSecureClient):
    ...
    for i, macro in enumerate(file_data, start=1):
        ...
        ok, result = _add_falco_macro(sdsecure, name=name, condition=macro["condition"]["condition"], append=macro['append'])
        if not ok:
