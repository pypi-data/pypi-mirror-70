...
def handle_error(data, counter, is_not_processed, project_id, topic_name=None):
    retry_limit_exceeded = False
    if counter >= 1 and is_not_processed:
        logging.info(
...
def handle_pubsub_retry(request, data, project_id, topic_name=None):
    ...
    is_not_processed = check_message_processed(msg_status_key)
    return handle_error(data, counter, is_not_processed, project_id, topic_name=topic_name)
