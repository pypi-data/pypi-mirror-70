...
services:
  {{params['project_name'] }}_python: &{{params['project_name'] }}_python # link to instance
    ...
    - 8000:8000
    environment:
      DJANGO_SETTINGS_MODULE: '{{params['project_name'] }}.settings'
    ...
    depends_on:
      ...
      - {{ params['project_name'] }}_celery_worker
  postgres:
  ...
  {{ params['project_name'] }}_celery_worker:
    ...
    command: {{params['project_name'] }}_celery -A {{params['project_name'] }} worker --loglevel=info
    ports: []
    depends_on:
      - postgres
      - {{params['project_name'] }}_rabbitmq
