...
def fixUndersegmentation(imLab, imGrey, listLabels, a, c, numVect = 100, vect = None, boundingBoxes = None, centresOfMass = None, NumberOfThreads = 1, verbose = False):
    ...
    for i in range(len(listLabels)):
        ...
        while Continue == False:
            ...
            #New Segmentation
            if verbose:
                print('Processing label '+str(1)+' of '+str(len(listLabels))+'. Iteration #'+str(itCounter))
            ...
            if numpy.max(newSeg) > 1:
                ...
                if any([a and b for a,b in zip(volNewSeg > 0.5, volNewSeg < 2.)]) == True:
                    ...
                    for lab in numpy.unique(newSeg[newSeg != 0]):
                        #Assign the new labels only to particles that have a normalized volume that makes sense
                        if volNewSeg[lab] > 0.5:
                    ...
                    labelDummyUnit[boundingBoxes[label_i][0] : boundingBoxes[label_i][1]+1, 
                                   ...
                                   boundingBoxes[label_i][4] : boundingBoxes[label_i][5]+1] = newSeg
                    #Add the grains
