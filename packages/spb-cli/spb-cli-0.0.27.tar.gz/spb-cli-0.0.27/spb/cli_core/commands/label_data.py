...
class LabelData():
    ...
    def upload_data(self, project, dataset_name, directory_path, include_label):
        ...
        console.print(f'Successful upload of {success_data_count} out of {len(asset_images)} data. ({round(success_data_count/len(asset_images)*100,2)}%)')

        if include_label:
            success_label_count=len(labels_path)-len(label_results[0])
            console.print(f'Successful upload of {success_label_count} out of {len(labels_path)} labels. ({round(success_label_count/len(labels_path)*100,2)}%)')
    ...
    def download(self, project, directory_path):
        ...
        console.print(f'Successful download of {label_success_count} out of {label_count} labels. ({round(label_success_count/label_count*100,2)}%)')
        data_success_count = label_count - len(data_results)
        console.print(f'Successful download of {data_success_count} out of {label_count} data. ({round(data_success_count/label_count*100,2)}%)')
    ...
    def _print_error_table(self, data_results = None, label_results = None):
        ...
        while True:
            ...
            if isinstance(data_results, dict):
                table.add_column("DATA UPLOAD", justify="right")
            if isinstance(label_results, dict):
                table.add_column("LABEL UPLOAD", justify="right")
...
def _download_worker(args):
    ...
    for label in labels:
        ...
        try:
            ...
            error = {'label':'Error while downloading label file.'}
        try:
            ...
            error.update({'data':'Error while downloading data file.'})
