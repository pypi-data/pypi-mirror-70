...
class StatusCastAPIDriver: 
	def __init__(self): 
		...
		self.password = config['status']['password']
		self.username = config['status']['username']
		self.base_url = config['status']['base_url']
	...
	def httpGet(self, endpoint,data=None):
		# Use authorized session to perform request
		response = self.auth_session.get(endpoint,params=data,verify=verify_ssl)

	def httpPost(self, endpoint,data): 
		if data is None:
			print("Payload for POST request must not be None")
			sys.exit(1)
		else: 
			# Use authorized session to perform request
			response = self.auth_session.post(endpoint,data=data,verify=verify_ssl)
	...
	def getAllSubscribers(self): 
		""" 
		For i in total pages of subscribers, request and print currentPage = i
		""" 
		print()
