...
def create_release(repo, new_version, message, project, deployed_cards_link):
    ...
    if new_release:
        ...
        post_on_slack(project, new_release, new_version, deployed_cards_link)
...
def post_on_slack(project, new_release, new_version, deployed_cards_link):
    with Halo(text="Scrivo su Slack...", spinner="dots", color="magenta"):
        response = slack.post(
            "deploy_channel",
            f"Ho effettuato il deploy di {project}. Nuova release con versione <{new_release.html_url}|{new_version}>",
        )
        if response is not None and len(deployed_cards_link) > 0:
            slack.reply_thread(
                "deploy_channel",
                response["ts"],
