...
class StyleAnalysisProcessor:
    ...
    def __init__(self, universe: str = _STOCK_FACTOR_RETURN_SOURCE_INDEX):
        self._updated_count: Dict[str, float] = {}
    ...
    def _upload_derived(self, df: pd.DataFrame, table_name: str):
        print(table_name)
        df.to_sql(table_name, DerivedDatabaseConnector().get_engine(), index=False, if_exists='append')

        if table_name not in self._updated_count:
            self._updated_count[table_name] = 0
    ...
    def process(self, start_date, end_date):
        ...
        try:
            ...
            # 将风格因子收益率的时间序列存到DB
            self._upload_derived(self._factor_return.loc[start_date:end_date, :].reset_index(), StyleAnalysisFactorReturn.__table__.name)
...
if __name__ == "__main__":
    StyleAnalysisProcessor('csi800').process('2005-01-01', '2014-12-31')
