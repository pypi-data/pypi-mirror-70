...
class ConfuseCallback(tf.keras.callbacks.Callback):
    def __init__(self, x_test, y_test, file_writer, class_names=None, y_argmax=False, figure_size=(12, 10)):
        ...
        if self.class_names is None:
            self.class_names = ["Class {:02d}".format(unique_label) for unique_label in np.unique(np.argmax(y_test, axis=1) if y_argmax else y_test)]

    def get_precision_recall_plot(self, con_mat):
        precisions = np.array([0] * len(self.class_names)).astype('float32')
        recalls = np.array([0] * len(self.class_names)).astype('float32')
        ...
        df = pd.DataFrame((self.class_names, precisions, recalls)).T
