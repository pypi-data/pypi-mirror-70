...
def download_folder_to_path(
    ...
    if asynchronous:
        raise NotImplementedError("Asynchronous downloading is WIP.")
...
async def _download_blobs_async(blobs, flatten, sublength, path):

    async with aiohttp.ClientSession() as session:
        storage_client = Storage(session=session)
        async for blob in async_list(blobs):
            if flatten:
                filename = blob.name.split('/')[-1]
            else:
                filename = os.path.join(*blob.name.split('/')[sublength:])
            ...
            async with aiofiles.open(os.path.join(full_filename), 'wb') as af:
                ...
                try:
                    await af.write(
                        await storage_client.download(blob.bucket.name, blob.name)
                        )
                except: # Sometimes there is an error with the MD5 hash, so retry.
                    await af.write(
                        await storage_client.download(blob.bucket.name, blob.name)
                        )
