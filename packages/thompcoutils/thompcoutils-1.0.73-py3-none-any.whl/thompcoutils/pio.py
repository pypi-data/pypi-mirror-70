...
class ADC:
    gains = {1: 4.096, 2: 6.144, 3: 6.144, 4: 1.024, 8: 0.512, 16: 0.256}
    running = False
    gain = None
    r1 = None
    r2 = None
    ...
    def __init__(self, gain=1, r1=1000000, r2=220000, tolerance=.1, update=.25):
        # Create an ADS1115 ADC (16-bit) instance.
        chip = 'ADS1115'

        if chip == 'ADS1115':
            ADC.adc = Adafruit_ADS1x15.ADS1115()
            ADC.resolution_range = {min: -32768, max: 32767}
        elif chip == 'ADS1015':
            ADC.adc = Adafruit_ADS1x15.ADS1015(address=0x49, busnum=1)
            ADC.resolution_range = {min: -2048, max: 2047}
        else:
            raise RuntimeError("Chip not selected")
        ADC.gain = gain
        ADC.r1 = r1
        ADC.r2 = r2
        ADC.tolerance = tolerance
        ADC.update = update
        ADC.thread = ThreadManager(ADC.thread_name, ADC._adc_thread)

    @staticmethod
    def stop():
        ADC.running = False
        ThreadManager.join_thread(ADC.thread_name)

    @staticmethod
    def _adc_thread():
        last_voltage = [0] * 4
        multiplier = 2 * ADC.gains[ADC.gain] / (ADC.resolution_range[max] - ADC.resolution_range[min])
        while ADC.running:
            values = [0] * 4
            voltages = [0] * 4
            append = False
            ...
            for i in range(4):
                ...
                v2 = values[i] * multiplier
                v3 = ((ADC.r1 * v2) / ADC.r2) + v2
                voltages[i] = v3
                if v3 > last_voltage[i] + ADC.tolerance or v3 < last_voltage[i] - ADC.tolerance:
                    append = True
                last_voltage[i] = v3
            if append:
                ADC.adc_queue.put_nowait(ADC.Values(time_stamp, values, voltages))
            time.sleep(ADC.update)

    @staticmethod
    def start():
        ADC.running = True
        ThreadManager.start_thread(ADC.thread_name)


class Button:
    # check out this article on debouncing:
    # https://www.raspberrypi.org/forums/viewtopic.php?t=134394
    def __init__(self, pin, callback, pull_up_down=GPIO.PUD_UP):
        self.pin = pin
    ...
    def _debounce_function(self, pin):
        ...
        if (time_now - self.last_push).microseconds > .1 * units.microseconds_per_second:
            if current_state:
                ...
                self.callback(pin, datetime.datetime.now() - self.press_time)
        self.last_push = time_now
