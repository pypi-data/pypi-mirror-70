...
def button_callback(pin, state, package):
    print('{} Pin {} now at {}.  Message:{}'.format(datetime.datetime.now(), pin, state, package))


def timed_button_callback(pin, duration, package):
    print('{} Pin {} now down for {}.  Message;{}'.format(datetime.datetime.now(), pin, duration, package))
...
class TestPio(unittest.TestCase):
    ...
    def dont_test_TimedButton(self):
        ...
        pio.TimedButton(16, timed_button_callback, "test TimedButton")
        while True:
            time.sleep(1)
