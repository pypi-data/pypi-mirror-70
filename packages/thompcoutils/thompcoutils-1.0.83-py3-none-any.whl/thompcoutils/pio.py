...
class ADC:
    ...
    def _adc_thread(adc):
        ...
        if append:
            adc.queue.put_nowait(adc.Values(time_stamp, values, voltages))
...
class TemperatureHumidity:
    ...
    def _get(self):
        ...
        if pio_loaded:
            self.ready = False
            self.humidity, temp = Adafruit_DHT.read_retry(self.sensor_type, self.pin)
            if self.humidity is not None and temp is not None:
                self.ready = True
