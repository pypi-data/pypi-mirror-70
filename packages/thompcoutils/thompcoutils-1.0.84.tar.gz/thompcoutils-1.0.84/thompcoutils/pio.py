...
class ADC:
    ...
    def __init__(self, chip_type=SensorType, gain=1, r1=1000000, r2=220000, tolerance=.1, update=.25):
        ...
        if chip_type == ADC.SensorType.ADS1115:
            ...
            raise RuntimeError("Invalid Chip Type")
        self.multiplier = 2 * self.gains[gain] / (self.resolution_range[max] - self.resolution_range[min])
        self.worker_thread = WorkerThread(callback_function=ADC._get_data_thread, sleep=self.update, parameters=self)
...
class TemperatureHumidity:
    ...
    @staticmethod
    def _get_data_thread(sensor):
        logger = get_logger()
        if pio_loaded:
            humidity, temperature = Adafruit_DHT.read_retry(sensor.sensor_type, sensor.pin)
            if humidity is not None and temperature is not None:
                if not sensor.centigrade:
                    temperature = 9.0 / 5.0 * temperature + 32
                logger.debug("pin:{}, temp:{}, humidity:{}".format(sensor.pin, temperature, humidity))
                sensor.queue.put(TemperatureHumidity.Values(temperature, humidity))
    ...
    def __init__(self, pin, sensor_type, centigrade=False):
        ...
        self.worker_thread = WorkerThread(callback_function=TemperatureHumidity._get_data_thread, sleep=1, parameters=self)
