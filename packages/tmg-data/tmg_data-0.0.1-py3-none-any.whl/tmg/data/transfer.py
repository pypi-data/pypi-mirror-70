...
class Client:
    ...
    def bq_to_gs(self, table, bucket_name, separator=',', print_header=True):
        ...
        logger.info('Extracting from {table} to gs://{bucket_name}/{table_id}_*'.format(
        ...
        logger.info('Getting the list of available blobs in gs://{}'.format(bucket_name))
    ...
    def gs_to_bq(self, gs_uri, table, auto_detect=True, skip_leading_rows=True, separator=',',
        ...
        logger.info('Loading BigQuery table {} from {}'.format(table, gs_uri))
    ...
    def bq_to_mysql(self, connection_string, bq_table, mysql_table):
        ...
        if len(file_names) > 1:
            logger.info('Merging {}'.format(','.join(file_names)))
    ...
    def mysql_to_gs(self, instance_name, database, query, gs_uri):
        ...
        logger.info('Exporting from MySQL database {} with query "{}" to {}'.format(database, query, gs_uri))
    ...
    def mysql_to_bq(self, instance_name, database, query, bq_table, bq_table_schema, write_preference='append'):
        ...
        logger.info('Creating the temporary bucket {}'.format(tmp_bucket_name))
        ...
        logger.info('Cleaning the exported file {}.csv'.format(tmp_file_name))
        ...
        # cleanup
        blob.delete()
        logger.info('Deleting temporary bucket {}'.format(tmp_bucket_name))
        tmp_bucket.delete()
    ...
    def bq_to_ftp(self, bq_table, ftp_connection_string, ftp_filepath, separator=',', print_header=True):
        ...
        if len(local_files) > 1:
            logger.info('Merging {}'.format(','.join(local_files)))
