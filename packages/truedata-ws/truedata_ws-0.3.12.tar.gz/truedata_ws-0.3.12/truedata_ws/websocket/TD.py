...
class LiveClient(WebSocketApp):
    ...
    def __init__(self, parent_app, url, *args):
        WebSocketApp.__init__(self, url, on_error=self.ws_error, on_message=self.on_msg_func, *args)
        self.segments = []
        self.max_symbols = 0
    ...
    def handle_trade_data(self, trade_tick):
        try:
            ...
            print(f'{Style.BRIGHT}{Fore.RED}This symbol(tick) is not tied to any req_id...{Style.RESET_ALL}')
    ...
    def handle_bid_ask_data(self, bidask_tick):
        try:
            ...
            print(f'{Style.BRIGHT}{Fore.RED}This symbol(bid-ask) is not tied to any req_id...{Style.RESET_ALL}')
    ...
    def handle_bar_data(self, bar_data):
        try:
            ...
            print(f'{Style.BRIGHT}{Fore.RED}This symbol(bar) is not tied to any req_id...{Style.RESET_ALL}')
...
class HistoricalWebsocket:
    ...
    def get_hist_bar_data(self, contract, end_time, start_time, bar_size):
        # print(f'{{"method": "gethistory", "interval": "{bar_size}", "symbol": "{contract}", "from": "{start_time}", "to": "{end_time}"}}')
...
class TD:
    ...
    def get_historical_data_from_duration(self, contract,
        ...
        if bar_size == 'tick':
            hist_data = self.historical_websocket.get_hist_tick_data(contract, end_time, start_time)
        else:
            bar_size = bar_size.replace(' ', '')
            if bar_size[-1] == 's':
                bar_size = bar_size[:-1]
            hist_data = self.historical_websocket.get_hist_bar_data(contract, end_time, start_time, bar_size)
