...
def user_timeline_extraction (node_list, consumerKey, consumerSecret,accessToken,accessTokenSecret, iter_tw,save_by, ruta_fichero, nombre_fichero):
    ...
    while ((k<= (len(node_list)+profile_iter)) & (ultima !=2) ):
      ...
      if(len(aux) != 0):  
        ...
        if path.exists(ruta_fichero + nombre_fichero):
          df2.to_csv(nombre_fichero,mode="a", index=False, header=False)
        else:
          df2.to_csv(nombre_fichero,index=False)         
        if len(list_users_noInfo) !=0:
          nodes = pd.DataFrame()
          nodes["nodos"] = list_users_noInfo
          if path.exists(ruta_fichero+"node_without_"+nombre_fichero):
            nodes.to_csv("node_without_"+ nombre_fichero, mode ="a",index=False, header=False)          
          else:
             nodes.to_csv("node_without_"+ nombre_fichero, index=False)
        k = k+profile_iter        
        if ultima ==1:          
          ultima =2
      else:
...
def NLP_analysis(data, keyword_corpus, hashtag_corpus):
    tweets_df = pd.DataFrame({'text':data["text_tweets_days_list"],'RT':data["RT_days_list"],'lang': data["lang_days_list"]})
    tweets_dff = pd.DataFrame()
    for i in range(len(tweets_df)):
      aux = pd.DataFrame({'text':tweets_df["text"].iloc[i],'RT':tweets_df["RT"].iloc[i],'lang': tweets_df["lang"].iloc[i]})
      tweets_dff = tweets_dff.append(aux, ignore_index=False).reset_index(drop=True)
...
def more_tweets_required(df,df_graph_features,consumerKey,consumerSecret,accessToken,accessTokenSecret,n_groups,save_by,ruta_fichero,nombre_fichero):
	...
	if len(node_list_moreData) >0:
		df_metadata_moreData, list_more_Data_noData = user_timeline_extraction(node_list_moreData,consumerKey,consumerSecret,accessToken,accessTokenSecret,n_groups,save_by,ruta_fichero, nombre_fichero)
		df_metadata_moreData = pd.read_csv(ruta_fichero + nombre_fichero)
...
def text_to_list_features_conversor (data):
    for index, row in data.iterrows():
        ...
        for i in range(len(text_days)):
            ...
            text_tweets_day = data["text_tweets_days_list"].iloc[index][i].replace("', '","¿¿??").strip("[]'").split("¿¿??")
            if len(RT_days[i])==1:
                 RT_day = list(RT_days[i])
            else:
