...
def upload_file(storj_obj, project, bucket_name, storj_path, upload_options, src_full_filename):
    ...
    upload_result, ls_error = storj_obj.upload_object(project, bucket_name, storj_path,
                                                      upload_options)
    ...
    while uploaded_total < data_len:
        ...
        lc_data_to_write = (c_uint8 * c_int32(len(lc_data_to_write)).value)(*lc_data_to_write)
        ...
        write_result, ls_error = storj_obj.upload_write(upload_result.upload, lc_data_to_write_ptr,
                                                        size_to_write)
        if ls_error is not None:
            return False, ls_error
        #
...
def download_file(storj_obj, project, bucket_name, storj_path, download_options,
    ...
    download_result, ls_error = storj_obj.download_object(project, bucket_name, storj_path,
                                                          download_options)
    if ls_error is not None:
        return False, ls_error
    #
    ...
    while True:
        ...
        lc_data_read_ptr, read_result, ls_error = storj_obj.download_read(download_result.download,
                                                                          size_to_read)
        if ls_error is not None:
            return False, ls_error
        #
...
if __name__ == "__main__":
    ...
    objects_list, err = StorjObj.list_objects(project_result.project, MY_BUCKET, lO_listOption)
    if err is not None:
        print(err)
        sys.exit()
    else:
