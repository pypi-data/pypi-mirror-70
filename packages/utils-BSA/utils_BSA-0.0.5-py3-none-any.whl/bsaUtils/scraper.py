...
def get_ground_sensors_data(HEADERS, stations_filename, base_url, AQ_DIR):
    ...
    stations = []
    loop.run_until_complete(scraper.get_station(stations, stations_filename, base_url, HEADERS))
    for batch_idx in range(0, len(stations), concurrent_requests):
        loop.run_until_complete(asyncio.gather(
            *(scraper.get_data(station, AQ_DIR, base_url, HEADERS) for station in stations[batch_idx:batch_idx+concurrent_requests])
        ))
