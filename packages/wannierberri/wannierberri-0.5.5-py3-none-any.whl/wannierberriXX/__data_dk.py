...
# which after summing over l',n' will give the generalized derivative
    ...
    @lazy_property.LazyProperty
    def gdOmegabar(self):
        dOn= self.Omega_bar_der_rediag
        dOln= self.Omega_Hbar[:,:,:,:,None].transpose(0,2,1,3,4)*self.D_H[:,:,:,None,:]-self.D_H[:,:,:,None,:].transpose(0,2,1,3,4)*self.Omega_Hbar[:,:,:,:,None]
    ...
    def gdHbar(self):
        Hbar = self.Morb_Hbar
        dHn= self.Morb_Hbar_der_diag
        dHln= Hbar[:,:,:,:,None].transpose(0,2,1,3,4)*self.D_H[:,:,:,None,:]-self.D_H[:,:,:,None,:].transpose(0,2,1,3,4)*Hbar[:,:,:,:,None]
    ...
    def derHplusTr(self):
        ...
        uo =( (dHln + dOln*E[:,:,N,N,N]).real 
            ...
            + Dnl[:,:,:,b,N]*dBPln[:,:,:,c,:] ) 
            - (Bplus[:,:,:,c,N]*dDln[:,:,:,b,:] 
            + Dnl[:,:,:,c,N]*dBPln[:,:,:,b,:])).real 
            + 2*((E[:,:,N,N,N]+E[:,N,:,N,N])*(Dnl[:,:,:,b,N]*dDln[:,:,:,c,:] 
            - Dnl[:,:,:,c,N]*dDln[:,:,:,b,:])).imag )
        
        uuo = -2*((Bplus[:,:,N,:,b,N]*dDlln[:,:,:,:,c,:] 
                 + Dnl[:,:,N,:,b,N]*dBPlln[:,:,:,:,c,:]) 
                 - (Bplus[:,:,N,:,c,N]*dDlln[:,:,:,:,b,:] 
                 + Dnl[:,:,N,:,c,N]*dBPlln[:,:,:,:,b,:])).real + 2*(E[:,:,N,N,N,N] + E[:,N,N,:,N,N])*(Dnl[:,:,N,:,b,N]*dDlln[:,:,:,:,c,:] - Dnl[:,:,N,:,b,N]*dDlln[:,:,:,:,c,:]).imag + (Dnl[:,:,N,:,b,N]*V[:,:,:,N,N,:]*Dln[:,N,:,:,c,N] - Dnl[:,:,N,:,c,N]*V[:,:,:,N,N,:]*Dln[:,N,:,:,b,N] ).imag
        
        uoo = -2*((Bplus[:,:,N,:,b,N]*dDlnn[:,:,:,:,c,:] 
                 + Dnl[:,:,N,:,b,N]*dBPlnn[:,:,:,:,c,:]) 
                 - (Bplus[:,:,N,:,c,N]*dDlnn[:,:,:,:,b,:] 
                 + Dnl[:,:,N,:,c,N]*dBPlnn[:,:,:,:,b,:])).real + 2*(E[:,:,N,N,N,N] + E[:,N,N,:,N,N])*(Dnl[:,:,N,:,b,N]*dDlnn[:,:,:,:,c,:] - Dnl[:,:,N,:,b,N]*dDlnn[:,:,:,:,c,:]).imag + (Dnl[:,:,N,:,b,N]*Dln[:,:,:,N,c,N]*V[:,N,:,:,N,:] - Dnl[:,:,N,:,c,N]*Dln[:,:,:,N,b,N]*V[:,N,:,:,N,:]).imag
