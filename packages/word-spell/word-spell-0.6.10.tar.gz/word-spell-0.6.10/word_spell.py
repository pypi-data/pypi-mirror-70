...
class Document():
    def __init__(self, path: str):
        ...
        if isinstance(self.xml, bytes):
            self.xml = self._get_word_xml().decode(encoding='utf-8')
    ...
    def render_from_template(self, out: str, **kwargs):
        ...
        xml = self.xml
        for v in vars:
            middle = r'(<[A-Za-z\"_ -=0-9<>/:\u4e00-\u9fa5]*>)?'.join(v)
            r = r'{(<[A-Za-z\"_ \-=0-9<>/:\u4e00-\u9fa5]*>)?{[ ]*(<[A-Za-z\_" \-=0-9<>/:\u4e00-\u9fa5]*>)?' + middle + r'(<[A-Za-z_<>=/0-9\- \":\u4e00-\u9fa5]*>)?[ ]*}(<[A-Za-z_<>=/0-9\- \":\u4e00-\u9fa5]*>)?}'
            exp = re.compile(r)
